...
ensure_edge_env_values() {
    ...
    ensure_env_value "$env_file" AUTOSCALE_UPSTREAM_LANE_PRESSURE_ENABLED true
    ...
    upgrade_env_default_value "$env_file" SUB2API_EDGE_GLOBAL_WORKERS 131072 9999
    ...
    upgrade_env_default_value "$env_file" SUB2API_EDGE_QUEUE_WAIT_BUDGET_MS 100 50
...
ensure_scheduler_env_values() {
    ...
    local legacy_autoscale_upstream_error_ratio
	...
	legacy_autoscale_upstream_error_ratio="$(read_env_value "$env_file" AUTOSCALE_UPSTREAM_ERROR_RATIO)"
	ensure_env_value "$env_file" AUTOSCALE_UPSTREAM_ERROR_RATIO 0.20
	ensure_env_value "$env_file" AUTOSCALE_GO_5XX_RATIO_LIMIT "${legacy_autoscale_upstream_error_ratio:-0.20}"
	ensure_env_value "$env_file" AUTOSCALE_EDGE_CONNECT_ERROR_RATIO_LIMIT 0.10
	ensure_env_value "$env_file" AUTOSCALE_EDGE_5XX_RATIO_LIMIT 0.20
	ensure_env_value "$env_file" AUTOSCALE_EDGE_429_RATIO_LIMIT 0
	ensure_env_value "$env_file" AUTOSCALE_EDGE_RETRIES_PER_REQUEST_LIMIT 0
	ensure_env_value "$env_file" AUTOSCALE_ERROR_MIN_SAMPLES 20
...
AUTOSCALE_UPSTREAM_LANE_PRESSURE_ENABLED=true
...
services:
  app:
    ...
    environment:
      ...
      - SIMPLE_MODE_AUTO_CREATE_DEFAULT_GROUPS=\${SIMPLE_MODE_AUTO_CREATE_DEFAULT_GROUPS:-true}
  ...
  autoscaler:
    ...
    environment:
      - COMPOSE_PROJECT_NAME=${COMPOSE_PROJECT_NAME}
      # The drain endpoints are authenticated independently on the Go and
      # Edge listeners. Keep both secrets available to the autoscaler so a
      # scale-down can drain a pair before Compose removes its container.
      - SUB2API_EDGE_INTERNAL_SECRET=\${SUB2API_EDGE_INTERNAL_SECRET:?SUB2API_EDGE_INTERNAL_SECRET is required}
      - GATEWAY_OPENAI_EDGE_RS_INTERNAL_SECRET=\${GATEWAY_OPENAI_EDGE_RS_INTERNAL_SECRET:?GATEWAY_OPENAI_EDGE_RS_INTERNAL_SECRET is required}
      ...
      - AUTOSCALE_GO_5XX_RATIO_LIMIT=\${AUTOSCALE_GO_5XX_RATIO_LIMIT:-}
      - AUTOSCALE_EDGE_CONNECT_ERROR_RATIO_LIMIT=\${AUTOSCALE_EDGE_CONNECT_ERROR_RATIO_LIMIT:-0.10}
      - AUTOSCALE_EDGE_5XX_RATIO_LIMIT=\${AUTOSCALE_EDGE_5XX_RATIO_LIMIT:-0.20}
      - AUTOSCALE_EDGE_429_RATIO_LIMIT=\${AUTOSCALE_EDGE_429_RATIO_LIMIT:-0}
      - AUTOSCALE_EDGE_RETRIES_PER_REQUEST_LIMIT=\${AUTOSCALE_EDGE_RETRIES_PER_REQUEST_LIMIT:-0}
      - AUTOSCALE_ERROR_MIN_SAMPLES=\${AUTOSCALE_ERROR_MIN_SAMPLES:-20}
      - AUTOSCALE_UPSTREAM_LANE_PRESSURE_ENABLED=\${AUTOSCALE_UPSTREAM_LANE_PRESSURE_ENABLED:-true}
      - REDIS_PASSWORD=\${REDIS_PASSWORD:-}
