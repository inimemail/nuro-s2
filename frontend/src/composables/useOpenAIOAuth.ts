...
export function useOpenAIOAuth() {
  ...
  const deviceExpiresIn = ref(0)
  const deviceInterval = ref(5)
  ...
  const resetState = () => {
    ...
    deviceExpiresIn.value = 0
    deviceInterval.value = 5
    loading.value = false
    error.value = ''
  }
  ...
  const startDeviceAuth = async (proxyId?: number | null): Promise<boolean> => {
    loading.value = true
    authUrl.value = ''
    sessionId.value = ''
    oauthState.value = ''
    ...
    try {
      const response = await adminAPI.accounts.startOpenAIDeviceAuth(proxyId)
      sessionId.value = response.session_id
      deviceUserCode.value = response.user_code
      deviceVerificationUrl.value = response.verification_url
      deviceExpiresIn.value = response.expires_in || 0
      deviceInterval.value = response.interval || 5
      return true
    } catch (err: any) {
      error.value = extractApiErrorMessage(err, t('admin.accounts.oauth.openai.failedToGenerateUrl'))
      appStore.showError(error.value)
  ...
  const exchangeDeviceAuth = async (
    currentSessionId: string,
    proxyId?: number | null
  ): Promise<OpenAITokenInfo | null> => {
    if (!currentSessionId) {
      error.value = 'Missing device auth session ID'
      return null
    }
    ...
    try {
      const tokenInfo = await adminAPI.accounts.exchangeOpenAIDeviceAuth(currentSessionId, proxyId)
