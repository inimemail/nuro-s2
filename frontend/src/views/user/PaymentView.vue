      ...
      <Transition name="modal">
        <div v-if="showRenewalModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" @click.self="closeRenewalModal">
          <div class="relative flex max-h-full w-full max-w-lg flex-col rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900">
            ...
            <div class="min-h-0 space-y-4 overflow-y-auto overscroll-contain pr-1">
...
const subFeeAmount = computed(() => {
  const paymentAmount = subPaymentAmount.value
  if (feeRate.value <= 0 || paymentAmount <= 0) return 0
  return ceilPaymentAmount((paymentAmount * feeRate.value) / 100, selectedCurrency.value)
})

const subTotalAmount = computed(() => {
  const paymentAmount = subPaymentAmount.value
  if (feeRate.value <= 0 || paymentAmount <= 0) return paymentAmount
  return roundPaymentAmount(paymentAmount + subFeeAmount.value, selectedCurrency.value)
})
