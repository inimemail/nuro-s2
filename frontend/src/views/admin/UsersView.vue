<template>
  <AppLayout>
    <TablePageLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <button class="btn btn-secondary flex-1 md:flex-initial" data-test="bulk-edit-limits" @click="showBulkEditModal = true">
              <Icon name="users" size="md" class="mr-2" />
              {{ selectedCount > 0
                ? t('admin.users.bulkLimits.action', { count: selectedCount })
                : t('admin.users.bulkLimits.actionAll') }}
            </button>
          </div>
        </div>
      </template>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-usage_kimi="{ row }">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'kimi')" />
          </template>
          ...
          <template #cell-usage_minimax="{ row }">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'minimax')" />
          </template>
    ...
    <BulkEditUserModal :show="showBulkEditModal" :selected-ids="selectedIds" @close="showBulkEditModal = false" @success="handleBulkLimitsSuccess" />
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'usage_kimi', label: t('admin.users.columns.usageKimi'), sortable: false },
  ...
  { key: 'usage_minimax', label: t('admin.users.columns.usageMiniMax'), sortable: false },
...
const DEFAULT_HIDDEN_COLUMNS = [
  ...
  'usage_kimi', 'usage_zhipu', 'usage_deepseek', 'usage_minimax',
...
const VERSION_NEW_HIDDEN_COLUMNS: Record<number, string[]> = {
  ...
  4: ['usage_grok'],
  5: ['usage_kimi', 'usage_zhipu', 'usage_deepseek'],
  6: ['usage_minimax']
}
...
const USAGE_COLUMN_KEYS: readonly string[] = ['usage', 'usage_anthropic', 'usage_openai', 'usage_gemini', 'usage_antigravity', 'usage_grok', 'usage_kimi', 'usage_zhipu', 'usage_deepseek', 'usage_minimax']
...
const builtInFilters = computed(() => [
  ...
  { key: 'apiKeyGroup', name: t('admin.users.apiKeyGroup'), type: 'select' as const }
])
...
const loadSavedFilters = () => {
  try {
    ...
    if (savedValues) {
      ...
      if (parsed.apiKeyGroup) filters.apiKeyGroup = parsed.apiKeyGroup
...
const { selectedIds, selectedCount, setSelectedIds, clear: clearSelection } = useTableSelection<AdminUser>({
  rows: sortedUsers,
  getId: user => user.id
})
...
const getUserSelectionLabel = (user: AdminUser) => t('admin.users.bulkLimits.selectUser', { email: user.email })
  ...
  try {
    ...
    const response = await adminAPI.users.list(
      ...
      pagination.page_size,
      {
        ...
        api_key_group_id: filters.apiKeyGroup ? Number(filters.apiKeyGroup) : undefined,
...
const toggleBuiltInFilter = (key: string) => {
  if (visibleFilters.has(key)) {
    ...
    if (key === 'apiKeyGroup') filters.apiKeyGroup = ''
  } else {
