...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.gatewayForwarding.codexHardeningTitle": "Codex 设置",
    "admin.settings.gatewayForwarding.codexClientRestrictionTitle": "Codex 客户端限制",
    "admin.settings.gatewayForwarding.codexHardeningDesc": "仅对开启 Codex-only 的账号生效。",
    "admin.settings.gatewayForwarding.minCodexVersion": "最低 Codex 版本",
    "admin.settings.gatewayForwarding.minCodexVersionPlaceholder": "例如 0.142.0",
    "admin.settings.gatewayForwarding.maxCodexVersion": "最高 Codex 版本",
    "admin.settings.gatewayForwarding.maxCodexVersionPlaceholder": "例如 0.200.0",
    "admin.settings.gatewayForwarding.codexVersionHint": "版本限制说明",
    "admin.settings.gatewayForwarding.codexFingerprintSignals": "Codex 引擎指纹信号",
    "admin.settings.gatewayForwarding.codexFingerprintSignalsDesc": "指纹说明",
    "admin.settings.gatewayForwarding.codexFpTypeHeaderExact": "头精确",
    "admin.settings.gatewayForwarding.codexFpTypeHeaderPrefix": "头前缀",
    "admin.settings.gatewayForwarding.codexFpTypeBodyPath": "body 路径",
    "admin.settings.gatewayForwarding.codexFpMatchPlaceholder": "匹配内容",
    "admin.settings.gatewayForwarding.codexFpRequired": "必须",
    "admin.settings.gatewayForwarding.codexFingerprintNoRequiredWarn": "未启用指纹门",
    "admin.settings.gatewayForwarding.codexAllowAppServer": "Codex app-server",
    "admin.settings.gatewayForwarding.codexAllowAppServerDesc": "app-server 说明",
    "admin.settings.gatewayForwarding.codexBlacklist": "黑名单",
    "admin.settings.gatewayForwarding.codexBlacklistDesc": "黑名单说明",
    "admin.settings.gatewayForwarding.codexWhitelist": "白名单",
    "admin.settings.gatewayForwarding.codexWhitelistDesc": "白名单说明",
    "admin.settings.gatewayForwarding.codexWhitelistSkipFingerprint": "跳过引擎指纹",
    "admin.settings.gatewayForwarding.codexWhitelistSkipFingerprintTooltip": "跳过说明",
    "admin.settings.gatewayForwarding.codexOriginatorPlaceholder": "originator",
    "admin.settings.gatewayForwarding.codexUaContainsPlaceholder": "User-Agent",
    "admin.settings.gatewayForwarding.codexAddRow": "添加一条",
    "admin.settings.gatewayForwarding.codexRemoveRow": "删除",
...
async function openSettingsTab(wrapper: ReturnType<typeof mountView>, tab: string) {
  const tabButton = wrapper
    .findAll("button")
    .find((node) => node.text().includes(`admin.settings.tabs.${tab}`));
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not render project GitHub payment guidance links", async () => {
    const wrapper = mountView();
...
describe("admin SettingsView platform quota matrix", () => {
  ...
  it("点击设置菜单各 tab 不会白屏，Users tab 保持 5 平台配额矩阵", async () => {
    const wrapper = mountView();
    await flushPromises();

    for (const tab of ["general", "agreement", "features", "security", "users", "gateway", "payment", "email", "backup"]) {
      await openSettingsTab(wrapper, tab);
      expect(wrapper.find("form").exists()).toBe(true);
      expect(wrapper.text().trim().length).toBeGreaterThan(0);
    }

    await openUsersTab(wrapper);
    expect(wrapper.findAll(".quota-matrix-row").length).toBeGreaterThanOrEqual(5);
  ...
  it("空输入（v-model.number 产出 \"\"）在提交时清洗为 null 而非空字符串", async () => {
    ...
    const anthropicDailyInput = inputs.find((i) => {
      const parent = i.element.closest(".quota-matrix-row");
