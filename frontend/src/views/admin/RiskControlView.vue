...
async function loadAll() {
  loading.value = true
  try {
    // Proxy selection is optional. A transient proxy-list failure must not
    // prevent the moderation configuration and runtime state from loading.
    const proxyLoader = (adminAPI as typeof adminAPI & { proxies?: { getAll?: () => Promise<Proxy[]> } }).proxies?.getAll
    const proxyItemsRequest = proxyLoader
      ? proxyLoader().catch(() => [] as Proxy[])
      : Promise.resolve([] as Proxy[])
