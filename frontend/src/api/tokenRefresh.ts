...
function readStoredTokenPair(snapshot: AuthSnapshot): RefreshTokenResponse | null {
  ...
  if (!accessToken || !refreshToken || !Number.isFinite(expiresAt) || expiresAt <= Date.now() || getStoredUserID() !== snapshot.userID) {
    return null
  }
  return {
...
function readPeerRefreshResult(snapshot: AuthSnapshot, failedAccessToken?: string | null): RefreshTokenResponse | null {
  const storedPair = readStoredTokenPair(snapshot)
  if (!storedPair) return null
  if (storedPair.refresh_token !== snapshot.refreshToken) return storedPair
  if (failedAccessToken && snapshot.accessToken !== failedAccessToken && storedPair.access_token === snapshot.accessToken) {
    return storedPair
  }
  if (!failedAccessToken) {
    const expiresAt = Number(localStorage.getItem(TOKEN_EXPIRES_AT_KEY))
    if (expiresAt === snapshot.expiresAt && storedPair.access_token === snapshot.accessToken && expiresAt > Date.now() + TOKEN_REFRESH_BUFFER_MS) {
      return storedPair
    }
  }
  return null
...
function persistTokenPair(tokens: RefreshTokenResponse): void {
  ...
  // The rotating token is the cross-tab commit marker, so publish it last.
...
async function requestTokenPair(
  ...
  try {
    ...
    if (payload.code !== 0 || !payload.data) throw new Error(payload.message || 'Token refresh failed')

    if (localStorage.getItem(REFRESH_TOKEN_KEY) !== snapshot.refreshToken || getStoredUserID() !== snapshot.userID) {
      const peerResult = readPeerRefreshResult(snapshot, failedAccessToken)
      if (peerResult) return peerResult
    ...
    const peerResult = await waitForPeerRefresh(
      snapshot,
      failedAccessToken,
      isTokenRejection && mayHaveUncoordinatedPeer ? peerRefreshDeadline : Date.now() + PEER_REFRESH_WAIT_MS
    )
    if (peerResult) return peerResult
    throw error
  }
...
/** Shares refresh work in one document and serializes rotating tokens across tabs. */
export function refreshAuthTokens(options: RefreshAuthTokensOptions = {}): Promise<RefreshTokenResponse> {
