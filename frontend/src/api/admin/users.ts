...
export interface BatchUpdateUserLimitsResponse { affected: number }
...
export async function list(
  ...
  filters?: {
    ...
    api_key_group_id?: number   // exact filter by API key bound group
...
export async function batchUpdateLimits(request: BatchUpdateUserLimitsRequest): Promise<BatchUpdateUserLimitsResponse> {
  const { data } = await apiClient.post<BatchUpdateUserLimitsResponse>('/admin/users/batch-limits', request)
  return data
}
 ...
 */
export type PlatformQuotaPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'grok' | 'kimi' | 'zhipu' | 'deepseek' | 'minimax'
