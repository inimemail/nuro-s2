...
export async function saveOllamaCloudUsageSession(id: number, session: string): Promise<OllamaCloudUsageState> {
  const { data } = await apiClient.put<OllamaCloudUsageState>(`/admin/accounts/${id}/ollama-cloud-usage/session`, { session })
  return data
}
...
export async function updateOllamaCloudUsageSettings(settings: OllamaCloudUsageSettings): Promise<OllamaCloudUsageSettings> {
  const { data } = await apiClient.put<OllamaCloudUsageSettings>('/admin/accounts/ollama-cloud-usage/settings', settings)
  return data
}
...
export interface OpenAIDeviceAuthStartResponse {
  session_id: string
  user_code: string
...
export async function startOpenAIDeviceAuth(
  proxyId?: number | null
): Promise<OpenAIDeviceAuthStartResponse> {
  const payload: { proxy_id?: number } = {}
  if (proxyId) {
    payload.proxy_id = proxyId
  }
  const { data } = await apiClient.post<OpenAIDeviceAuthStartResponse>(
    '/admin/openai/device-auth/start',
    payload
  )
  return data
}

export async function exchangeOpenAIDeviceAuth(
  sessionId: string,
  proxyId?: number | null
  ...
  const { data } = await apiClient.post<Record<string, unknown>>(
    '/admin/openai/device-auth/exchange',
    payload
  )
  return data
}
...
export async function syncFromCrs(params: {
  ...
  const { data } = await apiClient.post<{
    ...
    timeout: 180000 // 180s timeout: sync refreshes existing OAuth accounts serially
  })
  return data
}
...
export async function exportData(options?: {
  ...
  if (options?.ids && options.ids.length > 0) {
    ...
    const { platform, type, status, group, privacy_mode, pool_mode, search, sort_by, sort_order } = options.filters
    if (platform) params.platform = platform
    if (type) params.type = type
    if (status) params.status = status
    if (group) params.group = group
    if (privacy_mode) params.privacy_mode = privacy_mode
    if (pool_mode) params.pool_mode = pool_mode
