<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="form.platform === 'openai' && isOpenAIRequestPathCreate"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between gap-4">
          <div>
    ...
    <div v-else class="space-y-5">
      <OAuthAuthorizationFlow
        ...
        @start-device-auth="handleOpenAIStartDeviceAuth"
...
const cnBaseUrlPresets = computed(() => {
  ...
  if (form.platform === 'zhipu') {
    return [{ key: 'zhipu', label: cnApiMode.value === 'anthropic' ? t('admin.accounts.cnProviders.anthropic') : t('admin.accounts.cnProviders.zhipu'), url: cnApiMode.value === 'anthropic' ? 'https://open.bigmodel.cn/api/anthropic' : (cnBillingMode.value === 'coding_plan' ? 'https://open.bigmodel.cn/api/coding/paas/v4' : 'https://open.bigmodel.cn/api/paas/v4') }]
  }
  if (form.platform === 'deepseek') {
    return cnApiMode.value === 'responses'
      ? [{ key: 'deepseekResponses', label: t('admin.accounts.cnProviders.responses'), url: 'https://api.deepseek.com' }]
      : cnApiMode.value === 'anthropic'
        ? [{ key: 'deepseekAnthropic', label: t('admin.accounts.cnProviders.anthropic'), url: 'https://api.deepseek.com/anthropic' }]
      : [{ key: 'deepseek', label: t('admin.accounts.cnProviders.deepseek'), url: 'https://api.deepseek.com/v1' }]
  }
  if (form.platform === 'minimax') return [{ key: 'minimax', label: 'MiniMax', url: 'https://api.minimaxi.com/v1' }]
  return []
})
...
const selectCNBillingMode = (mode: CNBillingMode) => {
  ...
  if (form.platform === 'minimax' && cnApiMode.value === 'adaptive') {
    if (apiKeyBaseUrl.value === 'https://api.minimaxi.com/v1' && mode === 'coding_plan') apiKeyBaseUrl.value = 'https://api.minimaxi.com/v1'
  }
  if (form.platform === 'zhipu' && cnApiMode.value === 'adaptive') {
  ...
  if (form.platform === 'zhipu' && cnApiMode.value !== 'adaptive' && cnApiMode.value !== 'anthropic') {
    const paygURL = 'https://open.bigmodel.cn/api/paas/v4'
    const codingURL = 'https://open.bigmodel.cn/api/coding/paas/v4'
    const oldURL = mode === 'coding_plan' ? paygURL : codingURL
    const nextURL = mode === 'coding_plan' ? codingURL : paygURL
    if (apiKeyBaseUrl.value.trim() === oldURL) apiKeyBaseUrl.value = nextURL
  }
...
const syncCNChatBaseUrl = () => {
  if (isCNProvider.value && cnApiMode.value === 'adaptive') {
    const primary = apiKeyBaseUrl.value.trim()
...
const selectCNApiMode = (mode: CNApiMode) => {
  if (mode !== 'adaptive') {
    if (cnApiMode.value === 'adaptive') {
      ...
      if (selectedURL) apiKeyBaseUrl.value = selectedURL
    } else {
      ...
      if (selected && known.includes(current)) apiKeyBaseUrl.value = selected.url
    }
  }
...
const isManualInputMethod = computed(() => {
  ...
  return method === 'manual' || method === 'device_code'
})
...
const canExchangeCode = computed(() => {
  ...
  if (form.platform === 'openai') {
    if (oauthFlowRef.value?.inputMethod === 'device_code') {
      return openaiOAuth.sessionId.value && !openaiOAuth.loading.value
    }
...
const applyHeaderOverrideCredentials = (
  ...
  applyHeaderOverride(credentials, headerOverrideEnabled.value, headerOverrideRows.value, mode)
  return true
}
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (isOpenAIRequestPathCreate.value) {
  ...
  if (
    ...
    delete extra.openai_responses_arguments_object_compat
  }
  ...
  if (
    ...
    const stageError = validateOpenAIApiKeyFirstTokenTimeoutStageConfig(openaiOAuthFirstTokenTimeoutStageConfig.value, t)
    if (stageError) {
...
const buildCNProviderExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (cnApiMode.value === 'adaptive') {
    ...
    extra.cn_api_base_url_overrides = Object.fromEntries(cnAdaptiveProtocolOptions.value.map(option => [option.value, adaptiveBaseUrlOverrides.value[option.value] === true]))
  } else {
...
const handleSubmit = async () => {
  ...
  if (form.platform === 'anthropic' && accountCategory.value === 'bedrock') {
    ...
    if (poolModeEnabled.value) {
      ...
      if (poolSoftCooldownEnabled.value) {
        credentials.pool_soft_cooldown_error_threshold = normalizePoolSoftCooldownErrorThreshold(poolSoftCooldownErrorThreshold.value)
      }
  ...
  applyInterceptWarmup(credentials, interceptWarmupRequests.value, 'create')
  if (!applyHeaderOverrideCredentials(credentials, form.platform, 'apikey', 'create')) {
    return
  }
...
const handleOpenAIStartDeviceAuth = async () => {
  await openaiOAuth.startDeviceAuth(form.proxy_id)
}
...
const handleOpenAIExchange = async (authCode: string) => {
  ...
  const isDeviceCode = oauthFlowRef.value?.inputMethod === 'device_code'
  if (!oauthClient.sessionId.value) return
  if (!isDeviceCode && !authCode.trim()) return
  ...
  try {
    let tokenInfo
    if (isDeviceCode) {
      tokenInfo = await oauthClient.exchangeDeviceAuth(oauthClient.sessionId.value, form.proxy_id)
    } else {
      ...
      tokenInfo = await oauthClient.exchangeAuthCode(
        authCode.trim(),
...
const handleGrokExchange = async (authCode: string) => {
  ...
  try {
    const tokenInfo = await grokOAuth.exchangeAuthCode({
      ...
      state: oauthFlowRef.value?.oauthState || grokOAuth.state.value,
      proxyId: form.proxy_id
    })
    if (!tokenInfo) return
