        ...
        <div v-if="inputMethod === 'device_code'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.openai.deviceCodeDesc') }}
            </p>
            ...
            >
              ...
              {{
                loading
                  ? t('admin.accounts.oauth.openai.deviceCodeStarting')
                  : t('admin.accounts.oauth.openai.deviceCodeStart')
              }}
            </button>
            ...
            <div v-else class="space-y-4">
              <div>
                <label class="input-label">
                  <Icon name="link" size="sm" class="mr-1 inline text-blue-500" />
                  {{ t('admin.accounts.oauth.openai.deviceVerificationUrl') }}
                </label>
                <div class="flex items-center gap-2">
                  ...
                  <button
                    ...
                    :title="t('common.copy')"
                    @click="copyToClipboard(deviceVerificationUrl, 'URL copied to clipboard')"
                  >
              ...
              <div>
                <label class="input-label">
                  ...
                  {{ t('admin.accounts.oauth.openai.deviceUserCode') }}
                </label>
                <div class="flex items-center gap-2">
                  <input
                    ...
                    readonly
                    type="text"
                    class="input flex-1 bg-gray-50 font-mono text-lg font-semibold tracking-wide dark:bg-gray-700"
                  />
                  <button
                    type="button"
                    class="btn btn-secondary p-2"
                    :title="t('common.copy')"
                    @click="copyToClipboard(deviceUserCode, 'Code copied to clipboard')"
                  >
                    <Icon name="copy" size="sm" />
                  </button>
                </div>
                <p class="mt-2 text-xs text-blue-600 dark:text-blue-400">
                  {{ t('admin.accounts.oauth.openai.deviceCodeHint', { seconds: deviceExpiresIn || 900 }) }}
                </p>
              </div>

              <div class="rounded border border-amber-300 bg-amber-50 p-3 dark:border-amber-700 dark:bg-amber-900/30">
                <p class="text-xs text-amber-800 dark:text-amber-300">
                  {{ t('admin.accounts.oauth.openai.deviceCodeSecurityNotice') }}
                </p>
              </div>
...
const showMethodSelection = computed(() => props.showCookieOption || props.showRefreshTokenOption || props.showMobileRefreshTokenOption || props.showDeviceCodeOption || props.showSessionTokenOption || props.showAccessTokenOption || props.showCodexSessionImportOption || props.showAgentIdentityOption)
...
const handleStartDeviceAuth = () => {
  emit('start-device-auth')
}

const handleRestartDeviceAuth = () => {
  emit('start-device-auth')
}
