...
const showUsageWindows = computed(() => {
  ...
  if (props.account.type === 'apikey' && ['kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go'].includes(props.account.platform)) return true
...
const grokFreeTokenBar = computed(() => {
  ...
  return {
    utilization: Math.min(100, Math.max(0, (stats.tokens / limit) * 100)),
    limit,
    resetsAt: null
  }
})
