    ...
    >
      ...
      <div v-if="account.type === 'apikey'" class="space-y-4">
        <div v-if="!isCNProvider || cnApiMode !== 'adaptive'">
          ...
          />
          <div v-if="isCNProvider" class="mt-3">
            ...
            <div class="grid gap-2 sm:grid-cols-2">
              <button
                ...
                @click="selectCNBaseUrlPreset(preset.url)"
              >
          ...
          <div>
            ...
            <div class="mt-2 grid gap-3 sm:grid-cols-2" data-testid="cn-account-type-options">
              ...
              >
                ...
                <div class="min-w-0"><span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.cnProviders.payg') }}</span><span class="mt-0.5 block text-xs leading-5 text-gray-500 dark:text-gray-400">{{ t('admin.accounts.cnProviders.paygDesc') }}</span></div>
              </button>
              <button
              ...
              >
                ...
                <div class="min-w-0"><span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.cnProviders.codingPlan') }}</span><span class="mt-0.5 block text-xs leading-5 text-gray-500 dark:text-gray-400">{{ t('admin.accounts.cnProviders.codingPlanDesc') }}</span></div>
              </button>
            </div>
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.cnProviders.apiProtocol') }}</label>
            <div class="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" data-testid="cn-api-protocol-options">
              <button
                ...
                @click="selectCNApiMode(option.value)"
              ...
              >
                <div :class="cnIconClass(cnApiMode === option.value, option.value === 'chat_completions' ? 'sky' : option.value === 'responses' ? 'emerald' : 'orange')"><Icon :name="option.value === 'anthropic' ? 'sparkles' : option.value === 'adaptive' ? 'swap' : 'terminal'" size="sm" /></div>
                <div class="min-w-0"><span class="block text-sm font-medium text-gray-900 dark:text-white">{{ option.label }}</span><span class="mt-0.5 block text-xs leading-5 text-gray-500 dark:text-gray-400">{{ option.description }}</span></div>
              </button>
            </div>
          </div>
        </div>
        <div v-if="account.platform === 'zhipu' && cnBillingMode === 'coding_plan'" class="rounded-lg border border-violet-200 bg-violet-50/60 p-3 dark:border-violet-800/50 dark:bg-violet-900/10">
        ...
        <div
          ...
          data-testid="anthropic-apikey-auth-scheme-field"
        >
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <select
              ...
              class="input w-full text-sm sm:w-52"
        ...
        <!-- Header Override Section (Anthropic/OpenAI API Key only) -->
        <div
          v-if="showHeaderOverrideConfig"
          class="border-t border-gray-200 pt-4 dark:border-dark-600"
        >
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <div v-if="poolModeEnabled && account.platform === 'openai'" class="mt-3 rounded-lg bg-gray-50 p-3 dark:bg-dark-700/60">
            <div class="flex items-center justify-between gap-4">
              <div>
                <label class="input-label mb-0">{{ t('admin.accounts.imagePoolMode') }}</label>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.accounts.imagePoolModeHint') }}
                </p>
              </div>
              <button
                type="button"
                @click="imagePoolModeEnabled = !imagePoolModeEnabled"
                :class="[
                  'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
                  imagePoolModeEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
                ]"
              >
                <span
                  :class="[
                    'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                    imagePoolModeEnabled ? 'translate-x-5' : 'translate-x-0'
                  ]"
                />
              </button>
            </div>
          ...
          <PoolModeRaceRetryConditions
            v-if="poolModeEnabled && account.platform === 'openai' && upstreamConcurrencyRaceEnabled"
          ...
          <PoolModeRetryConditions
            v-else-if="poolModeEnabled && account.platform === 'openai'"
      ...
      <div
        v-if="account?.platform === 'openai' && !isSparkShadowAccount && (account?.type === 'oauth' || account?.type === 'setup-token' || account?.type === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between gap-4">
          <div>
      ...
      >
        ...
        >
          ...
          <div class="mt-4 flex items-center justify-between gap-4">
            <div>
              <label class="input-label mb-0">{{ t('admin.accounts.openai.responsesArgumentsObjectCompat') }}</label>
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accounts.openai.responsesArgumentsObjectCompatDesc') }}
              </p>
            </div>
            <button
              type="button"
              @click="openAIResponsesArgumentsObjectCompatEnabled = !openAIResponsesArgumentsObjectCompatEnabled"
              :class="[
                'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
                openAIResponsesArgumentsObjectCompatEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
              ]"
            >
              <span
                :class="[
                  'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                  openAIResponsesArgumentsObjectCompatEnabled ? 'translate-x-5' : 'translate-x-0'
                ]"
              />
            </button>
          </div>
      ...
      <!-- OpenAI Codex 图片工具账号级覆盖 -->
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'setup-token' || account?.type === 'apikey') && !isSparkShadowAccount"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
      ...
      >
        ...
        >
          <div class="flex items-center justify-between gap-4">
            <div class="min-w-0 flex-1">
              <label class="input-label mb-0 flex flex-wrap items-center gap-2">
                <span>{{ t('admin.accounts.openai.oauthChatGPTFirstTokenTimeoutPlaceholder') }}</span>
                <span
              ...
              <p :class="openAIFirstTokenTimeoutDescClass(openaiOAuthChatGPTFirstTokenTimeoutPlaceholderEnabled)">
          ...
          <div
            v-if="openaiOAuthChatGPTFirstTokenTimeoutPlaceholderEnabled"
            class="mt-3 rounded-md border border-amber-200/80 bg-white/70 p-3 dark:border-amber-800/70 dark:bg-dark-900/40"
          >
            <div class="flex items-center justify-between gap-4">
              <div class="min-w-0 flex-1">
                <label class="input-label mb-0 flex flex-wrap items-center gap-2">
                  <span>{{ t('admin.accounts.openai.firstTokenTimeoutPlaceholderGuard') }}</span>
                  <span
                    v-if="openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled"
                    class="rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300"
                  >
                    {{ openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardMaxMs }} ms
                  </span>
                </label>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.accounts.openai.firstTokenTimeoutPlaceholderGuardDesc') }}
                </p>
              </div>
              <button
                type="button"
                @click="
                  openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled =
                    !openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled
                "
                :class="openAIFirstTokenTimeoutSwitchClass(openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled)"
              >
                <span
                  :class="[
                    'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                    openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled ? 'translate-x-5' : 'translate-x-0'
                  ]"
                />
              </button>
            </div>
        ...
        <div
          v-if="showOpenAIAPIKeyTextStreamToggles"
          :class="openAIFirstTokenTimeoutPanelClass(openaiAPIKeyFirstTokenTimeoutPlaceholderEnabled)"
        >
          <div class="flex items-center justify-between gap-4">
            <div class="min-w-0 flex-1">
              <label class="input-label mb-0 flex flex-wrap items-center gap-2">
                <span>{{ t('admin.accounts.openai.apiKeyFirstTokenTimeoutPlaceholder') }}</span>
                <span
              ...
              <p :class="openAIFirstTokenTimeoutDescClass(openaiAPIKeyFirstTokenTimeoutPlaceholderEnabled)">
            ...
            <div class="flex items-center justify-between gap-4">
              <div class="min-w-0 flex-1">
                <label class="input-label mb-0 flex flex-wrap items-center gap-2">
                  <span>{{ t('admin.accounts.openai.firstTokenTimeoutPlaceholderGuard') }}</span>
                  <span
                    v-if="openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled"
                    class="rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300"
                  >
                    {{ openaiAPIKeyFirstTokenTimeoutPlaceholderGuardMaxMs }} ms
                  </span>
                </label>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.accounts.openai.firstTokenTimeoutPlaceholderGuardDesc') }}
              ...
              <button
                type="button"
                @click="
                  openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled =
                    !openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled
                "
                :class="openAIFirstTokenTimeoutSwitchClass(openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled)"
              >
                <span
                  :class="[
                    'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                    openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled ? 'translate-x-5' : 'translate-x-0'
                  ]"
                />
              </button>
            </div>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.anthropic.kiroMode') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.anthropic.kiroModeDesc') }}
            </p>
          </div>
          <button
            type="button"
            @click="anthropicKiroEnabled = !anthropicKiroEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              anthropicKiroEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                anthropicKiroEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
const baseUrlHint = computed(() => {
  ...
  if (props.account.platform === 'grok') return '留空使用官方 xAI API'
...
const loadCacheBoostAndIsolationFromCredentials = (credentials: Record<string, unknown>) => {
...
const applyCacheBoostAndIsolationCredentials = (credentials: Record<string, unknown>) => {
  ...
  if (showUpstreamStrongIsolationToggle.value && upstreamStrongIsolationEnabled.value) {
    ...
    delete credentials[keys.isolation]
  }
}
...
watch(openaiOAuthFirstTokenTimeoutStageConfig, (config) => {
  ...
  if (!first) return
  if (typeof first.placeholder_ms === 'number') openaiOAuthChatGPTFirstTokenTimeoutPlaceholderMs.value = first.placeholder_ms
  if (typeof last?.guard_max_ms === 'number') openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardMaxMs.value = last.guard_max_ms
...
function normalizeOpenAIFirstTokenTimeoutPlaceholderMs(value: unknown): number {
  ...
  if (!Number.isFinite(ms) || ms <= 0) {
    return OPENAI_FIRST_TOKEN_TIMEOUT_PLACEHOLDER_DEFAULT_MS
  }
  return Math.min(
    OPENAI_FIRST_TOKEN_TIMEOUT_PLACEHOLDER_MAX_MS,
    Math.max(OPENAI_FIRST_TOKEN_TIMEOUT_PLACEHOLDER_MIN_MS, ms)
  )
}
...
function resetOpenAIFirstTokenTimeoutPlaceholderMs(kind: 'oauth' | 'apikey') {
  if (kind === 'oauth') {
    ...
    openaiAPIKeyFirstTokenTimeoutPlaceholderMs.value = OPENAI_FIRST_TOKEN_TIMEOUT_PLACEHOLDER_DEFAULT_MS
  }
}

function resetOpenAIFirstTokenTimeoutPlaceholderGuard(kind: 'oauth' | 'apikey') {
  if (kind === 'oauth') {
    openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled.value = true
    openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardMaxMs.value =
      OPENAI_FIRST_TOKEN_TIMEOUT_PLACEHOLDER_GUARD_DEFAULT_MAX_MS
  } else {
    openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled.value = true
    openaiAPIKeyFirstTokenTimeoutPlaceholderGuardMaxMs.value =
      OPENAI_FIRST_TOKEN_TIMEOUT_PLACEHOLDER_GUARD_DEFAULT_MAX_MS
  }
}

function toggleOpenAIFirstTokenTimeoutPlaceholder(kind: 'oauth' | 'apikey') {
  if (kind === 'oauth') {
...
function openAIFirstTokenTimeoutDescClass(enabled: boolean) {
  return [
    'mt-1 text-xs',
    enabled ? 'text-amber-700 dark:text-amber-300' : 'text-gray-500 dark:text-gray-400'
  ]
}
...
const cnBaseUrlPresets = computed(() => {
  ...
  if (props.account?.platform === 'kimi') return [
    { label: t('admin.accounts.cnProviders.kimiMoonshot'), url: 'https://api.moonshot.cn/v1' },
    { label: t('admin.accounts.cnProviders.kimiCoding'), url: 'https://api.kimi.com/coding/v1' }
  ]
  if (props.account?.platform === 'zhipu') return [{ label: cnApiMode.value === 'anthropic' ? t('admin.accounts.cnProviders.anthropic') : t('admin.accounts.cnProviders.zhipu'), url: cnApiMode.value === 'anthropic' ? 'https://open.bigmodel.cn/api/anthropic' : (cnBillingMode.value === 'coding_plan' ? 'https://open.bigmodel.cn/api/coding/paas/v4' : 'https://open.bigmodel.cn/api/paas/v4') }]
  if (props.account?.platform === 'deepseek') {
    return cnApiMode.value === 'responses'
      ? [{ label: t('admin.accounts.cnProviders.responses'), url: 'https://api.deepseek.com' }]
      : cnApiMode.value === 'anthropic'
...
const cnOptionClass = (accent: 'sky' | 'violet' | 'emerald' | 'orange' | 'gray') => {
  const selected: Record<string, string> = {
    sky: 'border-sky-500 bg-sky-50 dark:bg-sky-900/20', violet: 'border-violet-500 bg-violet-50 dark:bg-violet-900/20',
    emerald: 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20', orange: 'border-orange-500 bg-orange-50 dark:bg-orange-900/20',
...
const cnIconClass = (selected: boolean, accent: 'sky' | 'violet' | 'emerald' | 'orange') => {
  const colors: Record<string, string> = { sky: 'bg-sky-500', violet: 'bg-violet-500', emerald: 'bg-emerald-500', orange: 'bg-orange-500' }
...
const selectCNBillingMode = (mode: CNBillingMode) => {
  if (props.account?.platform === 'kimi') {
    ...
    if (mode === 'coding_plan' && editBaseUrl.value === defaultPaygUrl) editBaseUrl.value = codingUrl
    if (mode === 'payg' && editBaseUrl.value === codingUrl) editBaseUrl.value = defaultPaygUrl
    if (cnApiMode.value === 'adaptive') {
      const oldChat = mode === 'coding_plan' ? defaultPaygUrl : codingUrl
      const nextChat = mode === 'coding_plan' ? codingUrl : defaultPaygUrl
      if (adaptiveBaseUrls.value.chat_completions === oldChat) adaptiveBaseUrls.value.chat_completions = nextChat
      const oldAnthropic = mode === 'coding_plan' ? 'https://api.moonshot.cn/anthropic' : 'https://api.kimi.com/coding'
      const nextAnthropic = mode === 'coding_plan' ? 'https://api.kimi.com/coding' : 'https://api.moonshot.cn/anthropic'
      if (adaptiveBaseUrls.value.anthropic === oldAnthropic) adaptiveBaseUrls.value.anthropic = nextAnthropic
    }
  }
  if (props.account?.platform === 'zhipu' && cnApiMode.value === 'adaptive') {
    const paygURL = 'https://open.bigmodel.cn/api/paas/v4'
    const codingURL = 'https://open.bigmodel.cn/api/coding/paas/v4'
    const oldURL = mode === 'coding_plan' ? paygURL : codingURL
    const nextURL = mode === 'coding_plan' ? codingURL : paygURL
    if (adaptiveBaseUrls.value.chat_completions === oldURL) adaptiveBaseUrls.value.chat_completions = nextURL
  }
  if (props.account?.platform === 'zhipu' && cnApiMode.value !== 'adaptive' && cnApiMode.value !== 'anthropic') {
    const paygURL = 'https://open.bigmodel.cn/api/paas/v4'
    const codingURL = 'https://open.bigmodel.cn/api/coding/paas/v4'
    const oldURL = mode === 'coding_plan' ? paygURL : codingURL
    const nextURL = mode === 'coding_plan' ? codingURL : paygURL
    if (editBaseUrl.value.trim() === oldURL) editBaseUrl.value = nextURL
  }
  cnBillingMode.value = mode
}

const selectCNApiMode = (mode: CNApiMode) => {
  if (mode !== 'adaptive') {
    if (cnApiMode.value === 'adaptive') {
      const selectedURL = adaptiveBaseUrls.value[mode]?.trim()
      if (selectedURL) editBaseUrl.value = selectedURL
    } else {
      const current = editBaseUrl.value.trim()
      const known = cnAdaptiveProtocolOptions.value.map(option => option.url)
      const selected = cnAdaptiveProtocolOptions.value.find(option => option.value === mode)
      if (selected && known.includes(current)) editBaseUrl.value = selected.url
    }
  }
  cnApiMode.value = mode
}

const selectCNBaseUrlPreset = (url: string) => {
  editBaseUrl.value = url
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  editZhipuOrganization.value =
    newAccount.platform === 'zhipu' && typeof credentials?.zhipu_organization === 'string'
  ...
  if (configuredAdaptiveUrls && typeof configuredAdaptiveUrls === 'object') {
  ...
  if ((!configuredAdaptiveUrls || typeof configuredAdaptiveUrls.chat_completions !== 'string' || !configuredAdaptiveUrls.chat_completions.trim()) && typeof credentials?.base_url === 'string' && credentials.base_url.trim()) {
    adaptiveBaseUrls.value.chat_completions = credentials.base_url.trim()
  }
  ...
  openAIResponsesArgumentsObjectCompatEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    if (newAccount.type === 'apikey') {
      ...
      openaiAPIKeyPreambleFlushEnabled.value = extra?.openai_apikey_preamble_flush_enabled === true
      openaiAPIKeySSECommentPreflushEnabled.value = extra?.openai_apikey_sse_comment_preflush_enabled === true
    ...
    if (newAccount.type === 'oauth') {
      openaiOAuthChatGPTPreambleFlushEnabled.value = extra?.openai_oauth_chatgpt_preamble_flush_enabled === true
      openaiOAuthChatGPTSSECommentPreflushEnabled.value = extra?.openai_oauth_chatgpt_sse_comment_preflush_enabled === true
      openaiOAuthChatGPTSafeTokenPlaceholderEnabled.value =
      ...
      openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled.value =
        extra?.openai_oauth_chatgpt_first_token_timeout_placeholder_guard_enabled !== false
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    upstreamConcurrencyRaceMaxElapsedMs.value = normalizeUpstreamConcurrencyRaceMaxElapsedMs(
      Number(credentials.upstream_concurrency_race_max_elapsed_ms ?? DEFAULT_UPSTREAM_CONCURRENCY_RACE_MAX_ELAPSED_MS)
    )
    const hasIndependentRaceRetryCount = credentials.upstream_concurrency_race_retry_count !== undefined
    const legacyPoolRetryCount = Number(credentials.pool_mode_retry_count ?? DEFAULT_POOL_MODE_RETRY_COUNT)
    const legacyNormalRetryCountBackup = Number(credentials.upstream_concurrency_race_retry_count_backup)
    poolModeRetryCount.value = !hasIndependentRaceRetryCount && Number.isFinite(legacyNormalRetryCountBackup)
      ? normalizePoolModeRetryCount(legacyNormalRetryCountBackup)
      : hasIndependentRaceRetryCount
        ? normalizePoolModeRetryCount(legacyPoolRetryCount)
        : upstreamConcurrencyRaceEnabled.value
          ? DEFAULT_POOL_MODE_RETRY_COUNT
          : normalizePoolModeRetryCount(legacyPoolRetryCount)
    upstreamConcurrencyRaceRetryCount.value = normalizeUpstreamConcurrencyRaceRetryCount(
      Number(
        hasIndependentRaceRetryCount
          ? credentials.upstream_concurrency_race_retry_count
          : DEFAULT_UPSTREAM_CONCURRENCY_RACE_RETRY_COUNT
      )
    )
    if (newAccount.platform === 'openai') {
      ...
      const configuredRaceRules = credentials.upstream_concurrency_race_http_rules
      if (Array.isArray(configuredRaceRules)) {
        const seenRaceMatchers = new Set<string>()
        upstreamConcurrencyRaceRules.value = configuredRaceRules
          .map((rule): PoolModeRaceRetryRule | null => {
            if (!rule || typeof rule !== 'object') return null
            const item = rule as Record<string, unknown>
            const matcher = String(item.matcher ?? '').trim().toLowerCase()
            const maxRetries = Number(item.max_retries)
            if ((!/^\d{3}$/.test(matcher) && matcher !== '5xx') ||
              (matcher !== '5xx' && (Number(matcher) < 100 || Number(matcher) > 599)) ||
              !Number.isFinite(maxRetries) || seenRaceMatchers.has(matcher)) return null
            seenRaceMatchers.add(matcher)
            return { matcher, max_retries: Math.max(0, Math.min(200, Math.trunc(maxRetries))) }
          })
          .filter((rule): rule is PoolModeRaceRetryRule => rule !== null)
      } else {
        upstreamConcurrencyRaceRules.value = DEFAULT_UPSTREAM_CONCURRENCY_RACE_RULES.map(rule => ({ ...rule }))
      }
      upstreamConcurrencyRaceTransportEnabled.value = credentials.upstream_concurrency_race_transport_enabled === undefined
        ? true
        : credentials.upstream_concurrency_race_transport_enabled !== false
      upstreamConcurrencyRaceTransportRetryCount.value = Number.isFinite(Number(credentials.upstream_concurrency_race_transport_retry_count))
        ? Math.max(0, Math.min(200, Math.trunc(Number(credentials.upstream_concurrency_race_transport_retry_count))))
        : upstreamConcurrencyRaceTransportEnabled.value ? 1 : 0
    } else {
...
const fillHeaderOverrideTemplate = () => {
  ...
  const existing = new Set(headerOverrideRows.value.map((row) => row.name.trim().toLowerCase()).filter(Boolean))
  for (const row of getHeaderOverrideTemplate(platform)) {
    if (!existing.has(row.name)) {
      headerOverrideRows.value.push(row)
      existing.add(row.name)
    }
  }
}

const applyHeaderOverrideCredentials = (credentials: Record<string, unknown>): boolean => {
  if (!showHeaderOverrideConfig.value) {
    return true
  }
...
const handleSubmit = async () => {
  ...
  if (
    props.account.platform === 'openai' &&
    (props.account.type === 'apikey' || props.account.type === 'oauth') &&
    !imagePoolModeEnabled.value &&
    ((props.account.type === 'apikey' && openaiAPIKeyFirstTokenTimeoutPlaceholderEnabled.value && openaiAPIKeyFirstTokenTimeoutPlaceholderGuardEnabled.value) ||
      (props.account.type === 'oauth' && openaiOAuthChatGPTFirstTokenTimeoutPlaceholderEnabled.value && openaiOAuthChatGPTFirstTokenTimeoutPlaceholderGuardEnabled.value))
  ) {
    const stageConfig = props.account.type === 'apikey'
      ? openaiAPIKeyFirstTokenTimeoutStageConfig.value
      : openaiOAuthFirstTokenTimeoutStageConfig.value
    const stageError = validateOpenAIApiKeyFirstTokenTimeoutStageConfig(stageConfig, t)
    if (stageError) {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      if (isCNProvider.value && cnApiMode.value === 'adaptive') {
        newCredentials.base_url = (adaptiveBaseUrls.value.chat_completions || cnAdaptiveProtocolOptions.value[0]?.url || newBaseUrl).trim()
        newCredentials.api_base_urls = Object.fromEntries(cnAdaptiveProtocolOptions.value.map(option => [option.value, (adaptiveBaseUrls.value[option.value] || option.url).trim()]))
      } else if (isCNProvider.value) {
      ...
      } else if (props.account.platform === 'zhipu') {
        delete newCredentials.zhipu_organization
        delete newCredentials.zhipu_project
      }
      ...
      if (poolModeEnabled.value) {
        if (props.account.platform === 'openai' && !imagePoolModeEnabled.value && upstreamConcurrencyRaceEnabled.value) {
        ...
        if (props.account.platform === 'openai') {
          if (!imagePoolModeEnabled.value && upstreamConcurrencyRaceEnabled.value) {
            newCredentials.upstream_concurrency_race_enabled = true
            ...
            newCredentials.upstream_concurrency_race_retry_count = normalizeUpstreamConcurrencyRaceRetryCount(
              upstreamConcurrencyRaceRetryCount.value
            )
            newCredentials.upstream_concurrency_race_http_rules = upstreamConcurrencyRaceRules.value.map(rule => ({ ...rule }))
            newCredentials.upstream_concurrency_race_transport_enabled = upstreamConcurrencyRaceTransportEnabled.value
            newCredentials.upstream_concurrency_race_transport_retry_count = Math.max(0, Math.min(200, Math.trunc(upstreamConcurrencyRaceTransportRetryCount.value)))
          } else {
            delete newCredentials.upstream_concurrency_race_enabled
            delete newCredentials.upstream_concurrency_race_retry_count
            ...
            delete newCredentials.upstream_concurrency_race_http_rules
            delete newCredentials.upstream_concurrency_race_transport_enabled
            delete newCredentials.upstream_concurrency_race_transport_retry_count
          }
      ...
      if (!applyHeaderOverrideCredentials(newCredentials)) {
        return
      }
    ...
    // Preserve CN provider plan/protocol choices without changing the account
    // type or the established gateway dispatch path.
    if (isCNProvider.value && props.account.type === 'apikey') {
      const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {}
      const newExtra: Record<string, unknown> = { ...currentExtra }
      newExtra.cn_billing_mode = cnBillingPlanSupported.value && cnBillingMode.value === 'coding_plan' ? 'coding_plan' : 'payg'
      newExtra.cn_api_mode = cnApiProtocolOptions.value.some(option => option.value === cnApiMode.value)
        ? cnApiMode.value
        : 'chat_completions'
      if (cnApiMode.value === 'adaptive') {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (props.account.type === 'oauth') {
        ...
        if (openaiOAuthChatGPTPreambleFlushEnabled.value) {
          newExtra.openai_oauth_chatgpt_preamble_flush_enabled = true
        } else {
          delete newExtra.openai_oauth_chatgpt_preamble_flush_enabled
        }
        if (openaiOAuthChatGPTSSECommentPreflushEnabled.value) {
          newExtra.openai_oauth_chatgpt_sse_comment_preflush_enabled = true
        } else {
          delete newExtra.openai_oauth_chatgpt_sse_comment_preflush_enabled
        }
        if (openaiOAuthChatGPTSafeTokenPlaceholderEnabled.value) {
          newExtra.openai_oauth_chatgpt_safe_token_placeholder_enabled = true
        } else {
          delete newExtra.openai_oauth_chatgpt_safe_token_placeholder_enabled
        }
        if (openaiOAuthChatGPTFirstTokenTimeoutPlaceholderEnabled.value) {
          ...
          delete newExtra.openai_oauth_chatgpt_first_token_timeout_placeholder_guard_enabled
          delete newExtra.openai_oauth_chatgpt_first_token_timeout_placeholder_guard_max_ms
        ...
        delete newExtra.openai_apikey_preamble_flush_enabled
        delete newExtra.openai_apikey_sse_comment_preflush_enabled
        delete newExtra.openai_apikey_safe_token_placeholder_enabled
        ...
        if (!imagePoolModeEnabled.value && openaiAPIKeyPreambleFlushEnabled.value) {
          newExtra.openai_apikey_preamble_flush_enabled = true
        } else {
          delete newExtra.openai_apikey_preamble_flush_enabled
        }
        if (!imagePoolModeEnabled.value && openaiAPIKeySSECommentPreflushEnabled.value) {
          newExtra.openai_apikey_sse_comment_preflush_enabled = true
        } else {
          delete newExtra.openai_apikey_sse_comment_preflush_enabled
        }
        if (!imagePoolModeEnabled.value && openaiAPIKeySafeTokenPlaceholderEnabled.value) {
          newExtra.openai_apikey_safe_token_placeholder_enabled = true
        } else {
          delete newExtra.openai_apikey_safe_token_placeholder_enabled
        }
