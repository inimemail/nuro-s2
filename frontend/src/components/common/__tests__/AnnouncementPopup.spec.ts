...
describe('AnnouncementPopup', () => {
  ...
  it('renders sanitized Markdown and HTML inside the shared styled container', async () => {
  ...
  it('keeps the user popup dismissal behavior', async () => {
    const store = useAnnouncementStore()
  ...
  it('keeps body scrolling locked until overlapping popup owners are both closed', async () => {
    const store = useAnnouncementStore()
    store.currentPopup = announcement
    const userPopup = mount(AnnouncementPopup)
    const previewPopup = mount(AnnouncementPopup, {
      props: { announcement, preview: true },
    })

    expect(document.body.style.overflow).toBe('hidden')
    await previewPopup.setProps({ announcement: null })
    expect(document.body.style.overflow).toBe('hidden')

    store.currentPopup = null
    await userPopup.vm.$nextTick()
    expect(document.body.style.overflow).toBe('')

    previewPopup.unmount()
    userPopup.unmount()
  })
})
