...
const dropdownStyle = computed(() => {
  if (!triggerRect.value) return {}
	...
	const viewportWidth = Math.max(0, viewportRight - dropdownViewportPadding)
	const preferredMinWidth = Math.max(dropdownMinimumWidth, rect.width)
	const minWidth = Math.min(preferredMinWidth, viewportWidth)
	const maxLeft = Math.max(dropdownViewportPadding, viewportRight - minWidth)
	const left = Math.min(Math.max(dropdownViewportPadding, rect.left), maxLeft)
	const availableWidth = Math.max(minWidth, viewportRight - left)
