...
const PLATFORMS: PlatformQuotaPlatform[] = ['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go']
...
async function onSave() {
  if (!props.user) return
  // 非法输入不能经归一化悄悄变成 null（无限额）。空输入仍表示无限额。
  const invalid: string[] = []
  for (const row of quotas.value) {
    for (const win of ['daily', 'weekly', 'monthly'] as const) {
      const v: unknown = row[`${win}_limit_usd` as const]
