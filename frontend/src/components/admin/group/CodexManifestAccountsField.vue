    ...
    <div v-if="config.enabled">
      ...
      <div class="relative" ref="searchContainerRef">
        ...
        >
          ...
          <button
            ...
            :class="{ 'opacity-50': config.account_ids.includes(account.id) }"
      ...
      <div class="mt-3 flex items-start justify-between gap-3">
        ...
        <button
          ...
          :class="[
            'relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full transition-colors',
            config.fallback_to_scheduler ? 'bg-primary-500' : 'bg-gray-300 dark:bg-dark-600',
          ]"
          :aria-label="t('admin.groups.codexModelsManifest.fallback')"
          @click="emitUpdate({ fallback_to_scheduler: !config.fallback_to_scheduler })"
        >
          <span
            :class="[
              'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
              config.fallback_to_scheduler ? 'translate-x-6' : 'translate-x-1',
            ]"
          />
        </button>
      </div>
...
const searchRunner = useKeyedDebouncedSearch<SimpleAccount[]>({
  ...
  search: async (keyword, { signal }) => {
    const res = await adminAPI.accounts.list(
      1,
      20,
      { search: keyword, platform: "openai", group: String(props.groupId) },
      { signal },
    );
...
const selectAccount = (account: SimpleAccount) => {
  ...
  emitUpdate({ account_ids: [...props.modelValue.account_ids, account.id] });
  searchKeyword.value = "";
  showDropdown.value = false;
  showValidationError.value = false;
};
const removeAccount = (id: number) =>
  emitUpdate({ account_ids: props.modelValue.account_ids.filter((item) => item !== id) });
const validate = (): boolean => {
  const valid = !props.modelValue.enabled || props.modelValue.account_ids.length > 0;
