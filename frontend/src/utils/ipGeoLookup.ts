...
function isFreshSuccess(
  entry: IpGeoEntry | undefined
): entry is IpGeoEntry & { status: 'success'; fetchedAt: number } {
...
export function isPrivateIp(ip: string): boolean {
  ...
  if (firstSegment === 'ff00' || firstSegment.startsWith('ff')) return true
...
function loadFromStorage(): void {
  try {
    ...
    // Ignore broken or inaccessible browser storage.
  }
}
...
function persistToStorage(): void {
  try {
    ...
    storage.setItem(CACHE_STORAGE_KEY, JSON.stringify(toStore))
  } catch {
    // Browser storage failure should not block usage log rendering.
  }
}
