...
export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      promptCacheCreationOptimizationReduceHint: '方案 A（默认）：使用 explicit + 30m，清理旧断点，仅为足够长的连续稳定 system/developer 前缀设置一个断点，在尽量减少重复创建的同时保留稳定前缀复用。',
      promptCacheCreationOptimizationSuppress: '尽量禁止缓存创建',
      promptCacheCreationOptimizationSuppressHint: '方案 B：仅使用 explicit，删除 TTL、旧 retention 和全部缓存断点且不新增断点，最大力度抑制 GPT-5.6 缓存创建。上游不支持 explicit 并触发兼容回退时，仍可能恢复隐式缓存创建。',
      promptCacheCreationOptimizationFree: '下游创建免费',
      promptCacheCreationOptimizationFreeHint: '方案 C：请求侧沿用方案 B；本节点保留真实缓存创建用于成本审计，但对下游将创建量归零并从总输入扣除，使同源码下游不显示、也不计费缓存创建。仅当前账号生效。',
      promptCacheCreationOptimizationInput125: '创建成本分摊',
      promptCacheCreationOptimizationInput125Hint: '方案 D：请求侧沿用方案 B；只要存在缓存创建量，下游创建量就归零，并按 GPT-5.6 价格比例分配到普通输入、少量输出和缓存读取，保持原费用口径。仅当前账号生效。',
