...
export default {
  openCodeUsage: {
    "title": "OpenCode Go 用量窗口",
    "description": "按真实请求活动刷新订阅用量。全局与账号开关均开启后才自动查询；空闲账号不持续轮询。",
    "refresh": "立即刷新",
    "preferences": "刷新设置",
    "auto": "自动",
    "accountAuto": "此账号自动刷新",
    "source": "查询来源",
    "configured": "使用账号已配置地址",
    "official": "官方 Go 订阅窗口",
    "officialHint": "使用此账号 API Key 查询 opencode.ai 的官方 Go 订阅窗口，仍沿用账号代理。",
    "configuredHint": "沿用账号的查询地址和代理，不改变模型转发配置。",
    "globalOff": "全局自动刷新已关闭，手动刷新仍可用。",
    "fiveHour": "5 小时",
    "weekly": "每周",
    "monthly": "每月",
    "updated": "更新于 {time}",
    "resets": "重置时间：{time}",
    "unknownReset": "上游未提供重置时间",
    "notFetched": "尚未查询 · 点击右上角刷新",
    "failed": "无法读取用量，请稍后重试",
    "interval": "活动持续时的最长等待（分钟）",
    "debounce": "活动停止后的防抖时间（分钟）",
    "saved": "设置已保存",
    "manualAvailable": "默认关闭；关闭后仍可在账号列表手动刷新。",
    "invalid": "最长等待须为 5–1440 的整数，防抖须为 1–60 的整数。"
},

  requestRetention: {
    invalid: '请输入 0–3650 的整数保留天数',
    "systemDays": "系统日志保留天数",
    "title": "自定义请求明细保留策略",
    "description": "关闭时沿用启动配置。开启后，0 表示永久保留；不会删除账务去重归档。保存前会展示预计影响，实际清理在后台分批执行。",
    "days": "请求明细保留天数（0–3650）",
    "confirm": "将自动清理 {cutoff} 之前的请求明细。根据数据库统计，预计涉及约 {rows} 条；该估算可能存在偏差。确认保存？",
    "unknown": "未知"
},
  claudeVersion: {
    "title": "Claude CLI 版本",
    "description": "开启后每小时检查官方稳定版本。默认关闭，仅在后台同步。",
    "autoSync": "自动同步 Claude CLI 版本",
    "manual": "手动固定版本（留空使用自动或内置值）",
    "effective": "当前生效版本",
    "precedence": "手动版本优先。关闭自动同步后，历史同步版本不再参与选择；现有客户端指纹优先级保持不变。",
    "lastSynced": "最近同步："
},
  backupArchive: {
    "title": "月度归档",
    "description": "从已完成的定时备份中保留归档，独立于普通备份保留策略。默认关闭。",
    "days": "每月归档日期",
    "monthEnd": "同时保留月末归档",
    "dateHint": "短月份会自动取月末。归档日期是保留标签，数据时间以实际备份时间为准；不会补造上个月的备份。",
    "retainCount": "归档保留份数",
    "retentionHint": "0 表示永久保留。关闭功能不会移除已有归档保护。",
    "badge": "月归档",
    "restoring": "正在恢复"
},
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      openai: {
        ...
        configTomlHint: '请确保以下内容位于 config.toml 文件的开头部分。修改 API Key 配置后需完全重启 Codex，并新建任务。',
      ...
      grok: {
        description: '将以下配置文件添加到 Grok CLI 配置目录中。',
        configTomlHint: '请将以下内容保存为 config.toml。',
        note: '请确保配置目录存在。macOS/Linux 用户可运行 mkdir -p ~/.grok 创建目录。',
        noteWindows: '按 Win+R，输入 %userprofile%\\.grok 打开配置目录。如目录不存在，请先手动创建。'
      },
      antigravity: {
  ...
  usage: {
    ...
    totalTokens: '总 Token',
    cacheTotal: '缓存',
    cacheBreakdown: '缓存创建：{creation} / 缓存读取：{read}',
    ...
    ipGeo: {
      ...
      failed: '查询失败，重试',
      private: '内网 IP',
      pending: '{count} 个 IP 待查询',
      batchFetch: '查询本页 IP',
      batchFetching: '查询中...',
      batchFailed: '部分 IP 归属地查询失败',
      refreshTitle: '刷新 IP 归属地',
      detailOrg: '组织',
      detailTimezone: '时区',
      detailAccuracy: '精度',
  ...
  availableChannels: {
    ...
    pricing: {
      ...
      cacheWrite1hPrice: '1h 缓存写入',
      cacheWrite1hPricePlaceholder: '沿用缓存写入价',
      cacheReadPrice: '缓存读取',
  ...
  admin: {
    ...
    users: {
      ...
      bulkLimits: {
        ...
        enableRPMLimit: '修改 RPM',
        unlimited: '0 表示不限制',
        nonNegativeInteger: '请输入大于等于 0 的整数',
        selectionLimit: '单次最多选择 {max} 个用户',
        confirm: '确定修改所选 {count} 个用户的限制吗？',
        confirmAll: '确定修改全部用户的限制吗？此操作会影响用户后续请求读取的并发与 RPM 配置。',
        apply: '应用',
        applying: '应用中...',
      ...
      restrictPublicGroupsHint: '开启后，该用户只能使用下方勾选的公开分组；关闭后可使用全部公开分组。',
    ...
    groups: {
      ...
      form: {
        ...
        upstreamBillingGuardRange: '上游倍率保护范围',
        upstreamBillingGuardMinLabel: '最低倍率',
        upstreamBillingGuardMinPlaceholder: '最低倍率，留空不限',
        upstreamBillingGuardMaxPlaceholder: '最高倍率，留空不限',
        upstreamBillingGuardMaxLabel: '最高倍率',
        upstreamBillingGuardUnlimited: '留空表示不保护',
        upstreamBillingGuardHint: '支持八个平台的 API Key 账号；低于下限或高于上限时只暂停当前分组，恢复到区间后自动恢复。',
        upstreamBillingGuardInvalid: '保护上下限必须大于或等于 0；同时填写时，下限必须小于上限',
        rpmLimit: '每分钟请求数 (RPM)',
        rpmLimitPlaceholder: '0 表示不限制',
        rpmLimitHint: '每用户在本分组每分钟最大请求数，0 = 不限制；一旦设置即接管该用户的限流（覆盖用户级 rpm_limit）',
        maxReasoningEffort: '最大思考等级',
        maxReasoningEffortUnlimited: '不限制',
        maxReasoningEffortHint: '仅对 OpenAI 分组生效，超过该等级的请求会被限制。',
        reasoningEffortOverLimit: '超限处理',
        reasoningEffortDowngrade: '自动降级',
        reasoningEffortDeny: '拒绝请求',
        reasoningEffortPolicy: '思考等级策略',
        reasoningEffortMappings: '等级映射',
        reasoningEffortMappingsEmpty: '暂无等级映射',
        reasoningEffortFrom: '原等级',
        reasoningEffortTo: '目标等级',
        reasoningEffortScope: '模型范围',
        reasoningEffortAllModels: '全部模型',
        reasoningEffortExact: '精确匹配',
        reasoningEffortPrefix: '前缀匹配',
        reasoningEffortSuffix: '后缀匹配',
        reasoningEffortModel: '模型名或模式',
        reasoningEffortModelPlaceholder: '例如 gpt-5.4',
        addReasoningEffortMapping: '添加思考等级映射',
        removeReasoningEffortMapping: '删除思考等级映射',
        reasoningEffortMappingInvalid: '思考等级映射必须使用支持的值；同一模型范围内的来源等级不能重复。',
        accountSchedulingStrategy: '账号调度策略',
        accountSchedulingStrategyHint: '仅影响当前分组；倍率保护、限流和现有冷却始终是硬限制。',
        strategyScope: '仅当前分组',
        strictPriority: '严格优先级',
        strictPriorityHint: '保持现有调度行为，优先使用账号优先级较高的层。',
        healthFirst: '自适应健康',
        healthLeading: '健康优先',
        healthLeadingShortHint: '优先选择健康账号',
        healthLeadingHint: '优先选择错误少的健康账号；首 Token 未达到慢速阈值时，倍率作为次级条件，续链请求保留原账号。',
        healthCostBalanced: '健康成本均衡',
        healthCostBalancedShortHint: '健康与成本综合',
        healthCostBalancedHint: '先排除异常账号，再锁定最低倍率层；低价层严重变慢时升级，续链请求保留原账号。',
        strictPriorityShortHint: '按账号原有优先级',
        adaptiveTTFTSwitch: '首 Token 慢速切换',
      ...
      peakRate: {
        ...
        peakEnd: '高峰结束',
        peakMultiplier: '高峰倍率',
        multiplierHint: '作用于 token 计费倍率；0 表示高峰 token 请求按 0 倍计费'
      },
      imagePricing: {
        title: '媒体生成计费',
        description: '配置图片/视频生成能力和基础单价，留空则使用默认价格',
        allowImageGeneration: '允许当前分组生图',
        batchImageTitle: '批量生图',
        batchImageDescription: '仅 Gemini 分组生效。开启后，当前分组的 API Key 可使用批量生图任务；折扣倍率用于最终结算，冻结倍率用于提交任务时预冻结余额。',
        allowBatchImageGeneration: '允许当前分组批量生图',
        batchImageDiscountMultiplier: '批量生图折扣倍率',
        batchImageHoldMultiplier: '批量生图冻结倍率',
        independentMultiplier: '生图倍率独立',
        imageMultiplier: '生图独立倍率',
        videoIndependentMultiplier: '视频倍率独立',
        videoMultiplier: '视频独立倍率',
        modeHint: '默认关闭独立倍率时，媒体费用使用当前分组有效倍率；开启独立倍率后，图片/视频分别使用自己的独立倍率。视频价格按秒计费。',
        finalPricePreview: '最终价格预览',
        notConfigured: '未配置'
      },
      webSearchPricing: {
      ...
      openaiLive: {
        title: 'OpenAI Live 实时会话',
        hint: '允许此分组使用 Live 实时会话端点。仅 OpenAI OAuth 文本账号参与调度；不改变普通请求、缓存策略或账号优先级。',
        capabilityChecking: '正在检查当前节点能力',
        capabilityReady: '当前节点已具备安全证明能力',
        capabilityUnavailable: '当前节点不具备运行能力',
        unsupportedTitle: '当前节点不支持 OpenAI Live',
        unsupportedMessage: '当前运行环境无法生成 Live 所需的安全证明。仍可保存开关用于其他兼容节点，但此节点上的 Live 请求会安全拒绝。',
        enableAnyway: '仍然启用'
      },
    ...
    channels: {
      ...
      form: {
        ...
        noModels: '未添加模型',
        timePricing: '分时倍率（可选）',
        timePricingEnable: '启用',
        timePricingDisable: '关闭',
        timePricingTimezonePlaceholder: 'IANA 时区，例如 Asia/Shanghai',
        timePricingStartPlaceholder: '开始时间',
        timePricingEndPlaceholder: '结束时间',
        timePricingMultiplierPlaceholder: '倍率',
        timePricingAddPeriod: '添加时段',
    ...
    channelMonitor: {
      ...
      linkedAccountRequired: '请选择配额数据源账号',
      columns: {
        name: '名称',
        provider: '供应商',
        primaryModel: '主模型',
      ...
      form: {
        ...
        provider: '平台',
        checkMode: '检测模式',
        checkModeProbe: '可用性探测',
        checkModeProbeHint: '发送轻量模型请求，检测可用性和响应延迟。',
        checkModeQuota: '仅配额',
        checkModeQuotaHint: '只读取账号额度或余额，不消耗模型 Token。',
        checkModeQuotaProbe: '探活 + 配额',
        checkModeQuotaProbeHint: '保留可用性检测，并在结果中附加账号配额快照。',
        linkedAccount: '配额数据源账号',
        linkedAccountPlaceholder: '搜索当前平台账号',
        linkedAccountHint: '配额检测独立运行，不参与请求调度和转发链路。',
        linkedAccountMissing: '原关联账号无法加载，请重新选择。',
        linkedAccountEmpty: '当前平台没有可用账号。',
        apiMode: 'OpenAI 协议',
    ...
    subscriptions: {
      ...
      restoreConfirm: "确定要恢复 '{user}' 的已撤销订阅吗？如果该用户同分组已有有效订阅，恢复会失败。",
      guide: {
        title: '订阅管理教程',
    ...
    accounts: {
      ...
      revertProxyFallback: '恢复原代理',
      revertProxyFallbackSuccess: '已恢复原代理',
      revertProxyFallbackFailed: '恢复原代理失败',
      ...
      upstreamBilling: {
        ...
        intervalSeconds: '探测周期（秒）',
        intervalMinutes: '探测周期（分钟）',
        autoProbe: '自动探测',
        autoProbeHint: '启用后按全局周期探测此账号；全局探测关闭时不会执行。',
        syncRate: '自动同步账号倍率',
        syncRateHint: '探测成功后将折算后的上游基准倍率写回账号倍率；开启后账号倍率由系统托管。',
        syncRateManagedHint: '账号倍率由上游探测自动同步，关闭自动同步后才可手工修改。',
        adaptiveSchedulingFactor: '上游倍率折算',
        adaptiveSchedulingFactorHint: '折算后的上游倍率统一用于健康调度、分组倍率保护、后台显示和自动同步；原始探测值仍保留用于审计。',
        adaptiveSchedulingObserved: '上游原始探测：{rate}',
        adaptiveSchedulingEffective: '折算后上游倍率：{rate}',
        adaptiveSchedulingFactorInvalid: '上游倍率折算系数必须在 0.001 到 100 之间。',
        manualMultiplier: '手动上游倍率',
        manual: '手动',
        manualMultiplierHint: '当前使用管理员手动倍率；该值已是最终调度倍率，不再乘折算系数。',
        manualMultiplierUnsupportedHint: '上游不支持自动倍率探测时，可填写最终调度倍率；留空则保持未知。',
        manualMultiplierPlaceholder: '留空表示未知',
        ...
        failedWithLast: '探测失败 · 保留上次倍率',
        guard: '倍率保护',
        guardHint: '探测倍率超出保护范围时停止新调度；恢复后自动解除。',
        groupGuardHint: '总开关开启后，自动应用当前绑定分组的保护范围；分组未配置则不限制。',
        advancedOverrides: '高级：账号-分组自定义保护范围',
        inheritDefault: '分组默认范围：{rate}',
        inheritPlaceholder: '上限：继承',
        overrideLabel: '{group} 的保护上限覆盖',
        overrideHint: '留空表示继承；下限只能提高，上限只能降低，分组未配置的一侧不可覆盖。',
        guardMaxMultiplier: '保护倍率上限',
        guardObserved: '当前有效倍率 {rate}x',
        guardRequiresAutoProbe: '开启倍率保护前必须先启用账号自动探测',
        blankMeansUnlimited: '留空表示不限制',
        unlimited: '不限制',
        selectGroupFirst: '当前未选择分组',
        noConfiguredGroup: '所选分组均未配置保护倍率，无法开启',
        groupNotConfigured: '分组未配置保护倍率',
        invalidGuardLimit: '保护倍率必须是大于或等于 0 的数字',
        currentRateDetail: '当前上游倍率：{rate}',
        guardLimitDetail: '保护范围：{rate}',
        guardBelowMin: '低于保护下限，当前分组暂停调度',
        guardProbeDisabled: '自动探测已关闭，当前分组暂停调度',
        guardGlobalProbeDisabled: '全局探测已关闭，当前显示上次探测倍率',
        guardDisabled: '账号保护已关闭',
        guardWaitingFirstProbe: '等待首次成功探测，当前暂可调度',
        guardPaused: '已超过保护上限，当前分组暂停调度',
        guardAvailable: '处于保护范围内，当前分组可调度',
        minimumOnlyRange: '最低 {rate}，最高不限',
        minInheritPlaceholder: '下限：继承',
        minUnavailablePlaceholder: '未配置下限',
        maxUnavailablePlaceholder: '未配置上限',
        minOverrideLabel: '{group} 的保护下限覆盖',
        overrideRangeInvalid: '生效后的保护下限必须小于保护上限',
      ...
      status: {
        ...
        billingGuardBlocked: '倍率保护停调',
        quotaExceeded: '配额超限',
        poolSoftCooldown: '池冷却',
        poolRecoveryProbing: '探测中',
        poolRecoveryPending: '待探测',
        unschedulable: '不可调度',
        rateLimitedUntil: '限流中，当前不参与调度，预计 {time} 自动恢复',
        rateLimitedAutoResume: '{time} 自动恢复',
        poolSoftCooldownAutoProbe: '{time} 后探测',
        poolRecoveryProbingShort: '恢复探测中',
        poolRecoveryPendingShort: '等待恢复探测',
        poolSoftCooldownReason: '池模式软冷却中，探测成功后自动恢复',
        poolSoftCooldownCode: '池模式软冷却中，状态码 {code}',
        poolSoftCooldownReasonWithCode: '池模式软冷却中，状态码 {code}：{reason}',
      ...
      bulkEdit: {
        ...
        shadowWarning: '所选目标包含 Spark 影子账号。影子账号只会应用安全的调度字段和模型映射；代理、凭据和上游功能变更会对影子账号忽略。'
      },
      ...
      cnProviders: {
        ...
        codingPlanDesc: '订阅制编程套餐，按套餐用量窗口限流。',
        apiProtocol: 'API 协议',
        adaptive: '自适应',
        adaptiveDesc: '按下游请求协议自动选择对应端点。',
        adaptiveEndpoints: '自适应协议端点',
        adaptiveEndpointsDesc: '根据下游请求协议选择对应端点，未填写时使用供应商默认地址。',
        chatCompletions: 'Chat Completions',
        chatCompletionsDesc: '标准 OpenAI 兼容端点。',
        anthropic: 'Anthropic',
        anthropicDesc: '供应商原生 Anthropic 端点。',
        responses: 'Responses',
        responsesDesc: 'OpenAI Responses 兼容端点。',
        baseUrlHint: '可使用官方地址或下方预设，也可以填写兼容网关地址。',
        ...
        deepseek: 'DeepSeek',
        zhipuTeam: {
          title: '团队版组织 / 项目 ID', organization: '组织 ID（团队版可选）', organizationPlaceholder: '团队版 Coding Plan 的组织 ID',
          project: '项目 ID（团队版可选）', projectPlaceholder: '团队版 Coding Plan 的项目 ID',
          hint: '仅团队版 GLM Coding Plan 需要填写，填写后用量查询走团队版端点；个人版留空即可。',
          help: { title: '如何获取组织 / 项目 ID', step1: '登录 bigmodel.cn 团队账号并进入 Coding Plan 团队页面。', step2: '按 F12 打开开发者工具并切换到 Network。', step3: '筛选 /api/biz/v1/organization 并打开匹配请求。', step4: 'URL 中 org- 段为组织 ID，proj_ 段为项目 ID。', example: '示例：.../organization/org-xxx/projects/proj_xxx/api_keys' }
        }
      },
      ...
      openai: {
        ...
        responsesArgumentsObjectCompatDesc:
          '仅对当前 OpenAI OAuth/APIKey 账号生效。开启后只会把 tool_search_call.arguments 中可解析为 JSON 对象的字符串转为对象，用于兼容部分非标准 Responses 上游；默认关闭。',
        ...
        firstTokenTimeoutStages: {
          ...
          remove: '删除阶段',
          placeholderRange: '补帧阈值必须在 1-100000 ms。',
          guardRange: '保护上限必须是大于 0 的整数，不设最大值。',
          guardBelowPlaceholder: '保护上限不能小于补帧阈值。',
          placeholderOrder: '必须大于阶段 {stage} 的补帧阈值。',
          guardOrder: '必须大于阶段 {stage} 的保护上限。',
          conflict: '阶段 {stage} 起存在顺序冲突，保存前必须处理。',
          repair: '修正后续阶段',
          removeInvalid: '删除无效后续',
          countInvalid: '阶段数量必须在 1-10 之间。',
          stagePlaceholderInvalid: '阶段 {stage} 的补帧阈值必须在 1-100000 ms。',
          stageGuardInvalid: '阶段 {stage} 的保护上限必须是大于 0 的整数。',
          stageGuardBelow: '阶段 {stage} 的保护上限不能小于补帧阈值。',
          stageOrderInvalid: '阶段 {stage} 的两个数值都必须大于阶段 {previous}。'
        },
        ...
        seedanceDescription: '原生异步视频生成 · 独立任务与计费',
        seedanceRequirements: '使用 API Key 和自定义上游地址；请配置模型映射以及明确的 Token / 按次价格。真实生成会产生费用，普通账号探测不会创建视频。',
        seedanceUnchanged: 'Seedance：保持不变',
        seedanceEnabled: 'Seedance：启用',
        seedanceDisabled: 'Seedance：禁用',
        seedanceTestTitle: 'Seedance 对接测试',
        seedanceTestHint: '通过真实下游入口验证创建、查询与计费。分组会按正常规则选择账号，不保证选中当前账号。Key 仅在本弹窗内使用，不会保存。',
        seedanceDownstreamKey: '你的下游 API Key（OpenAI 分组）',
        seedanceCostConsent: '我确认创建真实视频任务会产生上游费用，并按当前分组价格计费。',
        seedanceCreateTask: '创建视频任务',
        seedanceRecoverSubmission: '核对同一提交（不会重复创建）',
        seedanceCancelTask: '申请取消任务',
        seedanceNewTask: '准备另一个任务',
        seedanceTaskPersistence: '请保存任务 ID。关闭弹窗不会取消已提交的任务，后台会继续查询与结算。',
        seedanceCancellationPending: '已提交取消请求；仍会核对上游最终状态和实际用量，取消不代表免费。',
        ...
        codexImageGenerationBridgeBulkDesc: '批量编辑时可强制开启、强制关闭、完全阻断，或清除账号覆盖恢复为渠道策略。',
      ...
      promptCacheBoost: '缓存增强',
      promptCacheBoostHint: 'OpenAI OAuth 或 API Key 文本池生效。开启后会自动生成虚拟会话缓存键、使用 24h prompt cache，并在大上下文请求中保留可缓存前缀。',
      ...
      promptCacheAdvancedFeatures: '缓存高级增强',
      promptCacheSmartRouting: '缓存智能调度',
      promptCacheSmartRoutingHint: '根据真实缓存命中记录，在同级健康账号中优选热账号。',
      promptCacheAccountRelay: '缓存账号接力',
      promptCacheAccountRelayHint: '当前热账号不可用时，依次尝试其他命中过缓存的同级账号。',
      promptCacheKeyOptimization: '缓存键优化',
      promptCacheKeyOptimizationHint: '忽略非提示控制参数，减少相同静态前缀产生的缓存碎片。',
      promptCacheLongContextEnhancement: '长上下文增强',
      promptCacheLongContextEnhancementHint: '仅对高命中请求提前保留完整历史，不增加现有裁剪。',
      promptCacheCreationOptimization: '上游缓存创建优化',
      promptCacheCreationOptimizationHint: 'OpenAI OAuth 和 API Key 文本账号生效。关闭时请求行为与当前版本完全一致；开启后尽量控制 GPT-5.6 缓存创建。',
      promptCacheCreationOptimizationReduce: '减少缓存创建',
      promptCacheCreationOptimizationReduceHint: '方案 A（默认）：使用 explicit + 24h，清理旧断点，仅为足够长的连续稳定 system/developer 前缀设置一个断点，在尽量减少重复创建的同时保留稳定前缀复用。',
      promptCacheCreationOptimizationSuppress: '尽量禁止缓存创建',
      promptCacheCreationOptimizationSuppressHint: '方案 B：使用 explicit + 24h，清理旧 retention 和全部缓存断点且不新增断点，最大力度减少重复缓存创建。上游若忽略这些字段，或不支持 explicit 并触发兼容回退，仍可能产生缓存创建。',
      promptCacheCreationOptimizationFree: '下游创建免费',
      promptCacheCreationOptimizationFreeHint: '方案 C：请求侧沿用方案 B 的 explicit + 24h；本节点保留真实缓存创建用于成本审计，但对下游将创建量归零并从总输入扣除，使同源码下游不显示、也不计费缓存创建。仅当前账号生效。',
      promptCacheCreationOptimizationInput125: '创建成本分摊',
      promptCacheCreationOptimizationInput125Hint: '方案 D：请求侧沿用方案 B 的 explicit + 24h；只要存在缓存创建量，下游创建量就归零，并按 GPT-5.6 价格比例分配到普通输入、少量输出和缓存读取，保持原费用口径。仅当前账号生效。',
      downstreamCacheMarkup: '下游缓存高用量加成',
      downstreamCacheMarkupHint: '缓存读取达到阈值后增加下游 usage；本地真实用量、成本和账号统计保持不变。',
      downstreamCacheMarkupThreshold: '缓存读取触发阈值',
      downstreamCacheMarkupPercent: '加成比例',
      downstreamCacheMarkupAllocation: '加成金额分配',
      downstreamCacheMarkupInput: '普通输入',
      downstreamCacheMarkupRead: '缓存读取',
      downstreamCacheMarkupOutput: '输出',
      downstreamCacheMarkupZeroHint: '当前比例为 0%，不会产生下游加成。',
      ollamaUsageTitle: 'Ollama Cloud 用量',
      ollamaUsageHint: '使用加密保存的网页会话，在后台按模型请求活动刷新官方用量。不会参与网关路由。',
      ollamaSession: 'Ollama 网页会话',
      ollamaSessionPlaceholder: 'Cookie：name=value',
      ollamaSaveSession: '保存会话',
      ollamaRemoveSession: '移除',
      ollamaRefreshNow: '立即刷新',
      ollamaAccountAutoRefresh: '此账号自动刷新',
      ollamaBackgroundRefresh: '后台刷新任务',
      ollamaBackgroundRefreshHint: '只扫描数据库中已到期的账号，每轮最多处理 20 个共享密钥组。',
      ollamaMaxWaitMinutes: '持续请求时最长等待（分钟）',
      ollamaDebounceMinutes: '末次请求后静默时间（分钟）',
      ollamaFiveHour: '5 小时用量',
      ollamaSevenDay: '7 天用量',
      ollamaFiveHourShort: '5h',
      ollamaSevenDayShort: '7d',
      ollamaLastRefresh: '上次刷新',
      ollamaNextRefresh: '下次可刷新',
      ollamaNotConfigured: '未配置',
      ollamaConfigured: '已配置',
      ollamaStatusReady: '正常',
      ollamaStatusUnauthorized: '会话失效',
      ollamaStatusFailed: '刷新失败',
      ollamaEncryptionRequired: '必须配置固定的 TOTP_ENCRYPTION_KEY 才能安全保存会话。',
      ollamaRequestFailed: 'Ollama Cloud 请求失败',
      ...
      anthropicCacheBoostEnabledHint: '该 Anthropic 账号会自动优化 cache_control 断点。',
      anthropicCacheBoostDisabledHint: '该 Anthropic 账号未启用缓存增强。',
      promptCacheBoostEnabled: '已开启',
      promptCacheBoostEnabledHint: '该 OpenAI 账号会自动增强 prompt cache 命中。',
      promptCacheBoostDisabled: '未开启',
      promptCacheBoostDisabledHint: '该 OpenAI 账号未启用缓存增强。',
      promptCacheBoostNotApplicable: '不适用',
      promptCacheBoostNotApplicableHint: '仅 OpenAI OAuth、Anthropic OAuth/Setup Token 或 API Key 文本池可用。',
      upstreamStrongIsolation: '上游强隔离',
      upstreamStrongIsolationHint: '仅问题上游使用。开启后不向上游发送会话续写字段，并强制无状态请求；静态缓存增强仍会保留。',
      anthropicUpstreamStrongIsolation: 'Anthropic 上游强隔离',
      anthropicUpstreamStrongIsolationHint: '隔离 Anthropic 上游 session/conversation 等会话标识；不会移除 cache_control，不影响 Anthropic 缓存增强。',
      anthropicUpstreamStrongIsolationEnabledHint: '该 Anthropic 账号不会向上游发送 session/conversation 会话标识。',
      anthropicUpstreamStrongIsolationDisabledHint: '该 Anthropic 账号按普通模式转发上游会话标识。',
      upstreamStrongIsolationEnabled: '强隔离',
      upstreamStrongIsolationEnabledHint: '该账号不会向上游发送 session/conversation/previous_response 续写信号。',
      upstreamStrongIsolationDisabled: '未隔离',
      upstreamStrongIsolationDisabledHint: '该账号按普通模式转发上游会话信号。',
      upstreamStrongIsolationNotApplicable: '不适用',
      upstreamStrongIsolationNotApplicableHint: '仅 OpenAI OAuth、Anthropic OAuth/Setup Token 或 API Key 文本池可用。',
      upstreamConcurrencyRace: '抢上游并发',
      ...
      upstreamConcurrencyRaceRetryCount: '抢上游总重试次数',
      upstreamConcurrencyRaceRules: '同账号重试条件',
      upstreamConcurrencyRaceRulesHint: '仅抢上游并发开启时生效。精确状态码优先于 5xx；所有 HTTP 规则受抢上游总重试次数限制。',
      upstreamConcurrencyRaceRuleMatcher: 'HTTP 状态规则',
      upstreamConcurrencyRaceRuleLimit: '重试上限',
      upstreamConcurrencyRaceRule5xx: '全部服务器错误',
      upstreamConcurrencyRaceRulePlaceholder: '输入状态码或 5xx',
      upstreamConcurrencyRaceAddRule: '添加规则',
      upstreamConcurrencyRaceRemoveRule: '删除规则 {matcher}',
      upstreamConcurrencyRaceNoRules: '未添加状态规则',
      upstreamConcurrencyRaceRuleInvalid: '请输入 100–599 的状态码或 5xx',
      upstreamConcurrencyRaceRuleExists: '该状态规则已存在',
      upstreamConcurrencyRaceRuleBudget: 'HTTP 规则已用 {used}/{total} 次',
      upstreamConcurrencyRaceRuleBudgetExceeded: '规则总和已超过总重试次数，请调大总次数或减少其他规则。',
      upstreamConcurrencyRaceRuleBudgetFull: '总次数已分配完；新增规则默认 0 次。',
      upstreamConcurrencyRaceTransport: '系统临时错误自动重试',
      upstreamConcurrencyRaceTransportHint: '只处理连接失败、无响应和没有 HTTP 状态码的可恢复错误。',
      upstreamConcurrencyRaceTransportCount: '系统临时错误重试次数',
      upstreamConcurrencyRaceTransportBudgetExceeded: '系统临时错误重试次数不能超过抢上游总重试次数。',
      ...
      poolModeRetryConditions: '同账号重试条件',
      poolModeRetryConditionsHint: '只控制哪些错误触发同账号重试；重试间隔、次数和总耗时仍使用上方配置。',
      poolModeRetryHttpStatusCodes: 'HTTP 状态码',
      poolModeRetryStatusCodeRange: '可添加或删除 · 100–599',
      poolModeRetryStatusCodePlaceholder: '输入状态码，例如 503',
      poolModeRetryAddStatusCode: '添加',
      poolModeRetryRemoveStatusCode: '删除状态码 {code}',
      poolModeRetryNoStatusCodes: '未添加状态码',
      poolModeBuiltinRetry: '系统临时错误自动重试',
      poolModeBuiltinRetryHint: '开启后，连接失败、无响应、408、5xx 和可恢复的处理错误也会触发同账号重试。',
      poolModeRetryOverlapHint: '{codes} 同时被状态码和系统临时错误规则覆盖；重复命中只会进入一次重试流程。',
      poolModeRetryCode401: '凭证失效',
      poolModeRetryCode403: '权限拒绝',
      poolModeRetryCode408: '请求超时',
      poolModeRetryCode429: '请求限流',
      poolModeRetryCode500: '服务器错误',
      poolModeRetryCode502: '网关错误',
      poolModeRetryCode503: '服务不可用',
      poolModeRetryCode504: '网关超时',
      poolModeRetryCode529: '上游过载',
      ...
      grokCustomBaseUrl: {
        title: '自定义上游地址',
        hint: '开启后账号流量（对话、媒体和探测）按所填地址转发；OAuth 授权与令牌刷新仍走官方端点。',
      ...
      oauth: {
        ...
        openai: {
          ...
          errors: {
            ...
            OPENAI_DEVICE_AUTH_PENDING: '设备码尚未完成授权，请在浏览器中完成后再点击完成授权。',
            OPENAI_DEVICE_AUTH_SLOW_DOWN: '轮询过于频繁，请稍等几秒后再试。',
            OPENAI_DEVICE_AUTH_EXPIRED: '设备码已过期，请重新生成。',
            OPENAI_DEVICE_AUTH_DENIED: '设备码授权已被拒绝，请重新生成后再试。'
          },
          // Refresh Token auth
          ...
          deviceCodeStarting: '生成中...',
          deviceVerificationUrl: '验证地址',
          deviceUserCode: '设备代码',
          deviceCodeHint: '设备码通常在 {seconds} 秒后过期。',
          deviceCodeSecurityNotice: '设备代码可能被网络钓鱼攻击窃取。请勿分享设备代码；只在您自己的 OpenAI/Codex 登录页面输入。',
      ...
      videoPreview: '生成视频：',
      audioPreview: '生成音频：',
      grokUploadMedia: '上传测试媒体',
      grokUploadFirstFrame: '上传首帧图片（可选）',
      grokPromptLabel: '测试提示词',
      grokPromptPlaceholder: '输入测试内容；留空使用默认提示词',
      grokDirectTestHint: '此处直接测试所选账号；下游调用还受分组、调度、冷却和配额限制。',
      grokAudioRequired: '请上传不超过 8 MB 的音频文件。',
      grokMediaOptional: '图片可选，最大 8 MB。上传图片不会覆盖提示词。',
      grokMediaWrongType: '文件类型与当前测试模式不匹配。',
      grokMediaReadFailed: '文件读取失败，请重新上传。',
      testStreamTooLarge: '测试响应过大，请缩小请求后重试',
      testStreamIncomplete: '测试流已结束，但未收到完成结果，请重试。',
      grokTestIncomplete: '测试连接已结束，但未收到成功或失败结果，请查看服务端日志。',
      ttsSuccess: 'TTS 请求成功',
      grokMediaTooLarge: '测试媒体不能超过 8 MB',
      imageReceived: '已收到第 {count} 张测试图片',
      grokTestMode: 'Grok 测试模式',
      grokModes: { responses: 'Responses 文本', chat: 'Chat 文本', image: '图片', video: '视频', search: '搜索', tts: 'TTS', stt: 'STT', realtime: 'Realtime' },
      // Stats Modal
      viewStats: '查看统计',
    ...
    usage: {
      ...
      onlyMismatches: '仅模型不匹配',
      onlyMatches: '仅模型匹配',
      upstreamResponseModel: '响应模型',
      upstreamModelMismatch: '模型匹配',
      mismatch: '不匹配',
      match: '匹配',
      auditUnavailable: '未观测到',
      ipAddress: 'IP',
    ...
    settings: {
      ...
      features: {
        ...
        promptAudit: {
          title: 'Prompt 审计',
          description: '异步检查请求 Prompt 的安全风险，默认关闭。',
          configureLink: '打开 Prompt 审计',
          enabled: '启用 Prompt 审计',
          enabledHint: '关闭后管理页面和异步 Prompt 采集都会停止。',
        },
        affiliate: {
          title: '邀请返利',
      ...
      scheduling: {
        ...
        openAIPoolSoftCooldownMax: 'OpenAI 非图片池最大软冷却秒数',
        openAIPoolProbeTimeout: 'OpenAI 非图片池探测超时秒数',
      ...
      gatewayConcurrency: {
        ...
        reset: '恢复推荐默认',
        userWait: '用户槽位等待',
        userWaitHint: '1-30000ms；超时后快速返回 429，避免连接长时间排队。',
        accountWait: '账号槽位等待',
        accountWaitHint: '1-30000ms；用于账号回退路径。',
        edgeBudget: 'Edge 队列预算',
        edgeBudgetHint: '1-30000ms；部署 Edge 配置后重启 Edge 生效。',
        globalWorkers: 'Edge 全局并发',
        globalWorkersHint: '1-999999999；每个 Edge 副本同时等待上游响应头的 relay 工作数；活动流另计，保存后需重启 Edge。',
        extra: '用户额外等待数',
        extraHint: '0-50 个等待名额，叠加在用户并发上限之上。',
        retryAfter: 'Retry-After',
        retryHint: '1-60000ms；HTTP Retry-After 按协议向上取整为整秒。',
        edgeRestartHint: '用户/账号等待和 Retry-After 保存后立即对 Go 网关生效；Edge 队列预算和全局并发需更新 SUB2API_EDGE_QUEUE_WAIT_BUDGET_MS、SUB2API_EDGE_GLOBAL_WORKERS 后重启容器。',
        userWaitRange: '用户槽位等待必须在 1-30000ms 之间。',
        accountWaitRange: '账号槽位等待必须在 1-30000ms 之间。',
        edgeBudgetRange: 'Edge 队列预算必须在 1-30000ms 之间。',
      ...
      gatewayEdgeProtection: {
        ...
        connectTimeout: '建连超时',
        connectTimeoutHint: '单位 ms；必须大于 0，用于限制 Edge 建连和连接资源等待。',
        headerTimeout: '响应头超时',
        headerTimeoutHint: '单位 ms；必须大于 0，单次上游尝试等待响应头的最长时间。',
        headerBudget: '响应头总预算',
        headerBudgetHint: '单位 ms；必须大于 0，从第一次真实上游尝试开始计算，所有切号尝试共享。',
        bodyIdleTimeout: 'body 空闲超时',
        bodyIdleTimeoutHint: '单位 ms；必须大于 0，只限制连续无数据，不限制正常长流总时长。',
        maxAttempts: '响应头最大尝试',
        maxAttemptsHint: '响应头超时时，此请求在总预算内允许的真实上游尝试次数（包含首次尝试）。',
        failover: '响应头超时切号',
        failoverHint: '超时且尚未写出下游内容时，交给现有切号逻辑。',
        exposeRetriedUsage: '下游显示内部重试累计用量',
        exposeRetriedUsageHint: '开启后，最终响应的 usage 会包含本次请求已获得的所有真实上游尝试用量；仍只发送一次最终响应，不会产生多笔下游账单。',
        exposeRetriedUsageOn: '累计用量已展示',
        exposeRetriedUsageOff: '仅展示最终尝试',
        behaviorHint: 'SSE 注释预冲和空 token 占位保持原有设置与语义；Edge 只在下游尚未提交时进行切号。',
        timeoutRange: 'Edge 建连、响应头超时和总预算必须是大于 0 的安全整数。',
      ...
      gatewayForwarding: {
        ...
        claudeOAuthSystemPromptInjectionHint: '为非 Claude Code 客户端的 Claude OAuth 请求注入 Claude Code 形态的 system blocks。默认关闭，开启后会增加上游输入 prompt 体量。',
        claudeOAuthSystemPrompt: 'Claude Code 扩展提示词',
        claudeOAuthSystemPromptPlaceholder: '留空时使用内置 Claude Code 扩展提示词。',
        claudeOAuthSystemPromptHint: '兼容旧配置：用于 claude_code_expansion_prompt 占位符。',
        claudeOAuthSystemPromptBlocks: 'Claude OAuth System Blocks',
        claudeOAuthSystemPromptBlocksPlaceholder: '留空时使用内置三块默认配置。',
        claudeOAuthSystemPromptBlocksHint: '留空时使用内置三块默认。支持 billing_header、cc_version、fp、claude_code_system_prompt、claude_code_expansion_prompt 占位符。',
        claudeOAuthSystemBlockTitle: 'System Block {index}',
        claudeOAuthSystemBlockCustom: '自定义',
        claudeOAuthSystemBlockToggleVisible: '折叠或展开',
        claudeOAuthSystemBlockMoveUp: '上移',
        claudeOAuthSystemBlockMoveDown: '下移',
        claudeOAuthSystemBlockDelete: '删除',
        claudeOAuthSystemBlockPreset: '预设',
        claudeOAuthSystemBlockType: '类型',
        claudeOAuthSystemBlockTypeText: '文本',
        claudeOAuthSystemBlockContent: '内容',
        claudeOAuthSystemBlockCacheControl: 'Cache Control',
        claudeOAuthSystemBlockCache5m: '5 分钟',
        claudeOAuthSystemBlockCache1h: '1 小时',
        claudeOAuthSystemBlockAdd: '添加 block',
        claudeOAuthSystemBlockRestore: '恢复默认',
        claudeOAuthSystemPresetBillingHeader: 'Billing Header',
        claudeOAuthSystemPresetIdentity: 'Claude Code 身份提示词',
        claudeOAuthSystemPresetExpansion: 'Claude Code 扩展提示词',
        claudeOAuthSystemPresetCustom: '自定义',
        ...
        clientDatelineNormalizationHint: '开启后，清洗 Anthropic OAuth/Setup Token 请求体中客户端 dateline 的撇号和日期分隔符指纹，不改写普通用户文本。',
        ...
        openaiCodexVersionTitle: 'Codex 出站版本',
        openaiCodexVersionDesc: '仅用于 OpenAI OAuth / ChatGPT 内部链路的身份头；API Key 和其他平台不受影响。',
        openaiCodexAutoSync: '自动同步稳定版',
        openaiCodexManualVersion: '手工固定版本',
        openaiCodexManualVersionPlaceholder: '留空跟随同步值或内置 0.146.0',
        openaiCodexSyncedVersion: '最近同步版本',
        openaiCodexNotSynced: '尚未同步',
        openaiCodexVersionPrecedence: '生效顺序：手工固定版本 > 最近同步版本 > 内置 0.146.0。自动同步默认开启，每 6 小时检查一次官方稳定版且不会降级。',
        ...
        codexFpRequired: '必须',
        codexFingerprintNoRequiredWarn: '未勾选任何「必须」信号，引擎指纹门当前不生效。如需启用校验，请至少勾选一条信号。',
        codexAllowAppServer: 'Codex app-server',
        codexAllowAppServerDesc: '放行内嵌 Codex 引擎、经 app-server 协议接入的第三方客户端（如 Claude Code 的 codex 插件）。默认关闭；开启后此类客户端通过引擎指纹门即放行，关闭则仅放行官方客户端与白名单。',
        ...
        codexWhitelistSkipFingerprintTooltip: '风险：勾选后该条仅凭 originator + User-Agent 放行，不再要求引擎指纹兜底。仅用于可信但不发 codex 引擎指纹的第三方客户端。',
      ...
      payment: {
        ...
        subscriptionUsdToCnyRate: '订阅 USD→CNY 汇率',
        subscriptionUsdToCnyRateHint: '仅订阅订单生效。0 表示关闭换算，CNY 通道按套餐 price 直付；大于 0 时按 price × 汇率收款。',
        subscriptionUsdToCnyPreview: '预览：订阅 1 USD = {cny} CNY',
        disabled: '留空或 0 表示关闭',
        ...
        alipayMobilePrecreate: '支付宝移动端 App 深链',
        alipayMobilePrecreateHint: '移动端使用官方预创建二维码内容，由支付宝 App 直接打开',
        helpText: '帮助文本',
        helpImageUrl: '帮助图片链接',
      ...
      betaPolicy: {
        ...
        userIds: '用户 ID 范围',
        userIdsHint: '留空表示所有用户；填写后仅匹配这些 API Key 所属用户，且用户专属规则优先于全局规则。',
      ...
      openaiFastPolicy: {
        ...
        scopeBedrock: '仅 Bedrock 账号',
        userIds: '指定用户',
        userIdsHint: '留空表示全局生效。可按邮箱搜索并选择多个用户。',
        userSearchPlaceholder: '按邮箱搜索用户',
