...
export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      promptCacheCreationOptimizationReduceHint: 'Mode A (default): uses explicit + 30m, removes old breakpoints, and places one breakpoint only after a sufficiently large contiguous system/developer prefix to reduce repeated creation while retaining stable-prefix reuse.',
      promptCacheCreationOptimizationSuppress: 'Suppress Cache Creation',
      promptCacheCreationOptimizationSuppressHint: 'Mode B: uses explicit only, removes TTL, legacy retention, and every cache breakpoint, and never adds a new one to suppress GPT-5.6 cache creation as strongly as possible. If the upstream does not support explicit mode and compatibility fallback is triggered, implicit cache creation may resume.',
      promptCacheCreationOptimizationFree: 'Free Downstream Writes',
      promptCacheCreationOptimizationFreeHint: 'Mode C: uses Mode B for upstream requests. This node retains actual cache creation for cost auditing, while downstream usage reports zero creation and removes it from total input so same-source downstreams do not display or bill cache creation. Account-scoped only.',
      promptCacheCreationOptimizationInput125: 'Convert to 1.25x Input',
      promptCacheCreationOptimizationInput125Hint: 'Mode D: uses Mode B for upstream requests. This node retains actual cache creation, while downstream usage reports zero creation and converts it to 1.25x regular input. Cache reads remain unchanged. Account-scoped only.',
