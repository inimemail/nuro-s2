...
edge_env_value_or_default() {
    local key="$1"
    local default_value="$2"
    local file="${EDGE_ENV_READ_FILE:-$INSTALL_DIR/edge-rs.env}"

    # Key presence is distinct from a non-empty value: an explicit KEY= is a
    # user choice and must survive an installer upgrade.
...
write_edge_env() {
    local secret
    local edge_env_snapshot=""
    local EDGE_ENV_READ_FILE="$INSTALL_DIR/edge-rs.env"
    ...
    if [ -f "$INSTALL_DIR/edge-rs.env" ]; then
        edge_env_snapshot=$(mktemp)
        cp "$INSTALL_DIR/edge-rs.env" "$edge_env_snapshot"
        EDGE_ENV_READ_FILE="$edge_env_snapshot"
    fi
...
GATEWAY_OPENAI_EDGE_RS_LEASE_TTL_MS=$(edge_env_value_or_default "GATEWAY_OPENAI_EDGE_RS_LEASE_TTL_MS" "120000")
GATEWAY_CONCURRENCY_SLOT_TTL_MINUTES=$(edge_env_value_or_default "GATEWAY_CONCURRENCY_SLOT_TTL_MINUTES" "2")
...
SUB2API_EDGE_NODE_ID=$(edge_env_value_or_default "SUB2API_EDGE_NODE_ID" "sub2api-edge-rs")
SUB2API_EDGE_PREPARE_TIMEOUT_MS=$(edge_env_value_or_default "SUB2API_EDGE_PREPARE_TIMEOUT_MS" "1500")
...
SUB2API_EDGE_GLOBAL_WORKERS=$(edge_env_value_or_upgrade_default "SUB2API_EDGE_GLOBAL_WORKERS" "9999" "256" "512" "131072")
SUB2API_EDGE_PER_ACCOUNT_WORKERS=$(edge_env_value_or_upgrade_default "SUB2API_EDGE_PER_ACCOUNT_WORKERS" "0" "4" "32" "64" "128")
...
SUB2API_EDGE_MAX_PROXY_CLIENTS=$(edge_env_value_or_default "SUB2API_EDGE_MAX_PROXY_CLIENTS" "1024")
...
SUB2API_EDGE_QUEUE_WAIT_BUDGET_MS=$(edge_env_value_or_upgrade_default "SUB2API_EDGE_QUEUE_WAIT_BUDGET_MS" "50" "100" "150")
...
SUB2API_EDGE_MAX_WS_IDLE_KEYS=$(edge_env_value_or_default "SUB2API_EDGE_MAX_WS_IDLE_KEYS" "1024")
SUB2API_EDGE_WS_IDLE_TTL_SECS=$(edge_env_value_or_default "SUB2API_EDGE_WS_IDLE_TTL_SECS" "300")
SUB2API_EDGE_MAX_DYNAMIC_WARM_KEYS=$(edge_env_value_or_default "SUB2API_EDGE_MAX_DYNAMIC_WARM_KEYS" "4096")
SUB2API_EDGE_LOCAL_DATA_PLANE_FORCE_OFF=$(edge_env_value_or_default "SUB2API_EDGE_LOCAL_DATA_PLANE_FORCE_OFF" "false")
SUB2API_EDGE_LOCAL_CONTROL_POLL_MS=$(edge_env_value_or_default "SUB2API_EDGE_LOCAL_CONTROL_POLL_MS" "5000")
SUB2API_EDGE_LOCAL_ROUTE_HINT_TTL_SECS=$(edge_env_value_or_default "SUB2API_EDGE_LOCAL_ROUTE_HINT_TTL_SECS" "15")
SUB2API_EDGE_LOCAL_ROUTE_HINT_MAX_ENTRIES=$(edge_env_value_or_default "SUB2API_EDGE_LOCAL_ROUTE_HINT_MAX_ENTRIES" "65536")
SUB2API_EDGE_SETTLEMENT_WAL_ENABLED=$(edge_env_value_or_default "SUB2API_EDGE_SETTLEMENT_WAL_ENABLED" "true")
SUB2API_EDGE_SETTLEMENT_WAL_DIR=$(edge_env_value_or_default "SUB2API_EDGE_SETTLEMENT_WAL_DIR" "/opt/sub2api/data/edge-settlement-wal/sub2api-edge-rs")
SUB2API_EDGE_SETTLEMENT_WAL_COMPACT_RECORDS=$(edge_env_value_or_default "SUB2API_EDGE_SETTLEMENT_WAL_COMPACT_RECORDS" "100000")
SUB2API_EDGE_SETTLEMENT_WAL_COMPACT_BYTES=$(edge_env_value_or_default "SUB2API_EDGE_SETTLEMENT_WAL_COMPACT_BYTES" "67108864")
SUB2API_EDGE_UPSTREAM_WARM_URL=$(edge_env_value_or_default "SUB2API_EDGE_UPSTREAM_WARM_URL" "")
SUB2API_EDGE_UPSTREAM_WARM_INTERVAL_SECS=$(edge_env_value_or_default "SUB2API_EDGE_UPSTREAM_WARM_INTERVAL_SECS" "30")
SUB2API_EDGE_UPSTREAM_DYNAMIC_WARM_ACTIVE_SECS=$(edge_env_value_or_default "SUB2API_EDGE_UPSTREAM_DYNAMIC_WARM_ACTIVE_SECS" "300")
RUST_LOG=$(edge_env_value_or_default "RUST_LOG" "warn")
EOF
    if [ -n "$edge_env_snapshot" ]; then
        rm -f "$edge_env_snapshot"
    fi
