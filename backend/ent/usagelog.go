...
type UsageLog struct {
	...
	// Account-configured direct upstream response request identifier
	UpstreamRequestID *string `json:"upstream_request_id,omitempty"`
	// Explicit client-provided correlation ID; never derived from cache or sticky state
	...
	// SlotWaitMs holds the value of the "slot_wait_ms" field.
	SlotWaitMs *int `json:"slot_wait_ms,omitempty"`
	// UpstreamHeaderMs holds the value of the "upstream_header_ms" field.
	UpstreamHeaderMs *int `json:"upstream_header_ms,omitempty"`
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldSlotWaitMs:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field slot_wait_ms", values[i])
			} else if value.Valid {
				_m.SlotWaitMs = new(int)
				*_m.SlotWaitMs = int(value.Int64)
			}
		case usagelog.FieldUpstreamHeaderMs:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field upstream_header_ms", values[i])
			} else if value.Valid {
				_m.UpstreamHeaderMs = new(int)
				*_m.UpstreamHeaderMs = int(value.Int64)
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.SlotWaitMs; v != nil {
		builder.WriteString("slot_wait_ms=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	if v := _m.UpstreamHeaderMs; v != nil {
		builder.WriteString("upstream_header_ms=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
