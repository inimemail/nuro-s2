...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	schedulerEventBus := repository.ProvideSchedulerEventBus(redisClient, configConfig)
	schedulerSnapshotService := service.ProvideSchedulerSnapshotService(schedulerCache, schedulerOutboxRepository, accountRepository, groupRepository, configConfig, schedulerEventBus)
	...
	modelPlazaService := service.NewModelPlazaService(channelRepository, groupRepository, pricingService)
	channelService := service.ProvideChannelService(channelRepository, groupRepository, apiKeyAuthCacheInvalidator, pricingService, modelPlazaService, schedulerEventBus)
	...
	antigravityGatewayService := service.NewAntigravityGatewayService(accountRepository, gatewayCache, schedulerSnapshotService, antigravityTokenProvider, rateLimitService, httpUpstream, tlsFingerprintProfileService, settingService, internal500CounterCache)
	geminiMessagesCompatService := service.NewGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, tlsFingerprintProfileService, antigravityGatewayService, settingService, usageLogRepository, configConfig)
	...
	dashboardAggregationService := service.ProvideDashboardAggregationService(dashboardAggregationRepository, timingWheelService, configConfig, settingRepository, leaderLockCache, db)
	dashboardHandler := admin.NewDashboardHandler(dashboardService, dashboardAggregationService)
	proxyExitInfoProber := repository.NewProxyExitInfoProber(configConfig)
	proxyLatencyCache := repository.NewProxyLatencyCache(redisClient)
	privacyClientFactory := providePrivacyClientFactory()
	compositeAccountRuntimeBlocker := service.NewCompositeAccountRuntimeBlocker(openAIGatewayService, gatewayService, rateLimitService, openAITokenProvider)
	adminService := service.ProvideAdminService(userRepository, groupRepository, accountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, affiliateService, subscriptionService, userSubscriptionRepository, privacyClientFactory, compositeAccountRuntimeBlocker, channelService)
	...
	grokQuotaService := service.ProvideGrokQuotaService(accountRepository, proxyRepository, grokTokenProvider, httpUpstream, openAIGatewayService, configConfig)
	usageCache := service.NewUsageCache()
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, usageCache, identityCache, tlsFingerprintProfileService, openAITokenProvider, openAIGatewayService)
	accountTestService := service.NewAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, settingService, tlsFingerprintProfileService, openAITokenProvider)
	...
	imageStorageFactory := repository.ProvideImageStorageFactory(backupObjectStoreFactory)
	...
	backupHandler := admin.NewBackupHandlerWithImageStorage(backupService, userService, imageStorageSettingService)
	oAuthHandler := admin.NewOAuthHandler(oAuthService)
	openAIOAuthHandler := admin.NewOpenAIOAuthHandler(openAIOAuthService, adminService)
	geminiOAuthHandler := admin.NewGeminiOAuthHandler(geminiOAuthService)
	antigravityOAuthHandler := admin.NewAntigravityOAuthHandler(antigravityOAuthService)
	tokenRefreshService := service.ProvideTokenRefreshService(accountRepository, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, compositeTokenCacheInvalidator, schedulerCache, configConfig, tempUnschedCache, privacyClientFactory, proxyRepository, oAuthRefreshAPI, compositeAccountRuntimeBlocker)
	grokOAuthHandler := handler.ProvideGrokOAuthHandler(grokOAuthService, adminService, grokQuotaService, tokenRefreshService)
	...
	securityauditService := securityaudit.ProvideService(settingRepository, settingService, db, redisClient, secretEncryptor, configConfig)
	promptAdminHandler := securityaudit.NewPromptAdminHandler(securityauditService)
	...
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, rateLimitService)
	...
	compositeRouteHandler := admin.NewCompositeRouteHandler(compositeRouteResolver)
	cnProviderQuotaService := service.ProvideCNProviderQuotaService(accountRepository, proxyRepository, httpUpstream, configConfig)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, settingRepository, cnProviderQuotaService, leaderLockCache, db)
	...
	batchImageCleanupService := service.NewBatchImageCleanupService(batchImageRepository, accountRepository, configConfig)
	...
	modelPlazaHandler := handler.NewModelPlazaHandler(modelPlazaService, userRepository, userGroupRateRepository, settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, batchImageHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.ProvideJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.ProvideAdminAuthMiddleware(authService, userService, settingService, auditLogService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, redisClient, securityauditService)
	...
	openAICodexVersionSyncService := service.ProvideOpenAICodexVersionSyncService(settingRepository, settingService, gitHubReleaseClient, leaderLockCache, db, dashboardAggregationService, openCodeGoUsageService, backupService)
	...
	channelMonitorQuotaFetcher := service.NewChannelMonitorQuotaFetcher(accountRepository, accountUsageService, cnProviderQuotaService, cnProviderBalanceService)
	...
	opsIngressRejectAggregator := service.ProvideOpsIngressRejectAggregator(opsRepository, opsService, authCacheInvalidationWorker, apiKeyService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, schedulerSnapshotService, concurrencyService, tokenRefreshService, accountExpiryService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, openAIGatewayHandler, batchImageWorkerRuntime, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, securityauditService, authCacheInvalidationWorker, opsIngressRejectAggregator, apiKeyService)
	application := &Application{
		Server:      httpServer,
		PromptAudit: securityauditService,
		Cleanup:     v,
	}
