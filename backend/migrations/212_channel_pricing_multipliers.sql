...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'channel_model_pricing_fast_multiplier_positive') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'channel_model_pricing_flex_multiplier_positive') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'channel_pricing_intervals_multipliers_positive') THEN
        ALTER TABLE channel_pricing_intervals ADD CONSTRAINT channel_pricing_intervals_multipliers_positive CHECK (
