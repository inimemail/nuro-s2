...
func RegisterCommonRoutes(r *gin.Engine, cfg *config.Config) {
	...
	r.GET("/metrics", func(c *gin.Context) {
		...
		body += fmt.Sprintf("# TYPE sub2api_edge_prepare_cache_hits_total counter\nsub2api_edge_prepare_cache_hits_total %d\n", snapshot.EdgePrepareCacheHits)
		body += fmt.Sprintf("# TYPE sub2api_edge_prepare_cache_misses_total counter\nsub2api_edge_prepare_cache_misses_total %d\n", snapshot.EdgePrepareCacheMisses)
		body += fmt.Sprintf("# TYPE sub2api_edge_prepare_cache_evictions_total counter\nsub2api_edge_prepare_cache_evictions_total %d\n", snapshot.EdgePrepareCacheEvictions)
		body += fmt.Sprintf("# TYPE sub2api_fresh_load_requests_total counter\nsub2api_fresh_load_requests_total %d\n", snapshot.FreshLoadRequests)
		body += fmt.Sprintf("# TYPE sub2api_fresh_load_shared_total counter\nsub2api_fresh_load_shared_total %d\n", snapshot.FreshLoadShared)
		body += fmt.Sprintf("# TYPE sub2api_fresh_load_errors_total counter\nsub2api_fresh_load_errors_total %d\n", snapshot.FreshLoadErrors)
		body += fmt.Sprintf("# TYPE sub2api_fresh_load_duration_microseconds_total counter\nsub2api_fresh_load_duration_microseconds_total %d\n", snapshot.FreshLoadMicros)
		body += fmt.Sprintf("# TYPE sub2api_scheduler_snapshot_hits_total counter\nsub2api_scheduler_snapshot_hits_total %d\n", snapshot.SchedulerSnapshotHits)
		body += fmt.Sprintf("# TYPE sub2api_scheduler_snapshot_misses_total counter\nsub2api_scheduler_snapshot_misses_total %d\n", snapshot.SchedulerSnapshotMisses)
		body += fmt.Sprintf("# TYPE sub2api_scheduler_db_fallback_total counter\nsub2api_scheduler_db_fallback_total %d\n", snapshot.SchedulerDBFallbacks)
