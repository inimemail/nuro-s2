...
type OpsIngressRejectRepository interface {
	BatchUpsertIngressRejects(context.Context, []*OpsIngressRejectAggregate) error
	ListIngressRejects(context.Context, *OpsIngressRejectFilter) (*OpsIngressRejectList, error)
}
...
func NewOpsIngressRejectAggregator(repo OpsIngressRejectRepository) *OpsIngressRejectAggregator {
	...
	a := &OpsIngressRejectAggregator{repo: repo, ctx: ctx, cancel: cancel, flushCh: make(chan struct{}, 1)}
...
func (a *OpsIngressRejectAggregator) RecordIngressReject(reason, routeFamily, protocol, clientIP string, userID, apiKeyID int64) {
	...
	key := ingressRejectKey{
		reason: boundedDimension(reason, "other"), routeFamily: boundedDimension(routeFamily, "other"),
...
// snapshotLocked captures deltas. Periodic flushes retain dimensions for the
// whole minute so they cannot reset the cardinality budget.
...
func (a *OpsIngressRejectAggregator) flushPending() {
	for {
		...
		if err != nil {
			...
			a.lastError.Store("flush failed")
...
func (a *OpsIngressRejectAggregator) Health() OpsIngressRejectHealth {
	health := OpsIngressRejectHealth{Capacity: ingressRejectMaxEntries}
	if a == nil {
		return health
	}
	health.Cardinality = a.cardinality.Load()
	a.overflowMu.Lock()
	if a.overflow != nil {
		health.Cardinality++
	}
	a.overflowMu.Unlock()
	a.pendingMu.Lock()
	health.PendingBatches = len(a.pending)
	for _, batch := range a.pending {
		health.PendingRows += len(batch)
	}
	a.pendingMu.Unlock()
	health.Overflowed = a.overflowed.Load()
	health.Dropped = a.dropped.Load()
	health.Flushed = a.flushed.Load()
	health.FlushFailures = a.flushFailures.Load()
	health.Accepting = a.accepting.Load()
	if value := a.lastError.Load(); value != nil {
...
func ingressRejectHash(key ingressRejectKey) int {
	hash := uint64(1469598103934665603)
	for _, value := range []string{key.reason, key.routeFamily, key.protocol, key.clientIP} {
		for i := 0; i < len(value); i++ {
			hash ^= uint64(value[i])
			hash *= 1099511628211
		}
	}
	hash ^= uint64(key.userID)
	hash *= 1099511628211
	hash ^= uint64(key.apiKeyID)
	return int(hash & 0x7fffffff)
}

func aggregateFromKey(key ingressRejectKey, bucket, now time.Time) *OpsIngressRejectAggregate {
	item := &OpsIngressRejectAggregate{
		BucketStart: bucket, RejectReason: key.reason, RouteFamily: key.routeFamily, Protocol: key.protocol,
		ClientIP: key.clientIP, RequestCount: 1, FirstSeen: now, LastSeen: now,
	}
	if key.userID > 0 {
		value := key.userID
		item.UserID = &value
...
func (s *OpsService) SetIngressRejectAggregator(aggregator *OpsIngressRejectAggregator) {
	if s != nil {
		s.ingressRejectAggregator = aggregator
	}
}
