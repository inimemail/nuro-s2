...
func (s *AntigravityGatewayService) validateAntigravityCompatAccount(c *gin.Context, account *Account) error {
	...
	return s.writeAntigravityCompatError(
		...
		"compatible OAuth account required for this request",
	)
}
...
func (s *AntigravityGatewayService) prepareAntigravityCompatCall(
	...
	if s.tokenProvider == nil {
		return nil, s.writeAntigravityCompatError(c, http.StatusBadGateway, "api_error", "Upstream authentication is unavailable")
	}
	...
	if err != nil {
		_ = s.writeAntigravityCompatError(c, http.StatusBadRequest, "invalid_request_error", "Account project configuration is required")
		return nil, err
	}
...
func (s *AntigravityGatewayService) consumeAntigravityCompatResponse(
	...
	if streamResult == nil {
		if err == nil {
			err = errors.New("antigravity compatibility response result is nil")
		}
		return nil, err
	}
	...
	result := &ForwardResult{
		...
		NeutralOutcome:   streamResult.localCapacityLimited,
	}
	return result, err
}
...
func (s *AntigravityGatewayService) mapAntigravityCompatCollectionError(c *gin.Context, err error) error {
	...
	if errors.Is(err, ErrUpstreamResponseBodyTooLarge) {
		return s.writeAntigravityCompatError(c, http.StatusBadGateway, "response_too_large", "Upstream response too large")
	}
