...
func TestOpenAIGatewayServiceRecordUsage_MissingPricingRecordsZeroCostUsageLog(t *testing.T) {
	...
	require.Equal(t, "missing-pricing-model", usageRepo.lastLog.Model)
...
func TestOpenAIGatewayServiceRecordUsage_SparkShadowUsesCurrentParentBillingSetting(t *testing.T) {
	for _, parentEnabled := range []bool{false, true} {
		t.Run(strconv.FormatBool(parentEnabled), func(t *testing.T) {
			usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
			accountRepo := &openAIRecordUsageAccountRepoStub{account: &Account{
				ID:       4016,
				Platform: PlatformOpenAI,
				Type:     AccountTypeOAuth,
				Extra:    map[string]any{openAILongContextBillingEnabledKey: parentEnabled},
			}}
			svc := newOpenAIRecordUsageServiceForTest(
				usageRepo,
				&openAIRecordUsageUserRepoStub{},
			...
			err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
				...
				Account: &Account{
					...
					Extra: map[string]any{
						openAILongContextBillingEnabledKey: !parentEnabled,
					},
				},
			})
			...
			require.Equal(t, parentEnabled, usageRepo.lastLog.LongContextBillingApplied)
		})
	}
}
...
func TestOpenAIGatewayServiceRecordUsage_GrokVideoUsesPerSecondBilling(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:            "resp_video_per_second",
	...
	require.InDelta(t, 2.5, usageRepo.lastLog.TotalCost, 1e-12)
...
func TestOpenAIGatewayServiceRecordUsage_GrokVideoChannelOverrideUsesPerSecondBilling(t *testing.T) {
	groupID := int64(127)
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	svc.resolver = newOpenAIImageChannelPricingResolverForTest(t, groupID, "grok-imagine-video-1.5", 0.25)
	...
	require.InDelta(t, 2.5, usageRepo.lastLog.ActualCost, 1e-12)
	require.InDelta(t, 0.5, usageRepo.lastLog.RateMultiplier, 1e-12)
	require.NotNil(t, usageRepo.lastLog.BillingMode)
	require.Equal(t, string(BillingModeVideo), *usageRepo.lastLog.BillingMode)
}
