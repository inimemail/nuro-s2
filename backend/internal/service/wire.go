...
func ProvideCNProviderBalanceService(accountRepo AccountRepository, proxyRepo ProxyRepository, httpUpstream HTTPUpstream, cfg *config.Config) *CNProviderBalanceService {
...
func ProvideAccountUsageService(
	...
	service := NewAccountUsageService(
		...
		antigravityQuotaFetcher, grokQuotaFetcher, cache, identityCache,
...
// ProvideOpenAICodexVersionSyncService creates the opt-in background release
// synchronizer. The request path only reads the atomic identity snapshot.
...
func ProvideConcurrencyService(cache ConcurrencyCache, accountRepo AccountRepository, cfg *config.Config) *ConcurrencyService {
	...
	if cfg != nil {
		...
		svc.SetFreshLoadSingleflightEnabled(cfg.Gateway.Scheduling.FreshLoadSingleflightEnabled)
