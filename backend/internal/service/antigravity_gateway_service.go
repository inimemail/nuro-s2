...
//
// 默认使用生产端点，与 OAuth 登录/测试连接保持一致。daily/sandbox 仅供联调，
// 需要显式设置 GATEWAY_ANTIGRAVITY_FORWARD_BASE_URL=daily 或 sandbox。
...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			retryResp, retryErr := p.httpUpstream.DoWithTLS(retryReq, p.proxyURL, p.account.ID, p.account.Concurrency, s.resolveTLSProfile(p.account))
		...
		if s.cache != nil && p.sessionHash != "" {
			_ = deleteStickySessionAccountID(p.ctx, s.cache, p.groupID, p.sessionHash)
		}
...
urlFallbackLoop:
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			resp, err = p.httpUpstream.DoWithTLS(upstreamReq, p.proxyURL, p.account.ID, p.account.Concurrency, s.resolveTLSProfile(p.account))
			if err == nil && resp == nil {
...
func (s *AntigravityGatewayService) setModelRateLimitAndClearSession(p *handleModelRateLimitParams, info *antigravitySmartRetryInfo) {
	...
	if p.cache != nil && p.sessionHash != "" {
		_ = deleteStickySessionAccountID(p.ctx, p.cache, p.groupID, p.sessionHash)
	}
}
...
func (s *AntigravityGatewayService) collectClaudeStreamResponse(resp *http.Response, startTime time.Time, originalModel string) ([]byte, *antigravityStreamResult, error) {
	...
	maxResponseBytes := resolveUpstreamResponseReadLimit(nil)
	if s.settingService != nil {
		maxResponseBytes = resolveUpstreamResponseReadLimit(s.settingService.cfg)
	}
	...
	for {
		...
		case ev, ok := <-events:
			...
			if responseBytes > maxResponseBytes {
				return nil, nil, fmt.Errorf("%w: limit=%d", ErrUpstreamResponseBodyTooLarge, maxResponseBytes)
			}
			trimmed := strings.TrimRight(line, "\r\n")
...
returnResponse:
	...
	if err != nil {
		logger.LegacyPrintf("service.antigravity_gateway", "[antigravity-Forward] transform_error error=%v", err)
