...
const (
	...
	cnBalanceExtraSuffixLow       = "balance_low"
)
...
func (s *CNProviderBalanceService) queryBalanceForAccount(ctx context.Context, account *Account) (*CNProviderBalanceResult, error) {
	...
	case PlatformKimi:
		...
		balance, parseErr := parseKimiBalanceResponse(bodyBytes)
		if parseErr != nil {
...
func parseKimiBalanceResponse(body []byte) (float64, error) {
	if code := gjson.GetBytes(body, "code"); code.Exists() && code.Int() != 0 {
		return 0, fmt.Errorf("API error (code %d)", code.Int())
	}
	balanceValue := gjson.GetBytes(body, "data.available_balance")
	balance, ok := cnParseF64(balanceValue.Value())
	if !balanceValue.Exists() || !ok {
		return 0, fmt.Errorf("Invalid balance response: missing available_balance")
	}
	return balance, nil
}
