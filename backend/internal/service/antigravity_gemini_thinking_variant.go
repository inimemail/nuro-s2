...
func accountRawModelMappingHasKey(account *Account, key string) bool {
	...
	switch configured := account.Credentials["model_mapping"].(type) {
	case map[string]any:
	...
	// Preserve explicit wildcard/self mappings as well as exact aliases.
	_, matched := resolveRequestedModelInMapping(raw, key)
	return matched
}
...
func resolveGeminiThinkingVariant(account *Account, requestedModel string, body []byte) (string, bool) {
	alias, matched := resolveGeminiThinkingVariantModel(account, requestedModel, geminiThinkingLevelFromBody(body))
	if !matched {
		return "", false
	}
	return account.GetMappedModel(alias), true
}

type geminiThinkingVariantRequestKey struct{}

// Native Gemini is the only path that translates thinkingConfig into a model
// variant. Keep Messages and native Gemini accounts on their existing mapping.
func WithGeminiThinkingVariantRequest(ctx context.Context, body []byte) context.Context {
	return context.WithValue(ctx, geminiThinkingVariantRequestKey{}, geminiThinkingLevelFromBody(body))
}

func geminiThinkingVariantSchedulingModel(ctx context.Context, account *Account, requestedModel string) string {
	if ctx == nil || account == nil || account.Platform != PlatformAntigravity {
		return requestedModel
	}
	if level, ok := ctx.Value(geminiThinkingVariantRequestKey{}).(string); ok {
		if alias, matched := resolveGeminiThinkingVariantModel(account, requestedModel, level); matched {
			return alias
		}
	}
	return requestedModel
...
func resolveGeminiThinkingVariantModel(account *Account, requestedModel, preferred string) (string, bool) {
