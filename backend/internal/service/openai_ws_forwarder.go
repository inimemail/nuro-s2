...
func (e *openAIWSRejectedFieldError) Error() string {
	return "upstream rejected a supported Responses request field"
}

func openAIWSRejectedFieldResponse(err error) ([]byte, bool) {
	var rejectedErr *openAIWSRejectedFieldError
	if !errors.As(err, &rejectedErr) || rejectedErr == nil || len(rejectedErr.responseBody) == 0 {
		return nil, false
	}
	return append([]byte(nil), rejectedErr.responseBody...), true
}
...
type OpenAIWSIngressHooks struct {
	...
	// MapRequestModel resolves the client-facing model for each turn before
	// account mapping and policy evaluation. It must remain an in-memory lookup.
...
func normalizeOpenAIWSUpstreamEventForSafety(message []byte, eventType string) ([]byte, string, bool) {
	eventType = strings.TrimSpace(eventType)
	if eventType == "error" || eventType == "response.failed" || !openAIPassthroughResponseIsUnsafe(message) {
		return message, eventType, false
	}
	return safeOpenAIStreamErrorPayload("response.failed"), "response.failed", true
}
...
// IsOpenAIWSClientDisconnectError lets the handler keep downstream disconnects
// neutral for upstream account health.
func IsOpenAIWSClientDisconnectError(err error) bool {
...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	if c != nil && c.Request != nil {
		identityCtx = c.Request.Context()
	}
	authHeaders, err := s.buildOpenAIAuthenticationHeaders(identityCtx, account, token)
	if err != nil {
	...
	strongIsolationEnabled := account != nil && account.IsOpenAIUpstreamStrongIsolationEnabled()
	if strongIsolationEnabled {
		sessionResolution = openAIWSSessionHeaderResolution{
			SessionSource:      "strong_isolation",
			ConversationSource: "strong_isolation",
		}
	}
	if c != nil && c.Request != nil {
	...
	if state := strings.TrimSpace(turnState); state != "" && !strongIsolationEnabled {
		headers.Set(openAIWSTurnStateHeader, state)
	}
	if metadata := strings.TrimSpace(turnMetadata); metadata != "" && !strongIsolationEnabled {
...
func (s *OpenAIGatewayService) buildOpenAIWSCreatePayload(reqBody map[string]any, account *Account) map[string]any {
	...
	if account != nil && account.IsOpenAIUpstreamStrongIsolationEnabled() {
		applyOpenAIUpstreamStrongIsolationWSMap(payload, true)
	}
	return payload
}
...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	turnState := ""
	turnMetadata := ""
	if !strongIsolationEnabled && c != nil && c.Request != nil {
	...
	if !strongIsolationEnabled && sessionHash == "" {
	...
	if err != nil {
		...
		var dialErr *openAIWSDialError
		if !agentIdentityTaskRecoveryWasTried(ctx) && s.isAgentIdentityAccount(ctx, account) && errors.As(err, &dialErr) && isAgentIdentityTaskInvalidWSDialError(dialErr) {
			expectedTaskID := account.GetCredential("task_id")
			if recoveryErr := s.recoverAgentIdentityTask(ctx, account, expectedTaskID); recoveryErr != nil {
				return nil, wrapOpenAIWSFallback("auth_failed", fmt.Errorf("agent identity task recovery failed: %w", recoveryErr))
			}
			return nil, wrapOpenAIWSFallback("agent_identity_task_recovered", &agentIdentityTaskRecoveredError{})
		}
	...
	cleanExit := false
	defer func() {
		if strongIsolationEnabled || !cleanExit {
			lease.MarkBroken()
		}
		lease.Release()
	}()
	...
	if handshakeTurnState != "" {
		if !strongIsolationEnabled && stateStore != nil && sessionHash != "" {
			stateStore.BindSessionTurnState(groupID, sessionHash, handshakeTurnState, s.openAIWSSessionStickyTTL())
		}
		if !strongIsolationEnabled && c != nil {
	...
	for {
		...
		message = normalizeOpenAIWSDownstreamUsage(message, eventType, downstreamCacheUsageMode, downstreamCacheMarkup)
		...
		if eventType == "error" || eventType == "response.failed" {
			decision := s.handleOpenAICyberPolicyEvent(
				...
				account,
				false,
				lease.HandshakeHeaders().Get("x-request-id"),
				originalMessage,
		...
		if eventType == "error" {
			...
			clientErrMsg := sanitizeUpstreamErrorMessage(errMsg)
			if clientErrMsg == "" {
				clientErrMsg = safeUpstreamErrorMessage
			}
		...
		if reqStream {
			...
			if eventType == "response.failed" {
				_, _, clientResponseField = parseOpenAIWSEventEnvelope(clientMessage)
			}
			if clientResponseField.Exists() && clientResponseField.Type == gjson.JSON {
				finalResponse = []byte(clientResponseField.Raw)
			}
		}

		if isTerminalEvent {
			if len(pendingJSONDocuments) > 0 {
				// A terminal event ends one Responses turn. If the same transport
				// message also contained later documents, do not report the turn as
				// successful or let those events contaminate the next accounting
				// sample.
				lease.MarkBroken()
				return nil, errors.New("websocket response contained events after terminal event")
			}
			cleanExit = true
			break
		}
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if forceHTTPBridge {
		...
		wsPath = grokChatResponsesEndpoint
	} else {
		var err error
		wsURL, err = s.buildOpenAIResponsesWSURL(account)
		if err != nil {
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		if turnMetadata := strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)); turnMetadata != "" && !strongIsolationEnabled {
		...
		if strongIsolationEnabled {
			isolated, _, isolationErr := applyOpenAIUpstreamStrongIsolationWSBody(normalized, true)
			if isolationErr != nil {
				return openAIWSClientPayload{}, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", isolationErr)
			}
			normalized = isolated
		...
		return openAIWSClientPayload{
			...
			downstreamCacheMarkup:            downstreamCacheMarkup,
		}, nil
	}
	...
	if forceHTTPBridge || forceHTTPBridgeForStrongIsolation || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		bridgeReason := "threshold"
		if forceHTTPBridge {
			bridgeReason = "account_mode"
		} else if forceHTTPBridgeForStrongIsolation {
		...
		// is resolved per turn so a model switch cannot reuse another cache ID.
		...
		for turn := 1; ; turn++ {
			...
			if account.Platform == PlatformGrok {
				grokCacheIdentity, err = resolveGrokWSCacheIdentity(c, account, grokCacheSeedPayload, currentBridgePayload.payloadRaw, currentBridgePayload.originalModel)
				if err != nil {
	...
	agentIdentityTaskRecovered := false
	acquireTurnLease := func(turn int, preferred string, forcePreferredConn bool) (*openAIWSConnLease, error) {
		for {
			...
			req.ForceNewConn = dedicatedMode || strongIsolationEnabled
			...
			if acquireErr != nil {
				...
				var dialErr *openAIWSDialError
				if !agentIdentityTaskRecovered && s.isAgentIdentityAccount(ctx, account) && isAgentIdentityTaskInvalidWSDialResult(dialStatus, acquireErr) {
					expectedTaskID := strings.TrimSpace(account.GetCredential("task_id"))
					if recoveryErr := s.recoverAgentIdentityTask(ctx, account, expectedTaskID); recoveryErr != nil {
						return nil, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "upstream websocket authentication failed", recoveryErr)
					}
					updatedHeaders, _, headerErr := s.buildOpenAIWSHeaders(c, account, token, wsDecision, isCodexCLI, turnState, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), firstPayload.promptCacheKey)
					if headerErr != nil {
						return nil, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "upstream websocket authentication failed", headerErr)
					}
					baseAcquireReq.Headers = updatedHeaders
					agentIdentityTaskRecovered = true
					continue
				}
			...
			connID := strings.TrimSpace(lease.ConnID())
			if handshakeTurnState := strings.TrimSpace(lease.HandshakeHeader(openAIWSTurnStateHeader)); handshakeTurnState != "" && !strongIsolationEnabled {
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string, cacheCreationOptimizationApplied bool, downstreamCacheUsageMode string, downstreamCacheMarkup OpenAIDownstreamCacheMarkupPolicy) (*OpenAIForwardResult, error) {
		if lease == nil {
		...
		for {
			...
			if !wroteDownstream && (eventType == "error" || eventType == "response.failed") {
				_, _, rejectedField, rejectedErr := normalizeOpenAIResponsesRejectedFieldRetryBody(
					http.StatusBadRequest,
					payload,
					originalUpstreamMessage,
				)
				if rejectedErr == nil && rejectedField {
					lease.MarkBroken()
					return nil, &openAIWSRejectedFieldError{responseBody: append([]byte(nil), originalUpstreamMessage...)}
				}
			}
			if eventType == "error" {
				...
				// contradictory completed frame on a connection that has already failed.
				lease.MarkBroken()
			}
			isTokenEvent := isOpenAIWSTokenEvent(eventType)
			if openAIRequestPlaceholderCoordinationActive(c) {
			...
			upstreamMessage = normalizeOpenAIWSDownstreamUsage(upstreamMessage, eventType, downstreamCacheUsageMode, downstreamCacheMarkup)
			...
			if !clientDisconnected {
				...
				clientMessage := sanitizeOpenAIWSErrorEventForClient(upstreamMessage, eventType, wroteDownstream)
	...
	releaseSessionLease := func() {
		...
		if strongIsolationEnabled || !lastTurnClean {
	...
	recoverIngressRejectedField := func(relayErr error, turn int, connID string) bool {
		responseBody, ok := openAIWSRejectedFieldResponse(relayErr)
		if !ok {
			return false
		}
		nextPayload, field, changed, rewriteErr := rejectedFieldRetry.Rewrite(
			http.StatusBadRequest,
			currentPayload,
			responseBody,
		)
	...
	for {
		...
		if !strongIsolationEnabled && shouldInferIngressFunctionCallOutputPreviousResponseID(
			storeDisabled,
			turn,
			toolSignals,
		...
		if nextPayload.promptCacheKey != "" {
			...
			if headerErr != nil {
				return fmt.Errorf("refresh openai websocket authentication headers: %w", headerErr)
			}
			baseAcquireReq.Headers = updatedHeaders
		}
...
func (s *OpenAIGatewayService) performOpenAIWSGeneratePrewarm(
	...
	for {
		...
		originalEventType, eventResponseID, _ := parseOpenAIWSEventEnvelope(message)
		message, eventType, _ := normalizeOpenAIWSUpstreamEventForSafety(message, originalEventType)
		if eventType == "" {
			continue
		}
		prewarmEventCount++
		...
		if isOpenAIWSTerminalEvent(eventType) {
			...
			if !isOpenAIWSSuccessTerminalEvent(eventType) {
				lease.MarkBroken()
				return wrapOpenAIWSFallback("prewarm_unsuccessful_terminal", fmt.Errorf("prewarm ended with %s", eventType))
			}
			break
		}
	}
...
func (s *OpenAIGatewayService) selectAccountByPreviousResponseIDForCapability(
	...
	result, acquireErr := s.tryAcquireAccountSlot(ctx, accountID, account.Concurrency, account.Platform)
