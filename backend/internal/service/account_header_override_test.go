...
func TestHeaderOverrideCacheClearedWhenCloningSchedulerAccount(t *testing.T) {
	account := Account{
		Platform: PlatformOpenAI,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
			credKeyHeaderOverrideEnabled: true,
			credKeyHeaderOverrides:       map[string]any{"user-agent": "before"},
		},
	}
	require.Equal(t, map[string]string{"user-agent": "before"}, account.GetHeaderOverrides())
	require.True(t, account.headerOverrideCacheReady)

	clone := cloneSchedulerAccount(account)
	require.False(t, clone.headerOverrideCacheReady)
	require.Nil(t, clone.headerOverrideCache)

	rawOverrides := clone.Credentials[credKeyHeaderOverrides].(map[string]any)
	rawOverrides["user-agent"] = "after"
	require.Equal(t, map[string]string{"user-agent": "after"}, clone.GetHeaderOverrides())
}
