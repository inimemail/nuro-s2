...
type AdminService interface {
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode, poolMode string, sortBy, sortOrder string) ([]Account, int64, error)
...
func SetAdminServiceRuntimeBlocker(adminService AdminService, runtimeBlocker AccountRuntimeBlocker) {
	if impl, ok := adminService.(*adminServiceImpl); ok {
		impl.runtimeBlocker = runtimeBlocker
	}
}
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if input.Role != "" {
		...
		if user.Role == RoleAdmin && role == RoleUser {
			if input.ActorAdminID > 0 && input.ActorAdminID == user.ID {
...
func (s *adminServiceImpl) UpdateUserBalance(ctx context.Context, userID int64, balance float64, operation string, notes string) (*User, error) {
	...
	// Read before the atomic mutation. A post-mutation read can fail after the
	// balance has already committed, making a retry apply the adjustment twice.
...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode, poolMode string, sortBy, sortOrder string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, search, groupID, privacyMode, poolMode)
	if err != nil {
		return nil, 0, err
	}
...
type nonOpenAIPoolRecoveryProbeAdminKicker interface {
	MaybeKickNonOpenAIPoolRecoveryProbeFromAdminList(ctx context.Context, account *Account)
}
...
func (s *adminServiceImpl) attachAccountRuntimeState(ctx context.Context, account *Account) {
	...
	if reader, ok := s.runtimeBlocker.(nonOpenAIPoolRuntimeStateReader); ok {
		...
		if state.Cooling {
			if state.Due && !state.ProbeInFlight {
				if kicker, ok := s.runtimeBlocker.(nonOpenAIPoolRecoveryProbeAdminKicker); ok {
			...
			account.NonOpenAIPoolSoftCooldownProbeModel = state.ProbeModel
			account.NonOpenAIPoolSoftCooldownProbeKind = state.ProbeKind
			account.NonOpenAIPoolSoftCooldownSource = state.CooldownSource
			account.NonOpenAIPoolLastProbeStatusCode = state.LastProbeStatus
			account.NonOpenAIPoolLastProbeReason = state.LastProbeReason
...
// ValidateGrokMediaEligibilityExtra validates the tri-state media routing
// override. JSON null clears the override and returns to automatic detection.
...
func normalizeGrokMediaEligibilityUpdateExtra(account *Account, input *UpdateAccountInput, normalized map[string]any) (map[string]any, error) {
	...
	// The account editor sends this field as a small patch. Merge it with the
	// database copy so a media-policy edit cannot erase runtime observations or
	// unrelated administrator-owned extra fields.
	if len(input.Extra) == 1 {
		if raw, ok := input.Extra[GrokMediaEligibleExtraKey]; ok {
			merged := maps.Clone(account.Extra)
			if merged == nil {
				merged = make(map[string]any, 1)
			}
			if raw == nil {
				delete(merged, GrokMediaEligibleExtraKey)
			} else {
				merged[GrokMediaEligibleExtraKey] = raw
			}
			return merged, nil
		}
	}
	...
	// Probes own these fields. The current database values are restored by the
	// update path below; a client payload must never become the returned state.
	delete(normalized, GrokBillingSnapshotExtraKey)
	delete(normalized, GrokQuotaSnapshotExtraKey)
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.UpstreamBillingGuardEnabled != nil {
		account.UpstreamBillingGuardEnabled = guardCapableAccount && *input.UpstreamBillingGuardEnabled
	}
	if input.Notes != nil {
	...
	if input.Extra != nil {
		...
		for _, key := range []string{
			...
			GrokBillingSnapshotExtraKey, GrokQuotaSnapshotExtraKey,
		} {
	...
	if input.UpstreamBillingGuardEnabled != nil && *input.UpstreamBillingGuardEnabled {
		...
		if input.GroupIDs == nil {
			hasConfiguredLimit = account.HasUpstreamBillingGuardGroupLimit()
		} else {
			hasConfiguredLimit, err = hasConfiguredGuardGroup(*input.GroupIDs)
			if err != nil {
				return nil, err
			}
		}
	...
	// Removing the last protected group must not leave a misleading enabled
	// master switch. Group thresholds remain authoritative and are unchanged.
	if input.GroupIDs != nil && account.UpstreamBillingGuardEnabled {
		hasConfiguredLimit, checkErr := hasConfiguredGuardGroup(*input.GroupIDs)
		if checkErr != nil {
			return nil, checkErr
		}
		if !hasConfiguredLimit {
...
func (s *adminServiceImpl) CreateProxy(ctx context.Context, input *CreateProxyInput) (*Proxy, error) {
	...
	if mode == FallbackModeProxy && input.BackupProxyID == nil {
		return nil, infraerrors.BadRequest("PROXY_BACKUP_REQUIRED", "backup proxy is required when fallback mode is proxy")
	}
	if input.ExpiryWarnDays < 0 {
		return nil, infraerrors.BadRequest("PROXY_WARN_DAYS_INVALID", "expiry warn days must be non-negative")
	}
