...
var opsPlainTextSecretPatterns = []*regexp.Regexp{
	...
	regexp.MustCompile(`(?i)\b((?:api[_-]?key|apikey|access[_-]?token|refresh[_-]?token|id[_-]?token|session[_-]?token|token|authorization|password|passwd|secret|client[_-]?secret|private[_-]?key|signature)\s*[:=]\s*)(["']?)[^"'\s,;]{1,}`),
	...
	// Header-shaped credentials can contain spaces (for example, "Basic
	// value"). Redact the remainder of the line so no suffix survives.
	regexp.MustCompile(`(?i)\b(?:authorization|proxy-authorization|cookie|set-cookie)\s*[:=][^\r\n]+`),
}
...
func jitterDuration(base time.Duration, percent int) time.Duration {
	...
	factor := 1 - float64(percent)/100 + rand.Float64()*(2*float64(percent)/100)
	if factor <= 0 {
		return base
	}
...
func sanitizeOpsUpstreamErrors(entry *OpsInsertErrorLogInput) error {
	...
	const (
		...
		opsUpstreamErrorsOlderMessageMax = 512
	...
	for i, ev := range events {
		...
		if !keepBody {
			msg = truncateString(msg, opsUpstreamErrorsOlderMessageMax)
		} else {
			msg = truncateString(msg, 2048)
		}
		out.Message = msg
		...
		// Judge empty-event removal before clearing older details, so a real
		// status-only attempt is retained even outside the body window.
		if out.UpstreamStatusCode == 0 && out.Message == "" && detail == "" && strings.TrimSpace(out.UpstreamResponseBody) == "" {
			continue
		}
		if keepBody && detail != "" {
	...
	sanitized, _ = boundOpsUpstreamErrors(sanitized, opsUpstreamErrorsMaxEvents, opsUpstreamErrorsQueueMaxBytes)
	entry.UpstreamErrorsJSON = marshalOpsUpstreamErrors(sanitized)
	entry.UpstreamErrors = nil
	return nil
}

func boundOpsUpstreamErrors(events []*OpsUpstreamErrorEvent, maxEvents, maxBytes int) ([]*OpsUpstreamErrorEvent, int) {
	if len(events) == 0 {
		return events, 0
	}
	if maxEvents <= 0 {
	...
	for i := len(events) - 1; i >= 0; i-- {
		...
		// Always retain the newest event; otherwise retain only events that fit.
