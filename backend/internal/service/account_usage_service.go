...
func (s *AccountUsageService) GetUsage(ctx context.Context, accountID int64, force ...bool) (*UsageInfo, error) {
	...
	return s.getUsageForAccount(ctx, account, len(force) > 0 && force[0])
}

// getUsageForAccount reuses an already loaded account so quota monitors do not
// repeat the account/proxy/group lookup before querying upstream usage.
func (s *AccountUsageService) getUsageForAccount(ctx context.Context, account *Account, forceProbe bool) (*UsageInfo, error) {
	...
	if account.Platform == PlatformGrok {
		// Passive quota snapshots are not proof that invalid OAuth credentials
		// recovered. Only a successful refresh or explicit admin action may
		// restore a Grok account that reconciliation blocked.
...
func shouldRefreshOpenAICodexResetCredits(account *Account, now time.Time) bool {
	...
	raw, ok := account.Extra["codex_reset_credits_updated_at"]
	if !ok {
		return true
	}
...
func applyOpenAICodexResetCreditsFromExtra(usage *UsageInfo, extra map[string]any) {
...
func buildOpenAIUsageFromExtra(account *Account, now time.Time) *UsageInfo {
...
func (s *AccountUsageService) ConsumeOpenAICodexResetCredit(ctx context.Context, accountID int64) (*UsageInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("marshal openai reset credit request: %w", err)
	}
		...
		if updates := buildOpenAICodexResetCreditOptimisticUpdates(account, time.Now()); len(updates) > 0 {
			if s.accountRepo != nil {
				if updateErr := s.accountRepo.UpdateExtra(ctx, account.ID, updates); updateErr != nil {
					slog.Warn("openai_codex_reset_credit_optimistic_persist_failed", "account_id", accountID, "error", updateErr)
				}
			}
			mergeAccountExtra(account, updates)
		}
...
func buildOpenAICodexResetCreditOptimisticUpdates(account *Account, now time.Time) map[string]any {
	...
	if supportedRaw, ok := account.Extra["codex_reset_credits_supported"]; ok && !parseExtraBool(supportedRaw) {
		return nil
	}
	raw, ok := account.Extra["codex_reset_credits"]
	if !ok {
		return nil
	}
	count := parseExtraInt(raw)
...
func (s *AccountUsageService) getGrokUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	...
	if account != nil && account.IsGrokOAuth() && s.grokQuotaService != nil &&
		...
		var err error
		if force {
			// A manual refresh should use the complete quota path: billing first,
			// then the existing response-header probe for Free/partial accounts.
	...
	if s.usageLogRepo != nil && account != nil {
		...
		usage.GrokLocalUsage24h, usage.GrokLocalUsage7d, usage.GrokLocalUsageMonthly =
			grokLocalUsageForQuota(ctx, s.usageLogRepo, account.ID, usage.GrokBilling, time.Now().UTC())
	}
...
func grokLocalUsageForQuota(
	...
	stats, err := repo.GetAccountWindowStats(ctx, accountID, now.UTC().Add(-grokFreeQuotaWindow))
	if err != nil {
		slog.Warn("grok_rolling_24h_usage_query_failed", "account_id", accountID, "error", err)
		return nil, nil, nil
	}
	return windowStatsFromAccountStats(stats), nil, nil
}
