...
func TestOpenAIGatewayServiceForward_CodexImageBridgeSkipsCompact(t *testing.T) {
	...
	result, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.4","input":"compact me","stream":false}`))

	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	require.False(t, gjson.GetBytes(upstream.lastBody, `tools.#(type=="image_generation")`).Exists())
	require.False(t, strings.Contains(gjson.GetBytes(upstream.lastBody, "instructions").String(), "image_generation"))
