...
func (s *GatewayService) ForwardAsChatCompletions(
	...
	if resp.StatusCode >= 400 {
		...
		if failoverErr, ok := s.maybeAnthropicPoolClientErrorFailover(ctx, resp, c, account, mappedModel, "[Anthropic ChatCompletions Compat]", false); ok {
		...
		writeGatewayCCError(c, mapUpstreamStatusCode(resp.StatusCode), "server_error", safeUpstreamErrorMessage)
