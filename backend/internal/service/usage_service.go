...
// GetUsageTrendWithFilters returns trend data using the shared usage filters.
...
// GetModelStatsWithFiltersBySource returns model stats using the shared usage filters.
func (s *UsageService) GetModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, filters usagestats.UsageLogFilters, modelSource string) ([]usagestats.ModelStat, error) {
	...
	if sourceRepo, ok := s.usageRepo.(modelStatsBySourceRepo); ok {
		stats, err := sourceRepo.GetModelStatsWithFiltersBySource(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.RequestType, filters.Stream, filters.BillingType, usagestats.NormalizeModelSource(modelSource))
		if err != nil {
...
// GetGroupStatsWithFilters returns group stats using the shared usage filters.
