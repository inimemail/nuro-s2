...
func TestHasOpenAIImageGenerationTool_DetectsImageGenNamespace(t *testing.T) {
	tests := []struct {
		name    string
		reqBody map[string]any
		want    bool
		...
		{
			name: "input additional_tools namespace image_gen",
			reqBody: map[string]any{
				"input": []any{
					map[string]any{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.want, hasOpenAIImageGenerationTool(tt.reqBody))
		})
	}
}
...
func TestFilterCodexInput_PreservesEncryptedReasoningWithoutStoredID(t *testing.T) {
	build := func() []any {
		return []any{
			map[string]any{"type": "message", "id": "msg_0", "role": "user", "content": "hi"},
			map[string]any{
	...
	for _, preserve := range []bool{true, false} {
		...
		t.Run(fmt.Sprintf("preserveReferences=%v", preserve), func(t *testing.T) {
			...
			require.Equal(t, "gAAAA", reasoning["encrypted_content"])
