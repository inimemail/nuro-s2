...
func TestIsNonRetryableRefreshError(t *testing.T) {
	tests := []struct {
		...
		{name: "refresh_token_expired", err: errors.New(`OPENAI_OAUTH_TOKEN_REFRESH_FAILED: token refresh failed: status 401, body: {"error":{"code":"refresh_token_expired"}}`), expected: true},
