...
func (s *GeminiMessagesCompatService) markGeminiResponseSignal(c *gin.Context, account *Account, sig geminiResponseSignal, stream bool, upstreamRequestID string) {
	...
	if previous, ok := c.Get("gemini_response_signal_kind"); ok {
		if kind, ok := previous.(geminiResponseSignalKind); ok && kind > sig.Kind {
			return
		}
	}
	c.Set("gemini_response_signal_kind", sig.Kind)
	switch sig.Kind {
	case geminiSignalPromptBlocked, geminiSignalContentFilter:
		MarkOpsClientBusinessLimited(c, "upstream_content_policy")
	default:
		if reason, _ := c.Get(OpsClientBusinessLimitedReasonKey); reason == "upstream_content_policy" {
			c.Set(OpsClientBusinessLimitedKey, false)
			c.Set(OpsClientBusinessLimitedReasonKey, "")
		}
	}
	// The existing Ops context stores the upstream attempt separately. Keep the
	// in-band marker small and never copy provider payloads into the downstream
	// error or request context.
	MarkOpsStreamError(c, sig.Status, geminiSignalOpsErrorType(sig.Status), message)
	if event, ok := GetOpsStreamError(c); ok {
		event.Stream = &stream
		c.Set(OpsStreamErrorKey, event)
		c.Set("gemini_response_signal_event", event)
	}
}

// A later account attempt must not inherit an in-band outcome from a failed
// attempt. Clear only the marker owned by this observer, preserving other errors.
func resetGeminiResponseSignal(c *gin.Context) {
	if c == nil {
		return
	}
	if previous, ok := c.Get("gemini_response_signal_event"); ok {
		if event, ok := GetOpsStreamError(c); ok && previous == event {
			c.Set(OpsStreamErrorKey, nil)
		}
		c.Set("gemini_response_signal_event", nil)
		c.Set("gemini_response_signal_kind", geminiSignalNone)
		if reason, _ := c.Get(OpsClientBusinessLimitedReasonKey); reason == "upstream_content_policy" {
			c.Set(OpsClientBusinessLimitedKey, false)
			c.Set(OpsClientBusinessLimitedReasonKey, "")
		}
...
// Observe before validating/converting: an error envelope may be rejected by
// the existing safe-response guard and never reach the success path.
func (s *GeminiMessagesCompatService) observeGeminiPayload(c *gin.Context, body []byte, stream bool) {
	if sig, ok := detectGeminiResponseSignalInBody(body); ok {
		s.markGeminiResponseSignal(c, nil, sig, stream, "")
	} else if !stream && isGeminiEmptyResponseBody(body) {
		s.markGeminiEmptyResponse(c, nil, false, "")
	}
}

// Buffered clients still receive many upstream SSE chunks. An empty metadata
// chunk is not an empty response; only the collector's nil EOF signal means
// that the entire upstream stream lacked a response.
func (s *GeminiMessagesCompatService) observeGeminiCollectedPayload(c *gin.Context, body []byte) {
	if body == nil {
		s.markGeminiEmptyResponse(c, nil, false, "")
	} else if sig, ok := detectGeminiResponseSignalInBody(body); ok {
