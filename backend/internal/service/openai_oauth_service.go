...
// OpenAIDeviceAuthStartResult contains the browser URL and user code for
...
// StartDeviceAuth starts the Codex device-code authorization flow.
func (s *OpenAIOAuthService) StartDeviceAuth(ctx context.Context, proxyID *int64) (*OpenAIDeviceAuthStartResult, error) {
	proxyURL, err := s.resolveProxyURL(ctx, proxyID)
	if err != nil {
		return nil, err
	}

	deviceResp, err := s.oauthClient.StartDeviceAuth(ctx, proxyURL)
	if err != nil {
		return nil, err
	}
	if strings.TrimSpace(deviceResp.DeviceAuthID) == "" || strings.TrimSpace(deviceResp.UserCode) == "" {
		return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_DEVICE_AUTH_INVALID_RESPONSE", "device auth start response is incomplete")
	}

	sessionID, err := openai.GenerateSessionID()
	if err != nil {
		return nil, infraerrors.Newf(http.StatusInternalServerError, "OPENAI_DEVICE_AUTH_SESSION_FAILED", "failed to generate session ID: %v", err)
	}
	...
	session := &openai.OAuthSession{
		...
		DeviceUserCode:     deviceResp.UserCode,
		DevicePollInterval: deviceResp.Interval.Int64(),
		CreatedAt:          time.Now(),
	}
	s.sessionStore.Set(sessionID, session)
	...
	return &OpenAIDeviceAuthStartResult{
		...
		VerificationURL: firstNonEmptyString(deviceResp.VerificationURL, deviceResp.VerificationURI, openai.DeviceVerificationURL),
		ExpiresIn:       deviceResp.ExpiresIn.Int64(),
		Interval:        deviceResp.Interval.Int64(),
	}, nil
}

// ExchangeDeviceAuth completes the device-code authorization flow once the
// user approved the code in the browser.
func (s *OpenAIOAuthService) ExchangeDeviceAuth(ctx context.Context, sessionID string, proxyID *int64) (*OpenAITokenInfo, error) {
	session, ok := s.sessionStore.Get(sessionID)
	if !ok {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_DEVICE_AUTH_SESSION_NOT_FOUND", "device auth session not found or expired")
	}
	if strings.TrimSpace(session.DeviceAuthID) == "" {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_DEVICE_AUTH_SESSION_INVALID", "device auth session is invalid")
	}
	...
	deviceToken, err := s.oauthClient.PollDeviceAuth(ctx, session.DeviceAuthID, session.DeviceUserCode, proxyURL)
	if err != nil {
		return nil, err
	}
	...
	tokenInfo, err := s.exchangeCodeWithVerifier(ctx, deviceToken.Code, deviceToken.CodeVerifier, session.RedirectURI, proxyURL, clientID)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIOAuthService) ExchangeCode(ctx context.Context, input *OpenAIExchangeCodeInput) (*OpenAITokenInfo, error) {
	...
	tokenInfo, err := s.exchangeCodeWithVerifier(ctx, input.Code, session.CodeVerifier, redirectURI, proxyURL, clientID)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIOAuthService) exchangeCodeWithVerifier(ctx context.Context, code, codeVerifier, redirectURI, proxyURL, clientID string) (*OpenAITokenInfo, error) {
