...
func (s *AccountService) Create(ctx context.Context, req CreateAccountRequest) (*Account, error) {
	...
	if account.Type == AccountTypeAPIKey && len(req.GroupIDs) > 0 {
		for _, gid := range req.GroupIDs {
			...
			if g.RequireOAuthOnly && (g.Platform == PlatformOpenAI || g.Platform == PlatformAntigravity || g.Platform == PlatformAnthropic || g.Platform == PlatformGemini || g.Platform == PlatformGrok || g.Platform == PlatformKimi || g.Platform == PlatformZhipu || g.Platform == PlatformDeepSeek) {
