...
const AccountPoolModeNonPoolFilter = "non_pool"
const AccountPoolModePoolFilter = "pool"
const AccountPoolModeImagePoolFilter = "image_pool"
const AccountPoolModeTextPoolFilter = "text_pool"
...
type AccountRepository interface {
	...
	ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, poolMode string) ([]Account, *pagination.PaginationResult, error)
...
func (s *AccountService) Create(ctx context.Context, req CreateAccountRequest) (*Account, error) {
	...
	if account.Type == AccountTypeAPIKey && len(req.GroupIDs) > 0 {
		for _, gid := range req.GroupIDs {
			...
			if g.RequireOAuthOnly && (g.Platform == PlatformOpenAI || g.Platform == PlatformAntigravity || g.Platform == PlatformAnthropic || g.Platform == PlatformGemini || g.Platform == PlatformGrok || g.Platform == PlatformKimi || g.Platform == PlatformZhipu || g.Platform == PlatformDeepSeek || g.Platform == PlatformMiniMax) {
...
func (s *AccountService) TestCredentials(ctx context.Context, id int64) error {
	...
	case PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepSeek, PlatformMiniMax:
		// TODO: 测试 OpenAI-compatible provider 凭证
		return nil
	default:
