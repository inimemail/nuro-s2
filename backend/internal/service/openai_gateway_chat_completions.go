...
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	if account.IsOpenAIOAuthLike() {
		...
		// Chat Completions clients do not send a top-level Responses
		// instructions field. ChatGPT's Codex endpoint nevertheless requires the
		// field to exist (an empty string is valid), even when default Codex
		// instructions are intentionally disabled for this compatibility bridge.
		if !isResponsesShape {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		if !requestFirstTokenPlaceholder.UpstreamCommitted && !agentIdentityTaskRecoveryWasTried(ctx) && s.isAgentIdentityAccount(ctx, account) && isAgentIdentityTaskInvalidHTTPResponse(resp.StatusCode, respBody) {
			...
			return s.ForwardAsChatCompletions(markAgentIdentityTaskRecoveryTried(ctx), c, account, body, incomingPromptCacheKey, defaultMappedModel)
		}
...
func shouldForwardAPIKeyChatViaResponses(account *Account) bool {
	...
	if (account.IsDeepSeek() || account.IsKimi() || account.IsMiniMax()) && account.GetAPIProtocol() == APIProtocolResponses {
		return true
	}
