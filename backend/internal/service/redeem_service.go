...
func (s *RedeemService) reduceOrCancelSubscription(ctx context.Context, userID, groupID int64, reduceDays int, code string) error {
	...
	// Different redemption codes can target the same subscription in parallel.
	lockingRepo, ok := s.subscriptionService.userSubRepo.(userSubscriptionForUpdateRepository)
	if !ok {
		return fmt.Errorf("subscription repository does not support transactional reduction locking")
	}
	sub, err = lockingRepo.GetByIDForUpdate(ctx, sub.ID)
	if err != nil {
