...
func (c *stubGatewayCache) SetSessionAccountID(ctx context.Context, groupID int64, sessionHash string, accountID int64, ttl time.Duration) error {
	...
	if c.sessionTTLs == nil {
		c.sessionTTLs = make(map[string]time.Duration)
	}
	c.sessionTTLs[sessionHash] = ttl
	return nil
}
...
func TestOpenAISelectAccountWithLoadAwareness_StickyAccountIDWithoutSessionHash(t *testing.T) {
	...
	selection, err := svc.selectAccountWithLoadAwareness(context.Background(), &groupID, "", "gpt-5.2", nil, false, sticky.ID, "", "", PlatformOpenAI, -1)
	require.NoError(t, err)
	require.NotNil(t, selection)
...
func TestOpenAISelectAccountWithLoadAwareness_StickyMovedGroupClearsSession(t *testing.T) {
	sessionHash := "session-moved-group"
	groupID := int64(1)
	moved := Account{
		ID:          1,
		Platform:    PlatformOpenAI,
	...
	repo := stubOpenAIAccountRepo{accounts: []Account{moved, current}}
	cache := &stubGatewayCache{
		sessionBindings: map[string]int64{"openai:" + sessionHash: moved.ID},
	}
	...
	require.Equal(t, current.ID, cache.sessionBindings["openai:"+sessionHash])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}
...
func TestOpenAIStreamingOAuthPreambleFlushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestOpenAIStreamingAPIKeyPreambleFlushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-apikey-preamble-flush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIStreamingOAuthSSECommentPreflushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-oauth-comment-preflush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIStreamingAPIKeySSECommentPreflushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-apikey-comment-preflush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIRequestFirstTokenTimeoutPlaceholderUsesRequestDeadlineBeforeUpstreamResponse(t *testing.T) {
	setGinTestMode()
	for _, tt := range []struct {
		name        string
		accountType string
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.False(t, state.UpstreamCommitted)
			require.GreaterOrEqual(t, elapsed, 100*time.Millisecond)
			require.Contains(t, rec.Body.String(), `"type":"response.transport_progress.delta"`)
			require.True(t, OpenAIRequestPlaceholderWritten(c))
...
func TestOpenAIHTTPPlaceholderIngressBudgetDoesNotChangeServiceTTFT(t *testing.T) {
	setGinTestMode()
	streamBody := strings.Join([]string{
		`data: {"type":"response.output_text.delta","delta":"ok"}`,
		"",
		`data: {"type":"response.completed","response":{"id":"resp_ingress_budget","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			account := &Account{
				...
				Extra: map[string]any{
					openAIAPIKeyFirstTokenTimeoutPlaceholderEnabledExtraKey:      true,
					openAIAPIKeyFirstTokenTimeoutPlaceholderMsExtraKey:           5000,
					openAIAPIKeyFirstTokenTimeoutPlaceholderGuardEnabledExtraKey: false,
				},
			}

			serviceStart := time.Now()
			firstTokenMS, err := tt.forward(&OpenAIGatewayService{}, c, resp, account, serviceStart)
			serviceElapsedMS := int(time.Since(serviceStart).Milliseconds())
			require.NoError(t, err)
...
func TestOpenAIStreamRequiresGoPlaceholderCoordination(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Extra: map[string]any{}}
	require.False(t, svc.OpenAIStreamRequiresGoPlaceholderCoordination(account, "gpt-5.4"))

	account.Extra[openAIAPIKeySafeTokenPlaceholderExtraKey] = true
	require.True(t, svc.OpenAIStreamRequiresGoPlaceholderCoordination(account, "gpt-5.4"))
	...
	require.False(t, svc.OpenAIStreamRequiresGoPlaceholderCoordination(account, "gpt-image-1"))
}
...
func TestOpenAIStreamStructuralPreambleDoesNotCommitAttempt(t *testing.T) {
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.created"}`, "response.created"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.in_progress"}`, "response.in_progress"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.output_item.added","item":{"type":"message"}}`, "response.output_item.added"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.content_part.added"}`, "response.content_part.added"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.reasoning_summary_part.added"}`, "response.reasoning_summary_part.added"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.output_text.done","text":"ok"}`, "response.output_text.done"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.output_text.delta","delta":""}`, "response.output_text.delta"))
	require.False(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.transport_progress.delta","delta":"in_progress"}`, "response.transport_progress.delta"))
	require.True(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.output_item.added","item":{"type":"function_call"}}`, "response.output_item.added"))
	require.True(t, openAIStreamDataStartsCoordinatedClientOutput(`{"type":"response.output_text.delta","delta":"ok"}`, "response.output_text.delta"))
	require.True(t, openAIStreamDataStartsClientOutput(`{"type":"response.output_item.added","item":{"type":"message"}}`, "response.output_item.added"), "ordinary streams retain their legacy commitment semantics")
}
...
func TestOpenAIResponsesTimeoutPlaceholderSurvivesStructuralEvents(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Less(t, strings.Index(body, placeholder), strings.Index(body, `"type":"response.content_part.added"`))
			require.Less(t, strings.Index(body, `"type":"response.content_part.added"`), strings.Index(body, realDelta))
		})
	}
}
...
func TestOpenAIPlaceholderKeepsFailedAttemptPreamblePrivate(t *testing.T) {
	setGinTestMode()
	streamBody := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_failed_attempt"}}`,
		"",
		`data: {"type":"response.output_item.added","item":{"id":"msg_failed_attempt","type":"message"}}`,
		"",
	...
	for _, tt := range []struct {
		name    string
		forward func(*OpenAIGatewayService, *gin.Context, *http.Response, *Account) error
	}{
		{
			name: "native",
			forward: func(svc *OpenAIGatewayService, c *gin.Context, resp *http.Response, account *Account) error {
				_, err := svc.handleStreamingResponse(c.Request.Context(), resp, c, account, time.Now(), "gpt-5.4", "gpt-5.4")
				return err
			},
		},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			resp := &http.Response{
				...
				Header:     http.Header{"X-Request-Id": []string{"rid-failed-attempt"}},
			}
			account := &Account{ID: 1, Platform: PlatformOpenAI, Name: "acc"}

			err := tt.forward(&OpenAIGatewayService{}, c, resp, account)

			var failoverErr *UpstreamFailoverError
			require.ErrorAs(t, err, &failoverErr)
			require.NotContains(t, recorder.Body.String(), "resp_failed_attempt")
			require.NotContains(t, recorder.Body.String(), "msg_failed_attempt")
...
func TestOpenAIPlaceholderMaintainsOneResponsesStreamAcrossAccountFailover(t *testing.T) {
	setGinTestMode()
	failedStream := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_account_a"}}`,
		"",
		`data: {"type":"response.output_item.added","item":{"id":"msg_account_a","type":"message"}}`,
		"",
		`data: {"type":"response.failed","error":{"message":"upstream processing failed"}}`,
		"",
	}, "\n")
	successStream := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_account_b","model":"gpt-5.4"}}`,
		"",
		`data: {"type":"response.output_item.added","item":{"id":"msg_account_b","type":"message"}}`,
		"",
		`data: {"type":"response.content_part.added","part":{"type":"output_text","text":""}}`,
		"",
		`data: {"type":"response.output_text.delta","delta":"ok"}`,
		"",
		`data: {"type":"response.completed","response":{"id":"resp_account_b","model":"gpt-5.4","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}`,
		"",
	}, "\n")
	...
	for _, tt := range []struct {
	...
	}{
		{
			name: "native",
			forward: func(svc *OpenAIGatewayService, c *gin.Context, resp *http.Response, account *Account) error {
				_, err := svc.handleStreamingResponse(c.Request.Context(), resp, c, account, time.Now(), "gpt-5.4", "gpt-5.4", true)
				return err
			},
		},
		{
			name: "passthrough",
			forward: func(svc *OpenAIGatewayService, c *gin.Context, resp *http.Response, account *Account) error {
				_, err := svc.handleStreamingResponsePassthrough(c.Request.Context(), resp, c, account, time.Now(), "gpt-5.4", "gpt-5.4", true)
				return err
			},
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			firstErr := tt.forward(svc, c, &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(failedStream)),
				Header:     http.Header{"X-Request-Id": []string{"rid-account-a"}},
			}, &Account{ID: 1, Platform: PlatformOpenAI, Name: "account-a"})
			var failoverErr *UpstreamFailoverError
			require.ErrorAs(t, firstErr, &failoverErr)
			require.True(t, OpenAIRequestAllowsFailover(c, -1))

			secondErr := tt.forward(svc, c, &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(successStream)),
				Header:     http.Header{"X-Request-Id": []string{"rid-account-b"}},
			}, &Account{ID: 2, Platform: PlatformOpenAI, Name: "account-b"})
			require.NoError(t, secondErr)

			body := recorder.Body.String()
			require.Equal(t, 1, strings.Count(body, `"type":"response.transport_progress.delta"`))
			require.NotContains(t, body, "resp_account_a")
			require.NotContains(t, body, "msg_account_a")
			require.Contains(t, body, "resp_account_b")
			require.Contains(t, body, `"type":"response.output_text.delta","delta":"ok"`)
...
func TestOpenAIStreamingPassthroughOAuthPreambleFlushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-passthrough-preamble-flush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIStreamingPassthroughAPIKeyPreambleFlushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-passthrough-apikey-preamble-flush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIStreamingPassthroughOAuthSSECommentPreflushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-passthrough-oauth-comment-preflush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIStreamingPassthroughAPIKeySSECommentPreflushWritesBeforeFailedEvent(t *testing.T) {
	setGinTestMode()
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	resp := &http.Response{
		...
		Header: http.Header{"X-Request-Id": []string{"rid-passthrough-apikey-comment-preflush"}},
	}
	account := &Account{
		ID:       1,
		Platform: PlatformOpenAI,
...
func TestOpenAIBuildUpstreamRequestOpenAIPassthroughPreservesExplicitAPIKeyBetaHeader(t *testing.T) {
	...
	req, err := svc.buildUpstreamRequestOpenAIPassthrough(c.Request.Context(), c, account, []byte(`{"model":"gpt-5"}`), "token")
	require.NoError(t, err)
	require.Equal(t, "api-key-specific-beta", req.Header.Get("OpenAI-Beta"), "OAuth-only cleanup must not alter API-key passthrough headers")
}
...
func TestOpenAIBuildUpstreamRequestOAuthOfficialClientOriginatorCompatibility(t *testing.T) {
	// This table covers the legacy pairing escape hatch. Canonical enforcement
	// is the process default and is exercised by the Edge/OAuth plan tests.
...
func TestHandleSSEToJSON_NoTerminalResponseReturnsProtocolError(t *testing.T) {
	...
	result, err := svc.handleSSEToJSON(context.Background(), resp, c, &Account{ID: 1, Platform: PlatformOpenAI}, body, "gpt-4o", "gpt-4o")
	require.Error(t, err)
