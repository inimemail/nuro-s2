...
// contains only bounded reason counters; account IDs, platform names and
// multiplier values never leave the scheduler.
...
func (s openAISelectionFilterStats) summary(extra string) string {
	...
	for reason, count := range s.reasons {
		parts = append(parts, reason+"="+strconv.Itoa(count))
	}
	sort.Strings(parts)
	result := "pool=" + strconv.Itoa(s.pool)
	if len(parts) > 0 {
...
func logOpenAISelectionFilterStats(stats openAISelectionFilterStats, extra string) {
	slog.Warn("openai scheduler selection exhausted", "summary", stats.summary(extra))
}
...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if previousResponseID != "" {
		// previous_response_id is a continuity binding, including under adaptive
		// strategies. Moving it solely for cost/TTFT would break the upstream
		// response chain and cause recovery reconnects. The lookup below still
		// enforces schedulability, group, capability, priority, persisted/runtime
		// cooldowns and concurrency before returning the bound account.
		...
		if selection != nil && selection.Account != nil && s.service.isOpenAIProxyStreamQuarantined(selection.Account) {
			if selection.ReleaseFunc != nil {
				selection.ReleaseFunc()
			}
			selection = nil
		...
		if selection != nil && selection.Account != nil && !IsAdaptiveHealthSchedulingStrategy(req.AccountSchedulingStrategy) {
		...
		if selection != nil && selection.Account != nil && s.service.shouldSkipNonOpenAIPoolAccount(ctx, selection.Account) {
			if s.service.nonOpenAIPoolRuntime != nil {
				s.service.nonOpenAIPoolRuntime.releaseProbe(selection.Account)
			}
			if selection.ReleaseFunc != nil {
				selection.ReleaseFunc()
			}
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if account.IsUpstreamBillingGuardBlockedForGroup(req.GroupID) {
		// A group-scoped multiplier guard is reversible. Keep the existing
		// affinity binding while allowing this request to try another account.
		return nil, true, nil
	}
	if s.service.isOpenAIProxyStreamQuarantined(account) {
		// Proxy quarantine is process-local and short lived; replacing the
		// sticky key would make recovery lose the original cache affinity.
		return nil, true, nil
	}
	...
	if !isOpenAICompatibleRequestPlatformAccount(account, req.RequestPlatform) {
		if sessionHash != "" {
			_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		}
		return nil, false, nil
	...
	if compatible, reason := s.isAccountRequestCompatibleReason(ctx, account, req); !compatible {
		if openAIStickyCompatibilityFailureIsTemporary(reason) {
			return nil, true, nil
		}
		if sessionHash != "" {
			_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
	...
	if account != nil && account.IsUpstreamBillingGuardBlockedForGroup(req.GroupID) {
		return nil, true, nil
	}
...
func openAIStickyCompatibilityFailureIsTemporary(reason string) bool {
	switch reason {
	case "runtime_blocked", "proxy_stream_quarantined", "shadow_parent_unhealthy":
		return true
	default:
		return strings.HasPrefix(reason, "quota_auto_pause")
	}
}
...
func openAIStickyAccountTemporarilyUnavailable(ctx context.Context, s *OpenAIGatewayService, account *Account, groupID *int64) bool {
	...
	if s != nil && (s.isOpenAIProxyStreamQuarantined(account) ||
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	return s.buildOpenAIAccountLoadPlanWithHistory(req, filtered, loadMap, nil)
}

func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlanWithHistory(
	req OpenAIAccountScheduleRequest,
	filtered []*Account,
	loadMap map[int64]*AccountLoadInfo,
	...
	for _, account := range filtered {
		...
		ttftP90 := 0.0
		if IsAdaptiveHealthSchedulingStrategy(req.AccountSchedulingStrategy) {
			if summary, ok := validAccountTTFTHistory(history[account.ID]); ok {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		if acquireErr != nil {
			if s.service.nonOpenAIPoolRuntime != nil {
				s.service.nonOpenAIPoolRuntime.releaseProbe(fresh)
			}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if s.service.isOpenAIProxyStreamQuarantined(account) {
			filterStats.exclude("proxy_stream_quarantined")
			continue
		}
		...
		if !openAIAccountMatchesLockedPriority(account, req.LockedPriority) {
			filterStats.exclude("locked_priority")
			continue
		}
		...
		if paused, _ := shouldAutoPauseOpenAIAccountByQuota(ctx, account); paused {
			filterStats.exclude("quota_auto_pause")
			continue
		}
		if !s.isAccountTransportCompatible(account, req.RequiredTransport) {
	...
	plan := s.buildOpenAIAccountLoadPlanWithHistory(req, filtered, loadMap, history)
	...
	for _, candidate := range selectionOrder {
		...
		if IsAdaptiveHealthSchedulingStrategy(req.AccountSchedulingStrategy) && s.service.concurrencyService != nil {
			if waiting, waitErr := s.service.concurrencyService.GetAccountWaitingCount(ctx, fresh.ID); waitErr == nil && waiting >= cfg.FallbackMaxWaiting {
				continue
			}
			if _, primary := adaptivePrimaryTier[fresh.ID]; !primary {
				upgraded, upgradedCompactBlocked, upgradedErr := s.tryAcquireOpenAISelectionOrder(ctx, req, []openAIAccountCandidateScore{candidate})
				if upgradedErr != nil {
					return nil, candidateCount, topK, loadSkew, upgradedErr
				}
				compactBlocked = compactBlocked || upgradedCompactBlocked
				if upgraded != nil {
					return upgraded, candidateCount, topK, loadSkew, nil
				}
			}
		}
		return &AccountSelectionResult{
...
func (s *OpenAIGatewayService) selectRequiredOpenAIAccount(
	...
	account = s.recheckSelectedOpenAIAccountForGroup(
		ctx,
		account,
		groupID,
		requestedModel,
...
// Canonical 5h/7d fields take precedence. For legacy primary/secondary data,
// use the provider-advertised window lengths to normalize the labels instead of
// assuming primary is always the long window.
