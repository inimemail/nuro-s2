...
func TestOpenAIGatewayService_OAuthPassthrough_StreamKeepsToolNameAndBodyNormalized(t *testing.T) {
	...
	originalBody := []byte(`{"model":"gpt-5.2","stream":true,"store":true,"instructions":"local-test-instructions","input":[{"type":"text","text":"hi","namespace":"keep-input-namespace"}],"tools":[{"type":"namespace","name":"collaboration","description":"collaboration tools","tools":[{"type":"function","name":"invite","description":"invite user","parameters":{"type":"object"}}]}]}`)
	...
	require.Equal(t, "keep-input-namespace", gjson.GetBytes(upstream.lastBody, "input.0.namespace").String())
	...
	require.Equal(t, currentCodexIdentityRuntime().canonicalUserAgent, upstream.lastReq.Header.Get("User-Agent"))
...
func TestOpenAIGatewayService_OAuthPassthrough_CodexMissingInstructionsAddsDefault(t *testing.T) {
	setGinTestMode()
	...
	// Codex 模型且缺少 instructions，应补充兼容默认值后继续转发。
...
func TestOpenAIGatewayService_OAuthLegacy_CompositeCodexUAUsesCodexOriginator(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	// 浏览器型复合 UA 被替换为默认 Codex UA，originator 随最终 UA 配套。
...
func TestOpenAIGatewayService_APIKeyPassthrough_CompatDisabledPreservesStringInputAndTokenParams(t *testing.T) {
	setGinTestMode()
	...
	originalBody := []byte(`{"model":"gpt-5.2","stream":false,"max_output_tokens":128,"input":"hi"}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
		Body:       io.NopCloser(strings.NewReader(`{"output":[],"usage":{"input_tokens":1,"output_tokens":1}}`)),
...
func TestOpenAIGatewayService_APIKeyPassthrough_CompatEnabledPreservesNonResponsesBody(t *testing.T) {
	setGinTestMode()
...
func TestOpenAIGatewayService_APIKeyPassthrough_NormalizesStringInputForResponsesEndpoint(t *testing.T) {
	setGinTestMode()
