...
func isAgentIdentityTaskInvalidWSDialResult(statusCode int, err error) bool {
	var dialErr *openAIWSDialError
	if errors.As(err, &dialErr) && isAgentIdentityTaskInvalidWSDialError(dialErr) {
		return true
	}
	var handshakeErr *openAIWSHandshakeError
	return errors.As(err, &handshakeErr) && isAgentIdentityTaskInvalidHTTPResponse(statusCode, handshakeErr.Body)
}
