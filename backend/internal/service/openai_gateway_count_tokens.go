...
// request locally. Grok has no compatible counting endpoint, so this path must
// not select an account, acquire a slot, read credentials, or call upstream.
...
func (s *OpenAIGatewayService) buildInputTokensUpstreamRequest(
	...
	if account.Type == AccountTypeAPIKey {
		if baseURL := account.GetOpenAIBaseURL(); strings.TrimSpace(baseURL) != "" {
			...
			targetURL = buildOpenAIResponsesURL(validatedURL) + "/input_tokens"
		}
	}
