...
func TestImageStorageSettingsOwnCredentialsAreEncryptedAndMasked(t *testing.T) {
	...
	require.Contains(t, raw, `"secret_encryption_version":1`)
...
func TestImageStorageSettingsConnectionTestReusesVersionedSecret(t *testing.T) {
...
func TestImageStorageSettingsVersionedCiphertextFailsClosed(t *testing.T) {
	svc, repo, built := newImageStorageFixture(t, config.ImageStorageConfig{})
	raw, err := json.Marshal(ImageStorageSettings{
		Enabled: true, Bucket: "images", Endpoint: "https://storage.example.com",
		AccessKeyID: "ak", SecretAccessKey: "broken", SecretEncryptionVersion: storedSecretEncryptionVersion,
	})
	require.NoError(t, err)
...
func TestImageStorageSettingsBase64LookingLegacyPlaintextRemainsCompatible(t *testing.T) {
	svc, repo, built := newImageStorageFixture(t, config.ImageStorageConfig{})
	const legacySecret = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmn"
	raw, err := json.Marshal(ImageStorageSettings{
		Enabled: true, Bucket: "images", Endpoint: "https://storage.example.com",
		AccessKeyID: "ak", SecretAccessKey: legacySecret,
	})
	require.NoError(t, err)
	require.NoError(t, repo.Set(context.Background(), settingKeyImageStorageConfig, string(raw)))
	...
	require.True(t, enabled)
	require.Len(t, *built, 1)
	require.Equal(t, legacySecret, (*built)[0].SecretAccessKey)
}
...
func TestImageStorageSettingsRetriesTransientFactoryFailure(t *testing.T) {
	repo := newStubSettingRepo()
	encryptor := reversibleEncryptor{}
...
func TestImageStorageSettingsRetriesTransientRepositoryFailure(t *testing.T) {
	svc, repo, built := newImageStorageFixture(t, config.ImageStorageConfig{
		Enabled: true, Bucket: "fallback-images", Endpoint: "https://storage.example.com",
		AccessKeyID: "ak", SecretAccessKey: "sk",
	})
	repo.nextValueErr = errors.New("temporary database failure")
...
func TestImageStorageSettingsInvalidatesWhenBackupS3Rotates(t *testing.T) {
	svc, repo, built := newImageStorageFixture(t, config.ImageStorageConfig{})
	ctx := context.Background()
	seedBackupS3(t, repo, BackupS3Config{
		Endpoint: "https://old.example.com", Region: "auto", Bucket: "images",
	...
	_, err = svc.backup.UpdateS3Config(ctx, BackupS3Config{
		Endpoint: "https://new.example.com", Region: "auto", Bucket: "images",
...
func TestImageStorageSettingsFallBackToConfigFile(t *testing.T) {
	svc, _, built := newImageStorageFixture(t, config.ImageStorageConfig{
		...
		Prefix: "images/", MaxImageBytes: 1024,
	})
