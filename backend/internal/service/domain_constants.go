...
const (
	...
	SettingKeyRegistrationEmailDomainQuotaEnabled = "registration_email_domain_quota_enabled" // 非白名单主域名单账户配额，默认关闭
	SettingKeyOpenAICodexRoutingHintEnabled       = "openai_codex_routing_hint_enabled"
	...
	SettingKeyPromptAuditEnabled                  = "prompt_audit_enabled"             // 是否启用 Prompt 审计入口与异步采集链路
	...
	SettingKeyCompactHomeEnabled          = "compact_home_enabled"          // 使用精简首页预设
	...
	SettingKeyAnthropicKiroKnowledgePack  = "anthropic_kiro_knowledge_pack" // Kiro 保护知识包（JSON，可由后台任务自动刷新）
	...
	// 保留该 key 仅为向后兼容，开关不再产生任何效果。
	SettingKeyEnableCCHSigning = "enable_cch_signing"
	// SettingKeyEnableClaudeOAuthSystemPromptInjection 是否对 Claude OAuth mimic 路径注入可配置 Claude Code system blocks（默认 false）
	...
	// SettingKeyEnableClientDatelineNormalization 是否清洗 Anthropic 客户端 dateline 指纹（默认 true）
	...
	SettingKeyGatewayEdgeLocalDataPlaneEnabled = "gateway_edge_local_data_plane_enabled"
	...
	// SettingKeyCodexCLIOnlyAllowAppServerClients 是否允许 app-server 客户端策略扩展。
	SettingKeyCodexCLIOnlyAllowAppServerClients = "codex_cli_only_allow_app_server_clients"
	// SettingKeyCodexCLIOnlyEngineFingerprintSignals codex_cli_only 引擎指纹信号 JSON。
