...
func TestForwardAsChatCompletions_UnknownResponsesSupportUsesRawChatURL(t *testing.T) {
	setGinTestMode()
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_raw_unknown"}},
...
func TestBufferRawChatCompletions_RejectsOversizedResponse(t *testing.T) {
	...
	result, err := svc.bufferRawChatCompletions(context.Background(), c, resp, nil, "gpt-5.4", "gpt-5.4", "gpt-5.4", nil, nil, time.Now())
