...
// Keep the pre-first-token buffer bounded. An upstream that emits an
// unbounded metadata prelude must fail closed instead of growing one slice per
// event while the downstream is still waiting for its first token.
const (
	antigravityCompatMaxPendingEvents = 1024
	antigravityCompatMaxPendingBytes  = 16 << 20
)
...
func (s *antigravityCompatStreamSession) pendingBufferOverflowed() bool {
	return s != nil && s.pendingOverflow
}
...
func (s *antigravityCompatStreamSession) result(clientDisconnect bool) *antigravityStreamResult {
	return &antigravityStreamResult{
		...
		localCapacityLimited: s.localCapacity,
	}
}
...
func (s *antigravityCompatStreamSession) consumeClaudeData(eventType, payload string) {
	...
	s.emitOrBuffer(event, len(payload))
}

func (s *antigravityCompatStreamSession) emitOrBuffer(event apicompat.AnthropicStreamEvent, payloadBytes int) {
	if s.meaningfulData {
	...
	if len(s.pendingEvents) >= antigravityCompatMaxPendingEvents ||
		s.pendingBytes > antigravityCompatMaxPendingBytes-payloadBytes {
		s.pendingOverflow = true
		s.localCapacity = true
		return
	}
	s.pendingEvents = append(s.pendingEvents, event)
	s.pendingBytes += payloadBytes
...
func (s *AntigravityGatewayService) handleAntigravityCompatStream(
	...
	for {
		...
		case event, open := <-events:
			if !open {
				if antigravityCompatClientDisconnected(c, writer) {
			...
			if session.pendingBufferOverflowed() {
				logger.LegacyPrintf("service.antigravity_gateway", "Compatibility stream prelude exceeded bounded event buffer (%s)", prefix)
				writeAntigravityCompatStreamError(c, adapter, writer, "response_too_large")
				return session.result(false), fmt.Errorf("compatibility stream prelude exceeded bounded event buffer")
			}
...
func (s *AntigravityGatewayService) handleAntigravityCompatReadError(
	...
	clientDisconnected := antigravityCompatClientDisconnected(c, session.writer)
	...
	if disconnect, handled := handleStreamReadError(err, clientDisconnected, prefix); handled {
...
func antigravityCompatClientDisconnected(c *gin.Context, writer *antigravityClientWriter) bool {
	if writer != nil && writer.Disconnected() {
		return true
	}
