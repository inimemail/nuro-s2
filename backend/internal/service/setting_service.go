...
func parseForwardedClientIPHeadersSetting(value string) ([]string, error) {
	...
	if headers == nil {
		return nil, errors.New("parse forwarded_client_ip_headers: value must be a JSON array")
	}
...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		PasskeyEnabled:                   s.passkeyConfigured() && settings[SettingKeyPasskeyEnabled] == "true",
...
// directly from the settings store. Fail-closed: on error returns false.
...
// OmittedSettingKeys marks settings that were absent from a partial payload.
// A nil set preserves the existing whole-document update behavior.
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	settings.ModelPlazaDescription = strings.TrimSpace(settings.ModelPlazaDescription)
	if utf8.RuneCountInString(settings.ModelPlazaDescription) > ModelPlazaDescriptionMaxLength {
		return nil, infraerrors.BadRequest("MODEL_PLAZA_DESCRIPTION_TOO_LONG", "Model plaza description is too long (max 4000 characters)")
	}
...
func (s *SettingService) getGatewayForwardingSettingsCached(ctx context.Context) gatewayForwardingSettingsResult {
	...
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		if v, ok := values[SettingKeyOpenAIPoolDownstreamModelLimitProtectionEnabled]; ok && v != "" {
			openAIPoolDownstreamModelLimitProtection = v == "true"
		}
		openAIPoolRecoveryProbe := true
...
func (s *SettingService) IsOpenAIPoolDownstreamModelLimitProtectionEnabled(ctx context.Context) bool {
	return s == nil || s.getGatewayForwardingSettingsCached(ctx).openAIPoolDownstreamModelLimitProtection
}
...
func (s *SettingService) passkeyConfigured() bool {
	return s != nil && s.cfg != nil && s.cfg.WebAuthn.Enabled &&
		strings.TrimSpace(s.cfg.WebAuthn.RPID) != "" && len(s.cfg.WebAuthn.RPOrigins) > 0
}

// PasskeyEnabled requires both the immutable deployment boundary and the
// database-backed runtime switch. Store failures fail closed.
func (s *SettingService) PasskeyEnabled(ctx context.Context) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("get passkey setting: %w", err)
	}
	return value == "true", nil
}

func (s *SettingService) PasskeyConfiguration() (bool, string, []string) {
	if !s.passkeyConfigured() {
		return false, "", []string{}
	}
	return true, s.cfg.WebAuthn.RPID, append([]string(nil), s.cfg.WebAuthn.RPOrigins...)
}
...
func (s *SettingService) SetOpenAIFastPolicySettings(ctx context.Context, settings *OpenAIFastPolicySettings) error {
	...
	validTiers := map[string]bool{
		OpenAIFastTierAny: true, OpenAIFastTierPriority: true, OpenAIFastTierUltrafast: true, OpenAIFastTierFlex: true, OpenAIFastTierMissing: true,
	}
...
func (s *SettingService) GetDefaultPlatformQuotas(ctx context.Context) (map[string]*DefaultPlatformQuotaSetting, error) {
	out := newEmptyDefaultPlatformQuotaMap()
	...
	return out, nil // 补齐所有允许 platform key，保持与旧实现一致的下游契约
}

func newEmptyDefaultPlatformQuotaMap() map[string]*DefaultPlatformQuotaSetting {
