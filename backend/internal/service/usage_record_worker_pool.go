...
const (
	...
	// 默认 sync：溢出时提交方内联执行，保证计费任务不被静默丢弃。
...
const (
	...
	// Stopped is distinct from an intentional overflow drop: callers may need
	// to execute billing-critical work synchronously during process shutdown.
