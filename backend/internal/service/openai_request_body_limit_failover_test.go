...
type bodyLimitCloseTrackingReadCloser struct {
	*strings.Reader
	closed bool
}

func (r *bodyLimitCloseTrackingReadCloser) Close() error {
	r.closed = true
	return nil
}
...
func TestOpenAIRequestBodyLimitFailoverHTTP413SwitchesAccountsBeforeWrite(t *testing.T) {
	...
	for _, passthrough := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			body := &bodyLimitCloseTrackingReadCloser{Reader: strings.NewReader(upstreamBody)}
			...
			account := &Account{
				ID: 161, Name: name, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1,
				Credentials: map[string]any{
					"api_key": "sk-test", "base_url": "https://api.example.test", "pool_mode": true,
					"pool_mode_retry_status_codes": []any{float64(http.StatusRequestEntityTooLarge)},
				},
			...
			require.Equal(t, openAIRequestBodyTooLargeReason, failoverErr.Reason)
...
func TestOpenAIRequestBodyLimitFailoverContextWindow413DoesNotSwitchAccounts(t *testing.T) {
	...
	for _, passthrough := range []bool{false, true} {
		t.Run(fmt.Sprintf("passthrough_%t", passthrough), func(t *testing.T) {
			...
			require.NotContains(t, rec.Body.String(), "context window")
			require.True(t, body.closed)
		})
	}
}
