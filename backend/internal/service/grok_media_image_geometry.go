...
var grokImagineAspectRatioValues = []struct {
	...
	{"4:3", 4.0 / 3.0}, {"3:4", 3.0 / 4.0}, {"3:2", 1.5},
	{"2:3", 2.0 / 3.0}, {"2:1", 2}, {"1:2", 0.5},
	{"19.5:9", 19.5 / 9.0}, {"9:19.5", 9.0 / 19.5},
...
func applyGrokImagineImageGeometry(body []byte) ([]byte, error) {
	...
	if resolution == "" {
		if derived := grokImagineImageResolutionFromSize(size); derived != "" {
			var err error
			out, err = sjson.SetBytes(out, "resolution", derived)
			if err != nil {
				return nil, err
			}
		}
...
func assignGrokMediaResolution(value string, info *GrokMediaRequestInfo) {
	if info == nil || strings.TrimSpace(value) == "" {
		return
	}
	...
	info.Resolution = strings.TrimSpace(value)
}
...
func grokImagineAspectRatioFromSize(size string) string {
	...
	divisor := grokImagineGCD(width, height)
	exact := strconv.Itoa(width/divisor) + ":" + strconv.Itoa(height/divisor)
