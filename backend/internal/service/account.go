...
type Account struct {
	...
	OpenAIPoolSoftCooldownProbeModel string
	OpenAIPoolSoftCooldownProbeKind  string
	OpenAIPoolSoftCooldownSource     string
	OpenAIPoolLastProbeStatusCode    int
...
const (
	defaultPoolSoftCooldownErrorThreshold = 3
...
// forwarding protocol. Setup tokens share the protocol but have no refresh
// lifecycle.
func (a *Account) IsOpenAIOAuthLike() bool {
	return a != nil && a.IsOpenAI() && (a.Type == AccountTypeOAuth || a.Type == AccountTypeSetupToken)
}

// UsesOpenAICodexProtocol reports credentials routed through ChatGPT/Codex.
func (a *Account) UsesOpenAICodexProtocol() bool {
	return a != nil && a.IsOpenAIOAuthLike()
}
...
// GetGrokMediaBaseURL keeps OAuth text traffic on the CLI proxy while sending
// media requests to the official Imagine API, which supports large image
// inputs. API-key accounts keep their configured public HTTPS endpoint.
...
// IsOpenAIOAuthChatGPTPreambleFlushEnabled 返回 OAuth/ChatGPT 账号是否提前透传 Responses 前置事件。
// 字段：accounts.extra.openai_oauth_chatgpt_preamble_flush_enabled。
func (a *Account) IsOpenAIOAuthChatGPTPreambleFlushEnabled() bool {
	if a == nil || !a.IsOpenAIOAuth() || a.Extra == nil {
		return false
	}
...
func normalizeOpenAIFirstTokenTimeoutPlaceholderMs(ms int) int {
	...
	if ms < openAIFirstTokenTimeoutPlaceholderMinMs {
		return openAIFirstTokenTimeoutPlaceholderMinMs
	}
