...
type SystemSettings struct {
	...
	EnableCCHSigning                       bool   // 已废弃 no-op：新版 CLI 取消 cch 后网关不再注入/签名 cch
	EnableClaudeOAuthSystemPromptInjection bool   // 是否对 Claude OAuth mimic 路径注入可配置 Claude Code system blocks（默认 false）
...
func defaultContext1MModelWhitelist() []string {
	return []string{
		"claude-sonnet-5",
		"claude-sonnet-5-*",
		"claude-sonnet-5@*",
