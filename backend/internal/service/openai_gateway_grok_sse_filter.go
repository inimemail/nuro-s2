...
// Vendor Grok gateways may inject billing/keepalive ping frames. Responses
// clients treat the closed event enum strictly, so rewrite only those frames
// into an SSE comment while preserving all other framing byte-for-byte.
...
func filterGrokResponsesBillingPings(source io.Reader, destination *io.PipeWriter, closeSource func() error, maxLineSize int) {
	...
	for scanner.Scan() {
		...
		if len(pingFrame) > 0 {
			...
			if canExtendGrokResponsesPingFrame(line) && len(pingFrame) < grokResponsesPingFrameMaxLines && pingFrameBytes+len(line) <= grokResponsesPingFrameMaxBytes {
