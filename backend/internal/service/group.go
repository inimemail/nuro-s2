...
type Group struct {
	...
	// UpstreamBillingGuardMaxMultiplier is configured for supported API-key
	...
	RPMLimit int
	// Empty values leave request reasoning untouched.
...
// 未启用、配置非法或非高峰时段均返回 1.0。区间为左闭右开 [PeakStart, PeakEnd)，不支持跨天。
...
// NormalizePeakRateConfig 归一化最终落库的高峰配置。
...
// computePeakAwareMultipliers 把基础 token 倍率拆分为最终 token 倍率、图片倍率与视频倍率。
func computePeakAwareMultipliers(apiKey *APIKey, base float64, now time.Time) (text, image, video float64) {
