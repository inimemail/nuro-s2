...
func TestForwardGrokVideoMutationDoesNotCommitBeforeBinding(t *testing.T) {
	upstream := &grokMediaContentUpstreamStub{
		response: &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"request_id":"task-1"}`)),
		},
	}
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
	c, recorder := grokMediaContentTestContext(http.MethodPost, "https://api.example/v1/videos", nil)
	bindCalled := false

	result, err := svc.forwardGrokMedia(
		context.Background(), c, grokMediaContentTestAccount(),
		GrokMediaEndpointVideosGenerations, "", []byte(`{"model":"grok-imagine-video"}`), "application/json",
		func(responseID string) error {
			bindCalled = true
			require.Equal(t, "task-1", responseID)
...
func TestForwardGrokMediaContentSanitizesUnsafeRepresentationHeaders(t *testing.T) {
	upstream := &grokMediaContentUpstreamStub{
		responses: []*http.Response{grokMediaContentStatusResponse(`{"status":"completed"}`), {
			StatusCode: http.StatusOK,
			Header: http.Header{
				"Content-Type":        []string{"text/html; charset=utf-8"},
				"Content-Disposition": []string{`inline; filename="xai-internal-report.html"`},
			},
	...
	require.Equal(t, `attachment; filename="video.bin"`, recorder.Header().Get("Content-Disposition"))
	require.Equal(t, "nosniff", recorder.Header().Get("X-Content-Type-Options"))
	require.NotContains(t, recorder.Header().Get("Content-Disposition"), "xai")
}
