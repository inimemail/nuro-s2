...
func TestIsAgentIdentityTaskInvalidWSDialResultRequiresExplicitMarker(t *testing.T) {
	explicit := &openAIWSHandshakeError{Body: []byte(`{"error":{"code":"invalid_task_id"}}`), Err: errors.New("handshake failed")}
	require.True(t, isAgentIdentityTaskInvalidWSDialResult(http.StatusUnauthorized, explicit))
	generic := &openAIWSHandshakeError{Body: []byte(`{"error":{"message":"unauthorized"}}`), Err: errors.New("handshake failed")}
	require.False(t, isAgentIdentityTaskInvalidWSDialResult(http.StatusUnauthorized, generic))
	require.False(t, isAgentIdentityTaskInvalidWSDialResult(http.StatusForbidden, explicit))
}
