...
type authCacheInvalidationOutboxRepository struct{ db *sql.DB }
...
func (r *authCacheInvalidationOutboxRepository) ScheduleSecondPass(ctx context.Context, id int64, workerID string, availableAt time.Time) error {
	result, err := r.db.ExecContext(ctx, `
		...
		SET delivery_stage = 1, available_at = $3, last_error = NULL,
	...
	return requireOutboxAffected(result, "schedule second pass", id)
}

func (r *authCacheInvalidationOutboxRepository) DeleteClaimed(ctx context.Context, id int64, workerID string) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM auth_cache_invalidation_outbox WHERE id = $1 AND claimed_by = $2`, id, workerID)
	if err != nil {
		return err
	}
	return requireOutboxAffected(result, "delete claim", id)
}
...
func (r *authCacheInvalidationOutboxRepository) RetryClaimed(ctx context.Context, id int64, workerID string, availableAt time.Time, lastError string) error {
	...
	return requireOutboxAffected(result, "retry claim", id)
}

func (r *authCacheInvalidationOutboxRepository) Stats(ctx context.Context) (service.AuthCacheInvalidationOutboxStats, error) {
	var stats service.AuthCacheInvalidationOutboxStats
	var oldest sql.NullTime
	var lastError sql.NullString
	err := r.db.QueryRowContext(ctx, `
		SELECT COUNT(*), MIN(created_at), COALESCE(MAX(attempts), 0),
		       (SELECT last_error FROM auth_cache_invalidation_outbox
		        WHERE last_error IS NOT NULL ORDER BY available_at DESC, id DESC LIMIT 1)
		FROM auth_cache_invalidation_outbox
...
func requireOutboxAffected(result sql.Result, operation string, id int64) error {
	affected, err := result.RowsAffected()
	if err != nil {
		return err
	}
	if affected != 1 {
		return fmt.Errorf("auth cache invalidation %s %d lost ownership", operation, id)
	}
	return nil
}
