...
INSERT INTO batch_image_jobs (
    batch_id, session_id, user_id, api_key_id, account_id, provider, model, task_name, parent_batch_id, status,
...
RETURNING `+batchImageJobColumns,
		params.BatchID, params.SessionID, params.UserID, params.APIKeyID, params.AccountID, params.Provider, params.Model, params.TaskName, params.ParentBatchID, params.Status,
...
id, batch_id, session_id, user_id, api_key_id, account_id, provider, model, task_name, parent_batch_id, status,
provider_job_name, provider_input_ref, provider_output_ref, gcs_input_uri, gcs_output_uri,
item_count, success_count, fail_count, cancelled_count,
estimated_cost, hold_amount, actual_cost,
base_unit_price, group_rate_multiplier, account_rate_multiplier,
...
func scanBatchImageJob(row rowScanner) (*service.BatchImageJob, error) {
	...
	err := row.Scan(
		&job.ID, &job.BatchID, &sessionID, &job.UserID, &apiKeyID, &accountID, &job.Provider, &job.Model, &job.TaskName, &parentBatchID, &job.Status,
