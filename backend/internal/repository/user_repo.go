...
func (r *userRepository) create(ctx context.Context, userIn *service.User, guardEmailAlias bool, domainLimit string) error {
	...
	// Reuse an outer transaction before attempting to open one on the root
	// client. Context, not the root client's driver, carries cross-layer ent
	// transactions (for example OAuth signup plus invitation redemption).
	var tx *dbent.Tx
	var txClient *dbent.Client
	txCtx := ctx
	if existingTx := dbent.TxFromContext(ctx); existingTx != nil {
		txClient = existingTx.Client()
	} else if startedTx, err := r.client.Tx(ctx); err == nil {
		tx = startedTx
		defer func() { _ = tx.Rollback() }()
		txClient = tx.Client()
		txCtx = dbent.NewTxContext(ctx, tx)
	} else if errors.Is(err, dbent.ErrTxStarted) {
		// A repository built directly from tx.Client() is already transactional
		// even when the caller did not attach the Tx to its context.
		txClient = r.client
	} else {
		return err
	}
	...
	if guardEmailAlias {
		exists, err := existsByEmailAliasWithClient(txCtx, txClient, userIn.Email)
		if err != nil {
			return err
		}
		if exists {
...
func (r *userRepository) Update(ctx context.Context, userIn *service.User, fields service.UserUpdateFields) error {
	...
	// Keep user and allowed_groups updates in the caller's transaction when
...
// Refund processing must never deepen an existing deficit or race a concurrent
// billing deduction into an overdraft.
...
// DeductAvailableBalanceExact atomically deducts the requested amount only
// when the user's current non-negative balance can cover it. A zero result is
// an insufficient balance (or a concurrently removed user), and no balance
// is changed. Refunds use this variant when force is not explicitly enabled.
func (r *userRepository) DeductAvailableBalanceExact(ctx context.Context, id int64, amount float64) (deducted float64, err error) {
	if amount < 0 {
		return 0, fmt.Errorf("deduction amount must be nonnegative")
	}
	const updateSQL = `
...
// AdjustBalance is reserved for administrative balance edits. Request billing
// continues to use DeductBalance and therefore keeps the existing overdraft
// semantics.
func (r *userRepository) AdjustBalance(ctx context.Context, id int64, delta float64) (service.BalanceChange, error) {
	const query = `UPDATE users SET balance = balance + $1, updated_at = NOW()
		WHERE id = $2 AND deleted_at IS NULL AND balance + $1 >= 0
		RETURNING balance - $1, balance`
	change, ok, err := scanBalanceChange(ctx, clientFromContext(ctx, r.client), query, delta, id)
	if err != nil || ok {
		return change, err
	}
...
func (r *userRepository) SetBalance(ctx context.Context, id int64, value float64) (service.BalanceChange, error) {
	...
	const query = `UPDATE users AS u SET balance = $1, updated_at = NOW()
		FROM (SELECT id, balance FROM users WHERE id = $2 AND deleted_at IS NULL) AS prev
		WHERE u.id = prev.id AND u.deleted_at IS NULL RETURNING prev.balance, u.balance`
	change, ok, err := scanBalanceChange(ctx, clientFromContext(ctx, r.client), query, value, id)
	if err != nil {
		return service.BalanceChange{}, err
	}
	if !ok {
...
func (r *userRepository) currentBalance(ctx context.Context, id int64) (float64, error) {
	rows, err := clientFromContext(ctx, r.client).QueryContext(ctx, `SELECT balance FROM users WHERE id = $1 AND deleted_at IS NULL`, id)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
...
func scanBalanceChange(ctx context.Context, client *dbent.Client, query string, args ...any) (service.BalanceChange, bool, error) {
	...
	defer rows.Close()
	if !rows.Next() {
		return service.BalanceChange{}, false, rows.Err()
	}
...
func (r *userRepository) BatchUpdateLimits(ctx context.Context, userIDs []int64, concurrency, rpmLimit *int) (int, error) {
	...
	query := fmt.Sprintf("UPDATE users SET %s WHERE id = ANY($%d) AND deleted_at IS NULL", strings.Join(setClauses, ", "), len(args))
...
func existsByEmailAliasWithClient(ctx context.Context, client *dbent.Client, email string) (bool, error) {
	...
	preds := make([]predicate.User, 0, 4*len(probes))
	for _, probe := range probes {
		base := probe.Local + "@" + probe.Domain
		plusPattern := escapeLikeWildcards(probe.Local) + "+%@" + escapeLikeWildcards(probe.Domain)
		if probe.DotInsensitive {
			preds = append(preds, dotStrippedEmailEQ(base), dotStrippedEmailLike(plusPattern))
			continue
		}
		// Dots are significant outside the Gmail family. Keeping them in the
		// SQL probe prevents unrelated dot variants from consuming the bounded
		// candidate set before the real alias is examined.
		preds = append(preds,
			normalizedEmailEQ(base),
			normalizedEmailEQ(base+"."),
			normalizedEmailLike(plusPattern),
			normalizedEmailLike(plusPattern+"."),
		)
	}
...
func normalizedEmailExpr(b *entsql.Builder, s *entsql.Selector) *entsql.Builder {
	return b.WriteString("LOWER(TRIM(").Ident(s.C(dbuser.FieldEmail)).WriteString("))")
}

func normalizedEmailEQ(value string) predicate.User {
	return predicate.User(func(s *entsql.Selector) {
		s.Where(entsql.P(func(b *entsql.Builder) {
			normalizedEmailExpr(b, s).WriteString(" = ").Arg(value)
		}))
	})
}
...
func normalizedEmailLike(pattern string) predicate.User {
	return predicate.User(func(s *entsql.Selector) {
		s.Where(entsql.P(func(b *entsql.Builder) {
			normalizedEmailExpr(b, s).WriteString(" LIKE ").Arg(pattern).WriteString(` ESCAPE '\'`)
		}))
	})
}
