...
func (c *schedulerCache) GetSnapshot(ctx context.Context, bucket service.SchedulerBucket) ([]*service.Account, bool, error) {
	...
	// Interleave metadata and last-used keys so hydration needs one MGET pass.
	// Keeping the two values adjacent also avoids a mutable side lookup map.
	...
	for i := 0; i < len(values); i += 2 {
		...
		if err := applySchedulerLastUsed(account, values[i+1]); err != nil {
			return nil, false, err
		}
...
func applySchedulerLastUsed(account *service.Account, value any) error {
	...
	if err != nil {
		return fmt.Errorf("invalid last_used cache value: %w", err)
	}
...
func filterSchedulerCredentials(credentials map[string]any) map[string]any {
	...
	keys := []string{
		...
		"openai_downstream_cache_markup_enabled",
		"openai_downstream_cache_markup_threshold_tokens",
		"openai_downstream_cache_markup_percent_bps",
