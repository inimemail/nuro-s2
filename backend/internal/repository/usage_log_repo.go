...
func appendUsageLogBillingModeWhereCondition(conditions []string, args []any, billingMode string) ([]string, []any) {
	...
	case service.BillingModeImage:
		conditions = append(conditions, fmt.Sprintf("(billing_mode = %s OR (COALESCE(image_count, 0) > 0 AND COALESCE(video_count, 0) <= 0))", placeholder))
	case service.BillingModeVideo:
		conditions = append(conditions, fmt.Sprintf("(billing_mode = %s OR COALESCE(video_count, 0) > 0)", placeholder))
	case service.BillingModeToken:
		conditions = append(conditions, fmt.Sprintf("(billing_mode = %s OR ((billing_mode IS NULL OR billing_mode = '') AND COALESCE(image_count, 0) <= 0 AND COALESCE(video_count, 0) <= 0))", placeholder))
	default:
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	if filters.UpstreamModelMismatch != nil {
		if *filters.UpstreamModelMismatch {
			conditions = append(conditions, "upstream_model_mismatch IS TRUE")
		} else {
			conditions = append(conditions, "upstream_model_mismatch IS FALSE")
		}
	}
...
func resolveModelDimensionExpression(modelType string) string {
	...
	upstreamExpr := fmt.Sprintf("COALESCE(NULLIF(TRIM(upstream_model), ''), %s)", requestedExpr)
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		LongContextBillingApplied: longContextApplied,
