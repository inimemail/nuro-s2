...
type passkeySessionStore struct{ redis *redis.Client }
...
func (s *passkeySessionStore) Store(ctx context.Context, session *service.PasskeySession, ttl time.Duration) (string, error) {
	if session == nil || ttl <= 0 || s == nil || s.redis == nil {
		return "", fmt.Errorf("invalid passkey session store request")
	}
	random := make([]byte, 32)
...
func (s *passkeySessionStore) Consume(ctx context.Context, token string) (*service.PasskeySession, error) {
	token = strings.TrimSpace(token)
	if token == "" || len(token) > 128 || s == nil || s.redis == nil {
