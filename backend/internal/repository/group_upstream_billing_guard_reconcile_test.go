...
func TestReconcileUpstreamBillingGuardAccountsPreservesOverridesAndRefreshesDisabledAccounts(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH affected.*UPDATE accounts.*NOT EXISTS.*g2\.platform = a\.platform.*g2\.platform IN \('openai', 'anthropic', 'gemini', 'grok', 'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go'\).*SELECT id FROM disabled`).
		WithArgs(int64(9)).
...
func TestReconcileRemovedGroupBillingGuardsUsesRemainingSamePlatformPolicies(t *testing.T) {
	db, mock := newSQLMock(t)
	mock.ExpectQuery(`(?s)UPDATE accounts a.*a\.id = ANY\(\$1\).*a\.platform IN \('openai', 'anthropic', 'gemini', 'grok', 'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go'\).*g\.platform = a\.platform.*RETURNING a\.id`).
