...
func TestOpenAIBodyLimitFailoverExhaustedReturnsRedactedJSON413(t *testing.T) {
	...
	require.Equal(t, service.OpenAIRequestBodyTooLargeClientMessage, errBody["message"])
	require.NotContains(t, rec.Body.String(), "must-not-leak")
}

func TestOpenAIBodyLimitFailoverExhaustedReturnsRedactedResponsesSSE(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(nil))
