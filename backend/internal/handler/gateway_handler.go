...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	c.Request = c.Request.WithContext(service.WithGeminiCompatibleThinkingRequest(c.Request.Context(), parsedReq.Model, body))
	...
	for {
		...
		for {
			...
			parsedReq.Body = h.gatewayService.ApplyBedrockCCCompat(c, parsedReq.Body, parsedReq.Model, account, apiKey.GroupID)
			body = parsedReq.Body
...
func (h *GatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, platform string, streamStarted bool) {
	...
	if platform == service.PlatformOpenAI &&
		failoverErr.SkipPoolSoftCooldown &&
...
func generateRealisticMsgID() string {
	...
	if _, err := rand.Read(randomBytes); err != nil {
		return fallbackRealisticMsgID(time.Now().UnixNano())
	}
	b := make([]byte, idLen)
	for i := range b {
...
func fallbackRealisticMsgID(unixNano int64) string {
	return fmt.Sprintf("msg_01%022d", unixNano)
}
...
func (h *GatewayHandler) maybeLogCompatibilityFallbackMetrics(reqLog *zap.Logger) {
	...
	reqLog.Info("gateway.compatibility_fallback_metrics",
		...
		zap.Int64("sticky_cache_write_timeout_total", metrics.StickyCacheWriteTimeoutTotal),
		zap.Int64("sticky_cache_write_error_total", metrics.StickyCacheWriteErrorTotal),
		zap.Int64("sticky_cache_write_latency_ms_total", metrics.StickyCacheWriteLatencyMsTotal),
