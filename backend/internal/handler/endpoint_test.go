...
func TestDeriveUpstreamEndpoint(t *testing.T) {
	tests := []struct {
		...
		{"openai responses compact fallback", EndpointResponsesCompact, "", service.PlatformOpenAI, EndpointResponsesCompact},
		...
		{"kimi chat completions", EndpointChatCompletions, "/v1/chat/completions", service.PlatformKimi, EndpointChatCompletions},
		{"zhipu chat completions", EndpointChatCompletions, "/v1/chat/completions", service.PlatformZhipu, EndpointChatCompletions},
...
func TestGetInboundEndpoint_FallbackPrefersConcreteURLPath(t *testing.T) {
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	got := GetInboundEndpoint(c)
	require.Equal(t, EndpointResponsesCompact, got)
}
