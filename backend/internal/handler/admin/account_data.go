...
func (h *AccountHandler) ExportData(c *gin.Context) {
	...
	for i := range accounts {
		...
		dataAccounts = append(dataAccounts, DataAccount{
			...
			Extra:              service.RedactOpenCodeGoUsageExtra(acc.Extra),
			ProxyKey:           proxyKey,
...
func (h *AccountHandler) listAccountsFiltered(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode, poolMode, sortBy, sortOrder string) ([]service.Account, error) {
	page := 1
	pageSize := dataPageCap
	var out []service.Account
	for {
		items, total, err := h.adminService.ListAccounts(ctx, page, pageSize, platform, accountType, status, search, groupID, privacyMode, poolMode, sortBy, sortOrder)
		if err != nil {
			return nil, err
		}
		out = append(out, items...)
...
func (h *AccountHandler) resolveExportAccounts(ctx context.Context, ids []int64, c *gin.Context) ([]service.Account, error) {
	...
	return h.listAccountsFiltered(ctx, platform, accountType, status, search, groupID, privacyMode, poolMode, sortBy, sortOrder)
}
