...
// OpenAIDeviceAuthStartRequest represents the request for starting Codex device auth.
type OpenAIDeviceAuthStartRequest struct {
	ProxyID *int64 `json:"proxy_id"`
}

// StartDeviceAuth starts OpenAI Codex device-code authorization.
...
func (h *OpenAIOAuthHandler) StartDeviceAuth(c *gin.Context) {
	...
	result, err := h.openaiOAuthService.StartDeviceAuth(c.Request.Context(), req.ProxyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// OpenAIDeviceAuthExchangeRequest represents the request for completing Codex device auth.
type OpenAIDeviceAuthExchangeRequest struct {
	SessionID string `json:"session_id" binding:"required"`
	ProxyID   *int64 `json:"proxy_id"`
}

// ExchangeDeviceAuth exchanges an approved device-code session for OpenAI tokens.
// POST /api/v1/admin/openai/device-auth/exchange
func (h *OpenAIOAuthHandler) ExchangeDeviceAuth(c *gin.Context) {
	var req OpenAIDeviceAuthExchangeRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	tokenInfo, err := h.openaiOAuthService.ExchangeDeviceAuth(c.Request.Context(), req.SessionID, req.ProxyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
