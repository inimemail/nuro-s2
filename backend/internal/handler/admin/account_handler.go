...
type UpdateAccountRequest struct {
	...
	UpstreamBillingGuardGroupLimits    *map[int64]float64 `json:"upstream_billing_guard_group_limits"`
	UpstreamBillingGuardGroupMinLimits *map[int64]float64 `json:"upstream_billing_guard_group_min_limits"`
	ExpiresAt                          *int64             `json:"expires_at"`
	AutoPauseOnExpired                 *bool              `json:"auto_pause_on_expired"`
	ProbeEnabled                       *bool              `json:"upstream_billing_probe_enabled"`
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	poolMode := strings.TrimSpace(c.Query("pool_mode"))
	...
	accounts, total, err := h.adminService.ListAccounts(c.Request.Context(), page, pageSize, platform, accountType, status, search, groupID, privacyMode, poolMode, sortBy, sortOrder)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AccountHandler) Update(c *gin.Context) {
	...
	account, err := h.adminService.UpdateAccount(c.Request.Context(), accountID, &service.UpdateAccountInput{
		...
		Status:                             req.Status,
		GroupIDs:                           req.GroupIDs,
		UpstreamBillingGuardEnabled:        req.UpstreamBillingGuardEnabled,
		UpstreamBillingGuardGroupLimits:    req.UpstreamBillingGuardGroupLimits,
		UpstreamBillingGuardGroupMinLimits: req.UpstreamBillingGuardGroupMinLimits,
		ExpiresAt:                          req.ExpiresAt,
		AutoPauseOnExpired:                 req.AutoPauseOnExpired,
		ProbeEnabled:                       req.ProbeEnabled,
...
// RevertProxyFallback handles restoring the original proxy after an expiry fallback.
...
// BatchDelete deletes accounts with bounded concurrency while preserving
// stable, per-account results for the admin UI.
...
func (h *AccountHandler) BatchDelete(c *gin.Context) {
	...
	for _, id := range rootIDs {
		accountID := id
		g.Go(func() error {
			deleteErr := h.adminService.DeleteAccount(gctx, accountID)
			mu.Lock()
			defer mu.Unlock()

			affectedIDs := append([]int64{accountID}, dependentIDs[accountID]...)
			if deleteErr != nil {
				slog.Error("batch_delete_account_failed",
					"account_id", accountID,
					"error", logredact.RedactText(deleteErr.Error()),
				)
				message := safeAdminBatchErrorMessage(deleteErr)
				for _, affectedID := range affectedIDs {
					failedIDs = append(failedIDs, affectedID)
					errorsByAccount = append(errorsByAccount, deleteError{AccountID: affectedID, Error: message})
				}
				return nil
			}
	...
	executeAdminIdempotentJSON(c, "admin.accounts.batch_create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		for _, item := range req.Accounts {
			...
			if err := service.ValidateUpstreamRequestIDHeaderExtra(item.Extra); err != nil {
				failed++
				results = append(results, gin.H{"name": item.Name, "success": false, "error": safeAdminBatchErrorMessage(err)})
				continue
			}
