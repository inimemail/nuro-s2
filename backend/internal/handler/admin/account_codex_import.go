...
func resolveCodexImportExpiry(req CodexSessionImportRequest, item *codexImportAccount) (*int64, *time.Time, *bool, []string, error) {
	...
	// Agent Identity has no OAuth token lifetime. Runtime/task expiry is
	// recovered by the assertion path and must not auto-pause the account.
	if item.IsAgentIdentity {
...
func buildCodexAgentIdentityKeys(accountID string) []string {
	...
	// Team/workspace imports share user IDs and emails. Account identity is the
	// only stable boundary that prevents cross-team credential merging.
