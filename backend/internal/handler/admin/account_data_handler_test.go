...
func TestExportDataPassesAccountFiltersAndSort(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodGet,
		"/api/v1/admin/accounts/data?platform=openai&type=oauth&status=active&group=12&privacy_mode=blocked&pool_mode=image_pool&search=keyword&sort_by=priority&sort_order=desc",
		nil,
	)
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
	...
	require.Equal(t, "image_pool", adminSvc.lastListAccounts.poolMode)
