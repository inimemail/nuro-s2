...
func TestStripeRefundUsesStableAmountSpecificIdempotencyKey(t *testing.T) {
	...
	refund("12.35")
	_, err := provider.Refund(context.Background(), payment.RefundRequest{
		TradeNo: "pi_legacy_789",
		Amount:  "1.00",
	})
	require.NoError(t, err)
	_, err = provider.Refund(context.Background(), payment.RefundRequest{Amount: "2.00"})
	require.NoError(t, err)

	require.Len(t, backend.params, 5)
	...
	require.Equal(t, "re-pi_legacy_789-100", *backend.params[3].IdempotencyKey)
	require.Nil(t, backend.params[4].IdempotencyKey)
}
