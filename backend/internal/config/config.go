...
// WebAuthnConfig defines the deployment's WebAuthn relying-party boundary.
// These values are static and must never be inferred from request headers.
...
func (c *Config) SetTrustForwardedIPForAPIKeyACL(enabled bool) {
	settings := c.ForwardedClientIPSettings()
	c.SetForwardedClientIPSettings(enabled, settings.Headers)
}
...
func normalizeProxyProbeURLs(targets []ProbeURLConfig) ([]ProbeURLConfig, error) {
	...
	for i, target := range targets {
		...
		if rawURL == "" || parser == "" {
			return nil, fmt.Errorf("entry %d: url and parser are required", i)
		}
		switch parser {
		case "ip-api", "ipify", "httpbin", "chatgpt-trace":
		default:
			return nil, fmt.Errorf("entry %d: unsupported parser %q", i, target.Parser)
		}
		parsed, err := url.Parse(rawURL)
		if err != nil || parsed.Host == "" || (parsed.Scheme != "http" && parsed.Scheme != "https") {
			return nil, fmt.Errorf("entry %d: invalid http(s) url %q", i, target.URL)
		}
...
type BillingConfig struct {
	...
	// MinimumBalanceReserve keeps a small positive balance reserve before a
	// balance-mode user is eligible for new requests. 0 preserves legacy behavior.
...
type GatewayConfig struct {
	...
	// DisableCodexIdentityEnforcement keeps the legacy Codex identity pairing
	// behavior when true. Local deployments default to true; operators can
	// explicitly enable canonical identity enforcement by setting it to false.
...
type GatewaySchedulingConfig struct {
	...
	// PreferSoonestReset: 在已限定同一优先级、同一最低负载候选后，优先选择更快重置的账号。
...
// InvalidAuthAbuseConfig bounds repeated invalid API-key lookups per client
// address. It never changes scheduler or account state.
...
func setDefaults() {
	...
	// Passkeys are opt-in because every deployment must explicitly declare
	// its relying-party domain and browser origins.
	...
	viper.SetDefault("batch_image.max_download_bytes_per_request", int64(536870912))
	...
	viper.SetDefault("gateway.disable_codex_identity_enforcement", true)
	...
	viper.SetDefault("gateway.openai_edge_rs.lease_ttl_ms", 2*60*1000)
	...
	viper.SetDefault("gateway.openai_edge_rs.max_idle_conns_per_account", 128)
	...
	viper.SetDefault("gateway.concurrency_slot_ttl_minutes", 2) // 短故障回收 TTL；活跃请求自动续期
	...
	// 默认 sync：队列满时由提交方内联执行（提交点在响应写出之后，不阻塞首 token）。
	// sample/drop 会在溢出时静默丢弃计费任务，造成扣费与 usage_logs 对账缺口。
