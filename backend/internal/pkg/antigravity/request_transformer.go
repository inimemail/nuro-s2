...
func TransformClaudeToGeminiWithOptions(claudeReq *ClaudeRequest, projectID, mappedModel string, opts TransformOptions) ([]byte, error) {
	...
	// Gemini reasoning 模型在未携带工具时不接受强制空 ToolConfig。
...
func buildSystemInstruction(system json.RawMessage, modelName string, opts TransformOptions, tools []ClaudeTool) *GeminiContent {
	...
	if len(system) > 0 {
		...
		if err := json.Unmarshal(system, &sysStr); err == nil {
			sysStr = stripLeadingClaudeAttribution(sysStr)
			...
			if err := json.Unmarshal(system, &sysBlocks); err == nil {
				for _, block := range sysBlocks {
					block.Text = stripLeadingClaudeAttribution(block.Text)
...
func buildGenerationConfig(req *ClaudeRequest) *GeminiGenerationConfig {
	...
	// Gemini reasoning 模型会拒绝部分采样参数，保持请求最小化。
	if !isReasoning {
