...
func BuildBillingSummary(config *BillingConfig) *BillingSummary {
	...
	if !hasWeekly && !hasMonthly {
		// A successful billing response with an empty config is still an
		// authoritative observation. Free accounts may use this shape; callers
		// stamp the weekly/monthly observation time during the merge.
		return summary
	}
