...
func (i *ResponsesInputItem) UnmarshalJSON(data []byte) error {
	...
	*i = ResponsesInputItem{}
	...
	i.outputRaw = append([]byte(nil), output...)
...
func (i ResponsesInputItem) MarshalJSON() ([]byte, error) {
	...
	if raw := bytes.TrimSpace(i.outputRaw); len(raw) > 0 {
		output = append(json.RawMessage(nil), raw...)
	} else if i.Output != "" {
		encoded, err := json.Marshal(i.Output)
		if err != nil {
			return nil, err
		}
		output = encoded
...
func (o *ResponsesOutput) UnmarshalJSON(data []byte) error {
	...
	*o = ResponsesOutput(decoded)
	...
	if err := json.Unmarshal(arguments, &o.Arguments); err != nil {
...
type ResponsesStreamEvent struct {
	...
	// response.content_part.added / done and response.reasoning_summary_part.added / done
	...
	// Strict Responses clients require this field on every event. Zero is a
	// valid first-event value and is used by synthesized terminal events.
