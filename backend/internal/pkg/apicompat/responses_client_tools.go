...
// function protocol. Call IDs can return to their original fc_ prefix; output
// item IDs have no function-protocol counterpart and are dropped.
...
type responsesClientToolStreamCall struct {
	kind string
	name string
	// Keep the upstream ID for matching later events and emit the type-correct
	// client ID after restoring the original tool-call item type.
	callID       string
	itemID       string
	clientItemID string
	outputIdx    int
