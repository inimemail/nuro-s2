	...
	for _, p := range parts {
		...
		case "file":
			if p.File != nil && (p.File.FileData != "" || p.File.FileID != "") {
				responseParts = append(responseParts, ResponsesContentPart{
					Type: "input_file", Filename: p.File.Filename, FileData: p.File.FileData, FileID: p.File.FileID,
				})
			}
		}
	}
...
func convertChatToolsToResponses(tools []ChatTool, functions []ChatFunction) []ResponsesTool {
	...
	for _, t := range tools {
		if strings.EqualFold(strings.TrimSpace(t.Type), "x_search") {
			out = append(out, ResponsesTool{Type: "x_search", AllowedXHandles: t.AllowedXHandles, ExcludedXHandles: t.ExcludedXHandles, FromDate: t.FromDate, ToDate: t.ToDate, EnableImageUnderstanding: t.EnableImageUnderstanding, EnableVideoUnderstanding: t.EnableVideoUnderstanding})
			continue
		}
